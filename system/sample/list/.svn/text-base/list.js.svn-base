	...
	var sampledata = [{
        ...
        var ListView = Backbone.View.extend({
                ...
                events: {
                    ...
                    "click #ca_hojin_select"	:	"_onHojinSelectClick"		// 法人選択ボタン押下時
                ...
                initialize: function() {
                        ...
                        // validatorエラー時の表示領域
                        $('.cl_echoback').hide();
                        this.validator = clutil.validator($('#ca_search_form'), {
                        ...
                        // 選択サブ画面配置
                        hojinSelector = new cm330SelectorView ();

                },
                
                // 初期データ取得後に呼ばれる関数
                inittypename: function() {
                	// 区分selectorの作成
                    clutil.cltypeselector($('#ca_hojin_cd'), "gs_typename_FLAG");
                },
                
    			// 選択サブ画面起動
    			_onHojinSelectClick : function() {
    				hojinSelector.show(
        					$('#hojinSelectorArea'), 	// 配置場所
        					$('#ca_main')				// 親View
        			);
    				// 選択サブ画面復帰処理
    				hojinSelector.okProc = function(data){
    					if (data != null && data.length == 1) {
        					$("#ca_hojin_name").val(data[0].hojin_name);
    					}
    				}
    			},
