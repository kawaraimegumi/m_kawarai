	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			this.dataGrid = new ClGrid.ClAppGridView({
				...
				autoHeightDataCount: 12	// 12行超えるとキャンパス高を固定する。
			});
			...
			// データ部のメタデータGetterをオーバーライド。
			// 特定行（先頭3行）を行削除対象にしないためのメタデータを仕組む。
			this.dataGrid.getMetadata = function(rowIndex){
				console.info('clgridview.getMetadata: ', arguments);
				if(rowIndex < 3){
					return {
			...
			// 行削除［×］ボタンのトグルイベントをリスンする
			this.listenTo(this.dataGrid, 'row:delToggle', function(arg){
				var dataGrid = arg.dataGrid;	// ClGrid.ClAppGridView インスタンス
				var item = arg.item;			// 対象行データ
				if(arg.isDeleted){
					console.info('row:delToggle, toggle[off] - 削除 - ', item);
				}else{
					console.info('row:delToggle, toggle[on] - 復帰 - ', item);
				}
			});
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		render: function() {
			...
			this.dataGrid.render().setData({
				...
				gridOptions: {			// Slick.Grid へ渡すオプション
					frozenColumn: 1		// 列固定col[0]～col[1]まで。
				},
				data: boodata(10),		// データ。
