	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			this.deserialize({
				...
				srchCode: null,			// 補正項目コード
				srchName: null,			// 補正項目名称
			});
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			this.mdBaseView = new clutil.View.MDBaseView({
				title: '補正項目マスタ',
				subtitle: '一覧',
				btn_csv: false,	// 初期は非表示
			});

			// 検索パネル
			this.srchCondView = new SrchCondView({el: this.$('#ca_srchArea')});

			// グループID -- AMRSV0010 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMRSV0010';
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		doSrch: function(srchReq, selectedIds, $focusElem){
			...
			var defer = clutil.postJSON('AMRSV0010', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMRSV0010GetRsp.adjustItemList;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
		...
		getF_initdisp: function(initDispFlag) {
			return initDispFlag != 0 ? "○" : "";
		},
		...
		doDownload: function(){
			...
			var defer = clutil.postDLJSON('AMRSV0010', srchReq);
		...
		_doOpeAction: function(rtyp, e){
			var url = clcom.appRoot + '/AMRS/AMRSV0020/AMRSV0020.html';
			...
			if(this.savedReq){
				...
				myData = {
					...
					savedCond: this.savedReq.AMRSV0010GetReq,
					selectedIds: this.recListView.getSelectedRecs()
				};
				destData = {
					opeTypeId: rtyp,
