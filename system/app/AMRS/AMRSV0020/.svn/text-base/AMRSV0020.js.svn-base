...
$(function() {
	...
	var MainView = Backbone.View.extend({
		...
		_onMDGetCompleted: function(args, e){
			...
			var data = args.data;
			var adjustItem = data.getRsp.adjustItem;
			...
			case 'OK':
				...
				clutil.data2view(this.$("#ca_form"), adjustItem);
				...
				case am_proto_defs.AM_PROTO_COMMON_RTYPE_UPD:
					...
					// コードは入力不可
					clutil.inputReadonly(this.$('#ca_adjustItemCode'));

					// 初期フォーカスは補正項目名称
					this.resetFocus($("#ca_adjustItemName"));
					break;
		...
		initUIElement: function() {
			...
			// 補正箇所区分
			clutil.cltypeselector(this.$("#ca_adjGrpTypeID"), amcm_type.AMCM_TYPE_ADJ_GRP, 1);

			// カウンタ設置 #20140324
			clutil.cltxtFieldLimit($("#ca_adjustItemCode"));
			clutil.cltxtFieldLimit($("#ca_adjustItemName"));
		...
		_buildGetReqFunction: function(opeTypeId, pgIndex){
			...
			var getReq = {
				...
				// 取引先マスタ検索リクエスト
				getReq: {
					srchAdjustItemID: chkData.adjustItemID,		// 補正項目ID
		...
		_buildSubmitReqFunction: function(opeTypeId, pgIndex){
			...
			var adjustItem = clutil.view2data($("#ca_form"));
