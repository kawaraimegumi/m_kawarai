...
MyApp._Behaviors.Grid = Marionette.Behavior.extend({
	...
	onUpdateData: function(data, columns, options){
		...
		if(_.isFunction(gridOptions)){
			gridOptions = gridOptions(columns, data);
		}
	...
	onDomRefresh: function(){
		if (this.view.dataGrid){
			if(this.options.frozenRow != null){
				this.view.dataGrid.grid.setOptions({
					frozenRow: this.options.frozenRow});
			}
			this.view.dataGrid.grid.resizeCanvas();
		}
	},
