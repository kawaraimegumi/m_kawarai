			...
			.add({
				...
				onDependChange: function(e){
					var makerItgrp_code = e.model.get('makerItgrpCode'),
						itgrp_id = Number($("#ca_itgrpID").val()),
...
//					_this.getStoreBaseStock(attr, view, options);
//				});
			});
...
//				storeID		: rsp.baseStockRecord.storeCodeName.id,		// 店舗ID(表示ネタ確認用)
			...
			$.each(rsp.itemRecords, function(){
				...
				for (var i = 0; i < cellLoopMax; i++){
					...
					var baseStockQy = _this.getCellData(cellRecordsMap, this.itemCodeName.id, sizeData.sizeID).baseStockQy;
					list.push({
		...
		getCellData: function(cellRecordsMap, itemID, sizeID){
			var id = itemID + ':' + sizeID;
