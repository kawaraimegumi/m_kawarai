...
$(function(){
	...
	var SrchCondView = Backbone.View.extend({
		...
		events: {
			...
			'click #ca_link_store'	:	'_onLinkStoreClick',	// 店舗別基準在庫一覧リンク押下時
		...
		_onLinkStoreClick: function(e) {
			var url = clcom.appRoot + '/AMDS/AMDSV0051/AMDSV0051.html';
			var myData, destData;
			myData = {
				savedReq: null,
...
//			var _this = this;
			...
			// グループID -- AMDSV0050 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMDSV0050';
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		buildReq: function(argSrchReq){
			...
			// 結果表示種別
			srchReq.srchResultType = AMDSV0050Req.AMDSV0050_RESULTTYPE_ITEM;	// 品種別
		...
		doSrch: function(srchReq, chkData, $focusElem){
			...
			var defer = clutil.postJSON('AMDSV0050', srchReq).done(_.bind(function(data){
				...
				var recs = data.AMDSV0050GetRsp.itemRecords;
				if(_.isEmpty(recs)){
