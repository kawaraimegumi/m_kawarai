...
//			this.srchDatePicker = clutil.datepicker(this.$('#ca_srchStartDay'));
			...
			this.deserialize({
				...
				srchItgrpID: 0,							// 品種
				srchRankCode: null,						// 店舗ランクパターンコード
				srchRankName: null,						// 店舗ランクパターン名
				srchSub1ID: 0,							// サブクラス1
				srchSub2ID: 0,							// サブクラス2
				srchBrandID: 0,							// ブランド
				srchStyleID: 0,							// スタイル
...
//		},
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMDSV0010 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMDSV0010';
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		initUIElement: function(){
			...
			console.log('MainView initUIElement.');
			// 検索条件を再指定ボタンを隠す
		...
		doSrch: function(srchReq, chkData, $focusElem){
			...
			var defer = clutil.postJSON('AMDSV0010', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMDSV0010GetRsp.ptnRecords;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
		...
		doDownload: function(){
			...
			var defer = clutil.postDLJSON('AMDSV0010', srchReq);
...
//				this.$('#ca_AMRSV0010_add').focus();
		...
		_doOpeAction: function(rtyp, pgIndex, e){
			var url = clcom.appRoot + '/AMDS/AMDSV0020/AMDSV0020.html';
			...
			if(this.savedReq){
				...
				myData = {
					...
					savedCond: this.savedReq.AMDSV0010GetReq,
