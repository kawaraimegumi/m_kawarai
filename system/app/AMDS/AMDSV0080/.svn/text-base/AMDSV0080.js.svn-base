...
$(function(){
	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			// 自動振分停止期間
//			this.srchDatePicker = clutil.datepicker(this.$('#ca_srchStDate'));
			this.srchDatePicker = clutil.datepicker(this.$('#ca_srchEdDate'));
...
////				};
//				console.log(clcom.userInfo.user_typeid);
...
//				srchOrgID: orgID,						// 組織
				srchItgrpID: 0,							// 品種
				srchStDate: 0,							// 自動振分停止期間（開始） yyyymmdd
				srchEdDate: 0							// 自動振分停止期間（終了） yyyymmdd
			});
...
//			var _this = this;
			...
			// グループID -- AMDSV0080 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMDSV0080';
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		doSrch: function(srchReq, chkData, $focusElem){
			...
			var defer = clutil.postJSON('AMDSV0080', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMDSV0080GetRsp.autoStopRecords;
				if(_.isEmpty(recs)){
				...
				var unitID = srchReq.AMDSV0080GetReq.srchUnitID;
				$.each(recs,function(){
					this.unitID = unitID;
				});
		...
		doDownload: function(){
			...
			var defer = clutil.postDLJSON('AMDSV0080', srchReq);
...
//				this.$('#ca_AMRSV0010_add').focus();
		...
		_doOpeAction: function(rtyp, pgIndex/*一覧画面では使用しない*/, e){
			var url = clcom.appRoot + '/AMDS/AMDSV0090/AMDSV0090.html';
			...
			if(this.savedReq){
				...
				myData = {
					...
					savedCond: this.savedReq.AMDSV0080GetReq,
		...
		load: function(model, returnValue) {
			...
			if(!_.isEmpty(model.savedCond)){
				...
				if (clcom.userInfo.user_typeid != amcm_type.AMCM_VAL_USER_STORE && clcom.userInfo.user_typeid != amcm_type.AMCM_VAL_USER_STORE_MAN) {
					...
					$("#ca_srchOrgID").attr("readonly", (unitID == 0));
					$("#ca_btn_org_select").attr("disabled", (unitID == 0));
				}
			}
			// 再検索
