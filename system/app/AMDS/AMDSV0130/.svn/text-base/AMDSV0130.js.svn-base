...
//						var stDate = args.selectedRows[0].stDate;
//						if(today >= stDate){
//							return false;
//						}
					}
...
//					}
	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			// 検索日
			clutil.datepicker(this.$('#ca_srchStDate'));
			clutil.datepicker(this.$('#ca_srchEdDate'));
			...
			if(unit == null){
				...
				this.deserialize({
					...
					srchItgrpID: null,				// 品種
					srchStDate: null,	// 指示日 yyyymmdd
					srchEdDate: null					// 指示日 yyyymmdd
				});
			}
			else{
				// 初期値を設定
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMDSV0130 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMDSV0130';
		...
		doSrch: function(srchReq, selectedIds){
			...
			var defer = clutil.postJSON('AMDSV0130', srchReq).done(_.bind(function(data){
				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMDSV0130GetRsp.autoStopRecords;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
				...
				// id に contrlID を指定(編集から戻った際の再チェック用)
				for(var i=0; i<recs.length; i++){
					recs[i].id = recs[i].controlID;
				}
...
//			var defer = clutil.postDLJSON('AMDSV0130', srchReq);
...
//			}, this));
		...
		_doOpeAction: function(rtyp, e){
			var url = clcom.appRoot + '/AMDS/AMDSV0140/AMDSV0140.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
					savedReq: this.savedReq,
					savedCond: this.savedReq.AMDSV0130GetReq,
