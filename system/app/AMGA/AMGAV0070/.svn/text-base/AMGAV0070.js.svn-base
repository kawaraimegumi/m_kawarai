...
$(function() {
	...
	// コンフィギュレーション：MD版ABC分析フラグ
	Ana.Config.cond.CACMV0180.isMDABCAxis = true;
	Ana.Config.cond.CACMV0210.disp_way = 'disabled';	// 表示項目並びを変更できないように。
	Ana.Config.cond.CACMV0210.f_subtotal = 'disabled';	// 小計表示を変更できないように。
...
//			this.CAPAV0080Panel.render();		// 条件表示
	...
	anaProc.iniGet().done(_.bind(function(){
		...
		c.dispitemlist = this.buildDispItems([
			{
				// 0x11105d: itemId[0x5d] name[ABC評価実績]
				dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_ABC,
				sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VAL
			},{
				dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_QY_RANK,
				sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VAL
			},{
				// 0x111018: itemId[0x18] name[購入金額(税込)ランク実績]
				dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_AM_RANK,
				sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VAL
			},{
				// 0x111018: itemId[0x18] name[メーカー名]
				dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_ITEMATTR_MAKER_NAME,
				sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VAL
			},{
				// 0x111018: itemId[0x18] name[メーカー品番]
				dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_ITEMATTR_MAKER_CODE,
				sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VAL
			},{
				// 0x111018: itemId[0x18] name[カラー]
				dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_ITEMATTR_COLOR_NAME,
				sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VAL
			},{
				// 0x111018: itemId[0x18] name[商品名]
				dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_ITEMATTR_ITEM_NAME,
				sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VAL
			},{
			...
			//	// 0x117001: itemId[0x1] name[購入点数縦軸累計値]
			//	dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_QY_VACM,
			//	sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VAL
			//},{
				// 0x117001: itemId[0x1] name[購入点数縦軸累計構成比]
				dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_QY,
				sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VACMRT
			},{
				// 0x111015: itemId[0x15] name[購入金額(税込)実績]
				gbit: amanp_AnaDispItemDefs.AMAN_DI_G_SALE,
				dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_AM,
				sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VAL
			},{
				// 0x117015: itemId[0x15] name[購入金額(税込)縦軸構成比]
				gbit: amanp_AnaDispItemDefs.AMAN_DI_G_SALE,
				dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_AM,
				sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VSUMRT
			},{
			//	// 0x117015: itemId[0x15] name[購入金額(税込)縦軸累計値]
			//	dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_AM_VACM,
			//	sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VAL
			//},{
				// 0x117015: itemId[0x15] name[購入金額(税込)縦軸累計構成比]
				gbit: amanp_AnaDispItemDefs.AMAN_DI_G_SALE,
				dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_AM,
				sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VACMRT
			},{
		...
		c.dispvsortkeylist = this.buildSortkeyList([
			{
				// 0x111015: itemId[0x15] name[購入金額(税込)実績] - 降順
				dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_AM,
				sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VAL,
		...
		c.dispopt = {
			...
			disp_amunit	: amanp_AnaHead.AMANP_ANA_REQ_DISP_AMUNIT_1000	// 表示単位：千円単位
		};
		// ABC分析基準 - 売上高
		c.extra.mdabc_type = amanp_AnaHead.AMANP_ANA_REQ_ABC_TYPE_AM;
