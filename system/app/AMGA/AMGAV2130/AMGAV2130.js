...
$(function() {
	...
	var args = _.defaults({
		...
		f_anakind: 16,
		anamenuitem_name: "バスケット分析(LUCASベース)",
		directExec: null
	}, clcom.pageArgs);
...
//			});
			...
			this.CAPAV0090Panel.isValidCond = _.bind(function(anaproc){
				...
				var vAxisListEx2 = anaproc.cond.vAxisListEx2;
				var basketList = anaproc.cond.basket_list;
				var basketList2 = anaproc.cond.basket_list2;
				// 縦軸 - 1軸必須
...
//				}
				...
				if (vAxisListEx2.length < 1 || vAxisListEx2[0].kind == null || vAxisListEx[0].kind == 0) {
					var msg = '比較対象を設定してください。';
					this.validator.setErrorInfo({_eb_: msg});
					return false;
				}
...
//				el : $('#ca_CAPAV0060_view'),
			...
			// 軸設定パネル(その２)
			this.MDPAV0150Panel1 = new  MDPAV0150PanelView({
//				el : $('#ca_CAPAV0060_view'),
				id				: 'ca_MDPAV0150_view1',
				panelId			: 'MDPAV0150_1',
				side			: 'basis',
				navTitleLabel	: '基準を選択',
				$view		: $('#ca_panel'),
			...
			// 軸設定パネル(その３)
			this.MDPAV0150Panel2 = new  MDPAV0150PanelView({
//				el : $('#ca_CAPAV0060_view'),
				id				: 'ca_MDPAV0150_view2',
				panelId			: 'MDPAV0150_2',
				side			: 'alt',
				navTitleLabel	: '比較対象を選択',
				$view		: $('#ca_panel'),
				$parentView	: $('#ca_result'),
				anadata		: this.anadata,			// anaProc に入れる予定。
				anaProc		: this.anaProc,
...
//			this.CAPAV0080Panel.render();		// 条件表示
			...
			this.MDPAV0150Panel1.render();		// 軸
			this.MDPAV0150Panel2.render();		// 軸
