$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCMV1350SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_MDCMV1350_search"		:	"_onSearchClick",		// 検索ボタン押下
			"click #searchAgain"				:	"_onSearchAgainClick",	// 検索条件を再指定ボタン押下
			"click #ca_MDCMV1350_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_MDCMV1350_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_MDCMV1350_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_MDCMV1350_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_MDCMV1350_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_MDCMV1350_show_MDCMV1050"		:	"_onShowMDCMV1050Click",		// 商品分類選択ボタン押下

			"click #ca_MDCMV1350_main th.ca_th_sort"	:	"_onThSortClick",

			"click #ca_MDCMV1350_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV1350_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_MDCMV1350_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function() {
			...
			if (this.select_mode == clutil.cl_single_select) {
				this.$('.ca_MDCMV1350_multi').remove();
				this.$('#ca_MDCMV1350_chkall').remove();
			}
			clutil.initUIelement(this.$el);

			// 全選択チェックボックスを初期化
			this.chkall = clutil.checkallbox(this.$('#ca_MDCMV1350_chkall'),
					this.$('#ca_MDCMV1350_tbl'),
					this.$('#ca_MDCMV1350_tbody')
					);
			...
			if (this.isAnalyse_mode && this.select_mode != clutil.cl_single_select){
				...
				this.addtoSelected = clutil.addtoSelected(
						this.$('#ca_MDCMV1350_add'),
						this.$('#selected'),
						this.$('#mainColumn'));
			}

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_MDCMV1350_searchArea'),
					this.$('#ca_MDCMV1350_search'),
					this.$('#result'),
					this.$('#searchAgain'));

			// 商品分類選択画面
			this.MDCMV1350_MDCMV1050Selector = new  MDCMV1050SelectorView({
				el : this.$('#ca_MDCMV1350_MDCMV1050_dialog'),	// 配置場所
				$parentView		: this.$('#ca_MDCMV1350_main'),
				isAnalyse_mode	: this.isAnalyse_mode,			// 分析ユースかどうかフラグ？？？
//				ymd				: null,			// 検索日
				select_mode		: clutil.cl_single_select,	// 単一選択モード
				anaProc			: this.anaProc
			});
			this.MDCMV1350_MDCMV1050Selector.render();
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		initPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_MDCMV1350_pager1').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_MDCMV1350_pager_displaypanel1'),
			...
			this.$('#ca_MDCMV1350_pager2').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_MDCMV1350_pager_displaypanel2'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
					_this.onPageClick(pageNumber, itemsOnPage, true);
				},
		...
		show: function(editList, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV1350_main'), {
			...
			if (this.select_mode == clutil.cl_multiple_select &&
				...
				this.$('#ca_MDCMV1350_edit_tbl').html(html_source);
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_MDCMV1350_code'));
		},
		...
		_onShowMDCMV1050Click: function(e) {
			...
			// 選択された情報を初期値として検索する
			var initData = {};
			initData.func_id = this.$('#ca_MDCMV1350_itgrpfunc_id').val();
			initData.itgrp_id = this.$('#ca_MDCMV1350_itgrp_id').val();
			...
			this.MDCMV1350_MDCMV1050Selector.show(null, true, null, null, initData);
			//サブ画面復帰後処理
			this.MDCMV1350_MDCMV1050Selector.okProc = function(data) {
				if(data != null && data.length > 0) {
					_this.$('#ca_MDCMV1350_itgrpname').val(data[0].code + ":" + data[0].name);
					_this.$('#ca_MDCMV1350_itgrp_id').val(data[0].val);
					_this.$('#ca_MDCMV1350_itgrpfunc_id').val(data[0].func_id);
					_this.$('#ca_MDCMV1350_itgrplvl_id').val(data[0].lvl_id);
				}
				// ボタンにフォーカスする
				$(e.target).focus();
			}
		...
		_onThSortClick: function(e) {
			...
			// ソートキーを挿入
			this.req['sortreq'] = {
					sort_key : aman_se_coloritem_srch_req['AMGBA_PROTO_SORT_KEY_' + key],
		...
		setSort: function(req) {
			...
			var th = this.$('#ca_MDCMV1350_tbl').find('th[key=' + sortreq.key + ']');
		...
		_onSearchClick: function() {
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_MDCMV1350_searchArea'), 'ca_MDCMV1350_');
			// 全角、半角対応
		...
		onPageClick: function(pageNumber, itemsOnPage, isPager) {
			...
			// データを取得
			var uri = 'gsan_se_coloritem_srch';
			clutil.postAnaJSON(uri, this.req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (_this.resultList == null || _this.resultList.length == 0) {
						...
						case clutil.cl_multiple_select:
							$select_template = _this.$('#ca_MDCMV1350_tbody_multiple_tmp');
							break;
						case clutil.cl_single_select:
						default:
							$select_template = _this.$('#ca_MDCMV1350_tbody_single_tmp');
							break;
						}
						// 取得したデータを表示する
						$select_template.tmpl(_this.resultList).appendTo('#ca_MDCMV1350_tbody');

						clutil.initUIelement(_this.$('#ca_MDCMV1350_tbl'));
				...
				if (_this.$('#searchAgain').css('display') == 'none') {
					// 検索ボタンにフォーカスする
					_this.$('#ca_MDCMV1350_search').focus();
				} else {
					// 再検索ボタンにフォーカスする
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_MDCMV1350_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onAddClick:function() {
			...
			var chkId = [];
			$.each(this.$('#ca_MDCMV1350_tbody').find("[name=ca_MDCMV1350_chk]:checked"), function() {
			...
			if (chkId.length > 0) {
				...
				$.each(this.$('#ca_MDCMV1350_edit_tbl li'), function() {
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					// 重複チェック
					if(idlist.indexOf(select.coloritem_id) != -1){
						// 重複する場合は追加しない
						continue;
					}
					// 重複していない場合は追加
					var newdata = {};
					newdata.val = select.coloritem_id;
					newdata.code = select.code;
					newdata.name = select.name + ' ' + select.color_name;
					newdata.name2 = select.name + ' ' + select.color_name;
					// 種別を挿入する
					newdata.kind = amgbp_AnaDefs.AMGBA_DEFS_KIND_COLORITEM;

					html_source += this.makeEditTmp(newdata);
					num++;
					editlist.push(newdata);
				}
				this.$('#ca_MDCMV1350_edit_tbl').append(html_source);
			} else {
				// なにもしない
			}
			// 全選択チェックボックスを初期化
		...
		makeEditTmp: function(data) {
			...
			html_source += data.code + '</span><span class="btn-delete ca_MDCMV1350_edit_del"></span>'
		...
		_onEditDelAllClick: function() {
			...
			var idlist = [];
			var html_source = '';
			this.$('#ca_MDCMV1350_edit_tbl').html("");
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_MDCMV1350_edit_tbl').children());
			return editlist;
		},
		...
		_onCommitClick: function() {
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_MDCMV1350_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
