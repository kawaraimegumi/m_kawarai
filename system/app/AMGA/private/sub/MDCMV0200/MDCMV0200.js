$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCMV0200SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"change #ca_MDCMV0200_dispgroup"	:	"_onDispGroupSelect",	// 分類項目変更
			"click #ca_MDCMV0200_main .ca_MDCMV0200_add"
												:	"_onAddClick",			// 追加ボタン押下

			"click #ca_MDCMV0200_edit_tbody .ca_MDCMV0200_sort"
												:	"_onSortClick",			// ソートボタン押下

			"click #ca_MDCMV0200_main .ca_MDCMV0200_dispitem"
												:	"_onDispItemClick",		// 表示項目押下
			"click #ca_MDCMV0200_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下
			"click #ca_MDCMV0200_main .ca_MDCMV0200_edit_delete"
												:	"_onEditDeleteClick",	// 単一削除ボタン押下
			"click #ca_MDCMV0200_up"			:	"_onUpClick",			// ↑ボタン押下
			"click #ca_MDCMV0200_down"			:	"_onDownClick",			// ↓ボタン押下

			"click #ca_MDCMV0200_commit"		:	"_onCommitClick",		// 確定ボタン押下
			"click #ca_MDCMV0200_main .close"	:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV0200_cancel"		:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_MDCMV0200_main .modalBK"	:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function(editList) {
			...
			clutil.cltypeselector2(this.$('#ca_MDCMV0200_dispgroup'),
					dispgroupList, 0,
					1,
					'dispgroup');
			// 初期値
			...
			if (editList != null && editList.length > 0) {
				this.$('#ca_MDCMV0200_edit_tbody_tmp').tmpl(editList).appendTo('#ca_MDCMV0200_edit_tbody');
			}
			...
			if (vsortlist.length > 0) {
				...
				var tr = this.$('#ca_MDCMV0200_edit_tbody').find('tr[id=' + sortkey.dispitem_id + ']');
...
//			url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		show: function(editList, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV0200_main'), {
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_MDCMV0200_dispgroup'));
		},
		...
		_onDispGroupSelect: function(e) {
			// ボタンの初期化
			var buttons = this.$('button.ca_MDCMV0200_add');
			$(buttons).removeAttr('disabled');
			// 分類から表示項目を作成
			var dispitemList = this.getDispItem();
			this.$('#ca_MDCMV0200_tbody').empty();
			this.$('#ca_MDCMV0200_tbody_tmp').tmpl(dispitemList).appendTo('#ca_MDCMV0200_tbody');

			clutil.initUIelement(this.$('#ca_MDCMV0200_tbody'));
		},
		...
		getDispItem: function() {
			var dispgroup = this.$('#ca_MDCMV0200_dispgroup').val();
			var dispitemList = [];
...
//				});
		...
		setButtons: function() {
			var $buttons = this.$('button.ca_MDCMV0200_add');
			var $trlist = this.$('#ca_MDCMV0200_tbl').find('tr.selected');
		...
		_onSortClick: function(e) {
			...
			// すべてのソート解除
			this.$('#ca_MDCMV0200_edit_tbody').find('span.asc').removeClass('active');
			this.$('#ca_MDCMV0200_edit_tbody').find('span.dsc').removeClass('active');
		...
		_onEditDelAllClick: function() {
			this.$('#ca_MDCMV0200_edit_tbody').empty();
		},
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_MDCMV0200_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onAddClick:function(e) {
			...
			for (var i = 0; i < dispitemlist.length; i++) {
				...
				var editdispitemlist = this.$('#ca_MDCMV0200_edit_tbl').find('tr[id=' + dispitem_id + ']');
				// 既に追加されている場合は追加しない
				if (editdispitemlist.length == 0) {
					...
					this.$('#ca_MDCMV0200_edit_tbody_tmp').tmpl(newdispitem).appendTo('#ca_MDCMV0200_edit_tbody');
				}
			}
			this.initAnimate(this.$el);
		},
		...
		_onUpClick: function(e) {
			var trlist = this.$('#ca_MDCMV0200_edit_tbl').find('tr.selected');
			...
			var $tr_up = null;
			var trlist_edit = this.$('#ca_MDCMV0200_edit_tbl').find('tr');
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_MDCMV0200_edit_tbody').children());
			return editlist;
		},
		...
		getSortkey : function() {
			// ソートキー設定
			var sortkey = null;
			var $span = this.$('#ca_MDCMV0200_edit_tbody').find('span.active');
			if ($span.length > 0) {
