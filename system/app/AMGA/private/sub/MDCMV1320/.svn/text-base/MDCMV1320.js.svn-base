$(function() {
	 ...
	 * １条件あたりのセレクタビュークラス
	 */
	MDCMV1320RangeViewClass = Backbone.View.extend({
...
console.log("getAnaFocus: $sel="+$sel);
	 ...
	 * １条件あたりのセレクタビュークラス
	 */
	MDCMV1320SelectorViewClass = Backbone.View.extend({
		///className: 'fieldgroup',
		contTemplate: _.template(''
			+ '<div class="fieldgroup">'
			+ '<p class="gtitle"><%- typeTypeName %></p>'
		...
		render: function(){
			...
			if(this.kindlist == null){
                switch(this.options.anaFocusKind){
				 case amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE:
					// プライスライン(POS元上代)用
					this.kindlist = clcom.getPricelineList();
					break;
				 case amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE_SL:
					// プライスライン(POS元上代)用[営業部]
					this.kindlist = clcom.getPricelineSlList();
					break;
				 case amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE_SL_M:
					// プライスライン用[営業部]
...
console.log("getAnaFocus: $sel="+$sel);
	...
	//////////////////////////////////////////////
	// View
	MDCMV1320SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_MDCMV1320_commit"			:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_MDCMV1320_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV1320_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_MDCMV1320_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		typeRadioViews: [
			{
				options: {
					typeTypeName: '当初指定上代',
					anaFocusKind: amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_INIT,
					value1: null,
					value2: null
				},
				view: null
			},
			{
				options: {
					typeTypeName: '現指定上代',
					anaFocusKind: amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_CURR,
					value1: null,
					value2: null
				},
				view: null
			},
			{
				options: {
					typeTypeName: 'POS元上代',
					anaFocusKind: amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_ORGPOS,
					value1: null,
					value2: null
				},
				view: null
			},
			{
				options: {
					typeTypeName: '実上代',
					anaFocusKind: amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_POS,
					value1: null,
					value2: null
				},
				view: null
...
//					anaFocusKind: amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE_M,
//					value1: null,
//					value2: null
//				},
//				view: null
//			},
			{
				options: {
					...
					anaFocusKind: amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE_SL_M,
					value1: null,
					value2: null
				},
				view: null
			},
			{
				options: {
					...
					anaFocusKind: amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE,
					value1: null,
					value2: null
				},
				view: null
...
//					typeTypeName: 'プライスライン(POS元上代)',
//					anaFocusKind: amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE_SL,
//					value1: null,
//					value2: null
//				},
//				view: null
//			}
		...
		initUIelement: function(editList) {
			...
			_.each(this.typeRadioViews, function(item){
				if( item.options.anaFocusKind == amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE ||
				    item.options.anaFocusKind == amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE_M ||
				    item.options.anaFocusKind == amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE_SL ||
				    item.options.anaFocusKind == amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE_SL_M ){
					item.view = new MDCMV1320SelectorViewClass(item.options).render();
					...
					item.view = new MDCMV1320RangeViewClass(item.options).render();
					this.append(item.view.$el);
...
//			this.$el.html('');
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV1320_main'), {
		...
		// Idより選択されたデータを取得
		getData : function(){
			//var editList = clutil.view2data(this.$('#ca_MDCMV1320_searchArea'), 'ca_MDCMV1320_');
			var editList = _.reduce(this.typeRadioViews, function(list, typeRadioView){
				...
				if(anaFocus){
					if(anaFocus.kind == amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE ||
					   anaFocus.kind == amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE_M ||
					   anaFocus.kind == amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE_SL ||
					   anaFocus.kind == amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMPRICE_LINE_SL_M) {
						if(anaFocus.val != null && anaFocus.val != 0){
							list.push(anaFocus);
						}
					}
