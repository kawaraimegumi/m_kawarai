$(function () {
  //////////////////////////////////////////////
  // View
  MDCMV0110SelectorView = Backbone.View.extend({
    screenId: 'MDCMV0110',
    validator: null,

    // 押下イベント
    events: {
      'click .ca_MDCMV0110_node': '_onNodeClick', // ツリーノード押下

      //			"click #ca_MDCMV0110_search"		:	"_onSearchClick",		// 検索ボタン押下
      //			"click #ca_MDCMV0110_showSearchArea":	"_onShowSearchAreaClick",	// 検索条件を再指定ボタン押下
      'click #ca_MDCMV0110_add': '_onAddClick', // 追加ボタン押下

      'keyup .ca_MDCMV0110_srch_input': '_onSrchInputKeyPress', // テーブル上でリスト名編集確定

      'click .ca_MDCMV0110_edit_del': '_onEditDelClick', // 単一削除ボタン押下
      'click #ca_MDCMV0110_edit_delall': '_onEditDelAllClick', // すべて削除ボタン押下

      'click #ca_MDCMV0110_clear': '_onClearClick', // クリアボタン押下
      'click #ca_MDCMV0110_commit': '_onCommitClick', // 確定ボタン押下

      'click #ca_MDCMV0110_main .close': '_onCancelClick', // 閉じるボタン押下時
      'click #ca_MDCMV0110_cancel': '_onCancelClick', // キャンセルボタン押下時
      'click #ca_MDCMV0110_main .modalBK': '_onCancelClick', // 枠外押下時
    },
    ...
    initUIelement: function () {
      ...
      if (this.select_mode == clutil.cl_single_select) {
        this.$('.ca_MDCMV0110_multi').remove();
      }
      // 分析画面以外からのモード
      ...
      this.srchArea = clutil.controlSrchArea(
        this.$('#ca_MDCMV0110_searchArea'),
        this.$('#ca_MDCMV0110_search'),
        this.$('#result'),
        this.$('#searchAgain')
      );
    ...
    show: function (editList, isSubDialog, lvl, initData) {
      ...
      $('.cl_echoback').hide();
      // validatorエラー時の表示領域
      this.validator = clutil.validator(this.$('#ca_MDCMV0110_main'), {
      ...
      if (
        ...
        this.$('#ca_MDCMV0110_edit_tbl').html(html_source);
    ...
    _onNodeClick: function (e) {
      ...
      if (
        ...
        if (
          $(table).find($('tr.selected')).length == 1 &&
          $(table).hasClass('final')
        ) {
          return;
        }
      }
      ...
      for (var i = this.level; i > level; i--) {
        // 内部レベルより大きいボックスは消去する
        this.$('#ca_MDCMV0110_box_' + i)
          .closest('div.drilldownItem')
          .remove();
      ...
      if ($(table).find($('tr.selected')).length > 0) {
        ...
        this.$('#ca_MDCMV0110_box_' + select_level).addClass('final');
      }
            ...
            if (addr_id != null) {
              ...
              if (resultList == null || resultList.length == 0) {
                ...
                for (var i = 0; i < _this.idList.length; i++) {
                  var tr = $(
                    '#ca_MDCMV0110_addrtree tr[id=' + _this.idList[i] + ']'
                  );
              ...
              if (resultList == null || resultList.length == 0) {
                ...
                // 取得したデータを表示する
                _this
                  .$('#ca_MDCMV0110_box_tmp')
                  .tmpl(lvl_obj)
                  .appendTo('#ca_MDCMV0110_addrtree');
                _this
                  .$('#ca_MDCMV0110_node_tmp')
                  .tmpl(resultList)
                  .appendTo('#ca_MDCMV0110_box_' + _this.level);
              }
            }
            // スクロール
            $('#ca_MDCMV0110_addrtree').scrollTo(
              '#ca_MDCMV0110_box_' + _this.level
            );

            clutil.initUIelement(_this.$el);
            //					$('#ca_MDCMV0110_box_' + _this.level).closest('.yScroll').slimScroll({
            //						color: '#ccc',
            //					});
          } else {
            // ヘッダーにメッセージを表示
    ...
    makeTree: function (list) {
      ...
      this.$('#ca_MDCMV0110_box_tmp')
        .tmpl(lvl_obj)
        .appendTo('#ca_MDCMV0110_addrtree');
      this.$('#ca_MDCMV0110_node_tmp')
        .tmpl(list)
        .appendTo('#ca_MDCMV0110_box_' + this.level);
    ...
    clearResult: function () {
      // 結果表示エリアをクリア
      this.$('#ca_MDCMV0110_addrtree').empty();
      // validatorの初期化
      this.validator.clear();
      // 内部用レベルを初期化
      this.level = 0;
    ...
    _onClearClick: function () {
      clutil.viewClear(this.$('#ca_MDCMV0110_searchArea'));
      // validatorの初期化
      this.validator.clear();
    },
    ...
    _onAddClick: function (e) {
      ...
      var lvl_flag = false;
      $.each(
        this.$('#ca_MDCMV0110_addrtree table.final tr.selected'),
        function () {
          if (
            _this.lvl_id != -1 &&
      ...
      if (chkId.length > 0) {
        ...
        $.each(this.$('#ca_MDCMV0110_edit_tbl li'), function () {
        ...
        for (var i = 0; i < selectlist.length; i++) {
          ...
          // 種別を挿入する
          newdata.kind = amgbp_AnaDefs.AMGBA_DEFS_KIND_ADDR;
          newdata.attr = select.lvl_id;

          html_source += this.makeEditTmp(newdata);
          num++;
          editlist.push(newdata);
        }
        this.$('#ca_MDCMV0110_edit_tbl').append(html_source);
      } else {
        // なにもしない
      }
      // アニメーション表示のための初期化
    ...
    makeEditTmp: function (data) {
      ...
      html_source +=
        data.code +
        '</span><span class="btn-delete ca_MDCMV0110_edit_del"></span>';
      html_source +=
    ...
    _onEditDelAllClick: function () {
      ...
      var idlist = [];
      var html_source = '';
      this.$('#ca_MDCMV0110_edit_tbl').html('');
    ...
    getEditList: function () {
      ...
      editlist = clutil.tableview2data(
        this.$('#ca_MDCMV0110_edit_tbl').children()
      );
      return editlist;
    },
    ...
    _onCommitClick: function () {
      ...
      if (
        ...
        if (chkId.length > 0) {
          ...
          // ヘッダーにメッセージを表示
          this.validator.setErrorInfo({ _eb_: clmsg.ca_MDCMV0110_0001 });
        }
      } else {
        // 編集リストを取得
        this.$parentView.show();
