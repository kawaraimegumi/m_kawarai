$(function() {
	...
	CACMV0310SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"change #ca_CACMV0310_kind"		:	"_onKindSelect",		// 種別変更
//			"keyup #ca_CACMV0310_kind"		:	"_onKindSelect",		// 種別変更
//			"keydown #ca_CACMV0310_kind"		:	"_onKindSelect",		// 種別変更

			"keyup .ca_CACMV0310_srch_input"	:	"_onSrchInputKeyPress",	// テーブル上でリスト名編集確定

			"click .ca_CACMV0310_node"			:	"_onNodeClick",			// ツリーノード押下

//			"click #ca_CACMV0310_search"		:	"_onSearchClick",		// 検索ボタン押下
//			"click #ca_CACMV0310_showSearchArea":	"_onShowSearchAreaClick",	// 検索条件を再指定ボタン押下
		...
		initUIelement: function() {
			...
			// コンボボックスの中身を作成
			clutil.cltypeselector2(this.$('#ca_CACMV0310_kind'), this.kindlist, 0,
					1, 'attr');
			if (this.kindlist != null && this.kindlist.length > 0) {
				this.$('#ca_CACMV0310_kind').selectpicker('val', this.kindlist[0]);
			}
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_CACMV0310_kind'));
		},
		...
		_onNodeClick: function(e) {
			...
			for (var i = this.level; i > level; i--) {
				// 内部レベルより大きいボックスは消去する
				this.$('#ca_CACMV0310_box_' + i).closest('div.drilldownItem').remove();
			...
			if ($(table).find($('tr.selected')).length > 0) {
				...
				this.$('#ca_CACMV0310_box_' + select_level).addClass('final');
			}
		...
		onSearchClick: function(parent_id, lvl_no) {
			...
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_CACMV0310_searchArea'), 'ca_CACMV0310_');
			...
			if(Ana.Config.cond.CACMV0310 && !_.isEmpty(Ana.Config.cond.CACMV0310.itemspec_srch_resId)){
				uri = Ana.Config.cond.CACMV0310.itemspec_srch_resId;
			}
				...
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (resultList == null || resultList.length == 0) {
						...
						var lvl_obj = {
								level : _this.level};
						// 取得したデータを表示する
						_this.$('#ca_CACMV0310_box_tmp').tmpl(lvl_obj).appendTo('#ca_CACMV0310_itemspectree');
						_this.$('#ca_CACMV0310_node_tmp').tmpl(resultList).appendTo('#ca_CACMV0310_box_' + _this.level);
					}
					// スクロール
			        $('#ca_CACMV0310_itemspectree').scrollTo('#ca_CACMV0310_box_' + _this.level);
					clutil.initUIelement(_this.$el);
//					$('#ca_CACMV0310_box_' + _this.level).closest('.yScroll').slimScroll({
//						color: '#ccc',
//					});
				} else {
					// ヘッダーにメッセージを表示
		...
		clearResult: function() {
			// 結果表示エリアをクリア
			this.$('#ca_CACMV0310_itemspectree').empty();
			// validatorの初期化
			this.validator.clear();
			// 内部用レベルを初期化
			this.level = 0;
		...
		_onAddClick:function(e) {
			...
			var chkId = [];
			var _this = this;
			$.each(this.$("#ca_CACMV0310_itemspectree table.final tr.selected"), function() {
				chkId.push(this.id);
			});
			...
			if (chkId.length > 0) {
				...
				var idlist = [];
				var html_source = '';
				//var kind = this.$('#ca_CACMV0310_kind').val();
				var kind = amanp_AnaDefs.AMAN_DEFS_KIND_ITEMSPEC;

				$.each(this.$('#ca_CACMV0310_edit_tbl li'), function() {
					//var edit_kind = $(this).find('input[name=kind]').val()
					var edit_kind = amanp_AnaDefs.AMAN_DEFS_KIND_ITEMSPEC;
					if (kind == edit_kind) {
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					newdata.name2 = this.$('#ca_CACMV0310_kind').find('option[value=' +
							this.$('#ca_CACMV0310_kind').val() + ']').html();
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					for (var i = 0; i < selectlist.length; i++) {
						...
						// 種別を挿入する
						newdata.kind = this.$('#ca_CACMV0310_kind').val();
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_CACMV0310_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
