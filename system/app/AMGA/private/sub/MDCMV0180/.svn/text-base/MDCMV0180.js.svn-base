...
$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCMV0180SelectorView = Backbone.View.extend({
		screenId : "MDCMV0180",
		validator: null,
		...
		// 押下イベント
		events: {
			"change #ca_MDCMV0180_main .ca_MDCMV0180_axis_kind"	:	"_onAxisKindSelect",		// 軸種別項目変更
			"change #ca_MDCMV0180_main .ca_MDCMV0180_axis_attr"	:	"_onAxisAttrSelect",		// 軸属性項目変更
//			"keyup #ca_CACMV0020_func_id"		:	"_onFuncSelect",		// 種別変更
//			"keydown #ca_CACMV0020_func_id"		:	"_onFuncSelect",		// 種別変更

			"click #ca_MDCMV0180_main .button_MDCMV0180_show_MDCMV0190"	:	"_onAxisDispClick",	// 表示項目選択サブ画面

			"click #ca_MDCMV0180_main .ca_MDCMV0180_clear"		:	"_onClearClick",		// クリアボタン押下
			"click #ca_MDCMV0180_main .ca_MDCMV0180_default"	:	"_onDefaultClick",		// 初期値に戻すボタン押下

			"click #ca_MDCMV0180_commit"						:	"_onCommitClick",	// 確定ボタン押下

			"change #ca_MDCMV0180_main .ca_MDCMV0180_val"		:	"_onFromAmFocusout",	// 開始値フォーカスアウト

			"click #ca_MDCMV0180_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV0180_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_MDCMV0180_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function(axisFolder) {
			...
			if (this.anadata.axis_num.v_axis_num == 1) {
				this.$('.ca_MDCMV0180_v2').remove();
				this.$('.ca_MDCMV0180_v3').remove();
			} else if (this.anadata.axis_num.v_axis_num == 2) {
				this.$('.ca_MDCMV0180_v3').remove();
			} else if (this.anadata.axis_num.v_axis_num == 0) {
				this.$('.ca_MDCMV0180_v').remove();
				this.$('.ca_MDCMV0180_v1').remove();
				this.$('.ca_MDCMV0180_v2').remove();
				this.$('.ca_MDCMV0180_v3').remove();
			}
			...
			if (this.anadata.axis_num.h_axis_num == 1) {
				this.$('.ca_MDCMV0180_h2').remove();
			} else if (this.anadata.axis_num.h_axis_num == 0) {
				this.$('.ca_MDCMV0180_h').remove();
				this.$('.ca_MDCMV0180_h1').remove();
				this.$('.ca_MDCMV0180_h2').remove();
			}
			// 併売分析（併売点数分析）の場合は軸1を「購入点数」に固定。
			// 軸2は「未指定」または「品種」にする
			if (this.options.isMDRcpt) {
				...
				// 縦軸１ 未選択値なし
				clutil.cltypeselector2(this.$('select.ca_MDCMV0180_axis_kind[axis-type=v1]'),
						axis1, 0,
						1,
						'index');
				// 縦軸２
				clutil.cltypeselector2(this.$('select.ca_MDCMV0180_axis_kind[axis-type=v2]'),
						axis2, 1,
						1,
						'index');
			} else {
			...
			// 縦軸３
			clutil.cltypeselector2(this.$('select.ca_MDCMV0180_axis_kind[axis-type=v3]'),
					this.axisList, 1,
					1,
					'index');
			// 横軸１
			clutil.cltypeselector2(this.$('select.ca_MDCMV0180_axis_kind[axis-type=h1]'),
					this.axisList, 1,
					1,
					'index');
			// 横軸２
			clutil.cltypeselector2(this.$('select.ca_MDCMV0180_axis_kind[axis-type=h2]'),
					this.axisList, 1,
					1,
					'index');

			// MD商品分析版のABC軸基準設定
//			if(!Ana.Config.cond.MDCMV0180.isMDABCAxis){
//				this.$('.ca_MDCMV0180_mdabc').remove();
//			}
			...
			if (axisFolder != null) {
				// 編集データを反映
				clutil.data2view(this.$('.ca_MDCMV0180_searchArea'), axisFolder, 'ca_MDCMV0180_');
				...
				if (vAxisList != null && vAxisList.length > 0) {
					for (var i = 1; i <= vAxisList.length; i++) {
						...
						this.$('select.ca_MDCMV0180_axis_kind[axis-type=v' + i + ']')
				...
				if (hAxisList != null && hAxisList.length > 0) {
					for (var i = 1; i <= hAxisList.length; i++) {
						...
						this.$('select.ca_MDCMV0180_axis_kind[axis-type=h' + i + ']')
							.selectpicker('val', this.getAxisListIndex(axis.kind, axis.func_id));
				...
				// MD商品分析 ABC 基準の設定
//				if(Ana.Config.cond.MDCMV0180.isMDABCAxis){
//					this.$(':radio[name="ca_MDCMV0180_mdabc_type"][value=' + axisFolder.mdabc_type + ']').radio('check');
//				}
			}

			// 軸選択注意喚起ラベル
//			if (Ana.Config.cond.MDCMV0180.attentionText) {
//				this.$('#title .attentionText').text(Ana.Config.cond.MDCMV0180.attentionText);
//			}

			// 表示属性選択補助画面
			this.MDCMV0180_MDCMV0190Selector = new  MDCMV0190SelectorView({
				el : this.$('#ca_MDCMV0180_MDCMV0190_dialog'),	// 配置場所
				$parentView		: this.$('#ca_MDCMV0180_main'),
...
//				ymd				: null,			// 検索日
			...
			this.MDCMV0180_MDCMV0190Selector.render();
		...
		setAttr : function(axis, axis_type) {
			var select = this.$('select.ca_MDCMV0180_axis_attr[axis-type=' + axis_type + ']');
			var select_m = this.$('select.ca_MDCMV0180_axis_attr[axis-type=' + axis_type + '_m]');
		...
		setMstItem: function(axis, axis_type, mstitemList) {
			...
			var $input = this.$("input.input_MDCNV0180_mastitem[axis-type='" + axis_type + "']");
...
//			url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		show: function(axisFolder, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV0180_main'), {
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('select.ca_MDCMV0180_axis_kind[axis-type=v1]'));
		},
			...
			case amanp_AnaDefs.AMAN_DEFS_KIND_COLORITEM:	// TODO 未定義？
				...
				if ($div_s) {
					clutil.initcltypeselector2(
							$div_s, null, 1, null,
							null, null, null,
							{name : "info", 'axis-type' : axis_type}, "mbn wt280 flleft ca_MDCMV0180_axis_attr");
				}
				if ($div_m) {
					clutil.initcltypeselector2(
							$div_m, null, 1, null,
...
//				clutil.cltypeselector2($select, null, 1);
				...
				clutil.initcltypeselector2(
						$div, attrlist, 0, 1,
						'cond_attr', null, null,
						option, "mbn wt280 flleft ca_MDCMV0180_axis_attr");
...
//						'cond_attr');
		...
		getAttrSelectDiv : function(kind, axis_type) {
			var div = this.$('div.ca_MDCMV0180_axis_attr[axis-type=' + axis_type + ']');
			var div_m = this.$('div.ca_MDCMV0180_axis_attr[axis-type=' + axis_type + '_m]');
			var div_i = this.$('div.ca_MDCMV0180_axis_attr[axis-type=' + axis_type + '_i]');
		...
		_onAxisAttrSelect: function(e) {
			...
			var index = Number(this.$('select.ca_MDCMV0180_axis_kind[axis-type=' + axis_type + ']').val()) - 1;
			var kind = this.axisList[index].cond_kind;
			var attr = this.$('select.ca_MDCMV0180_axis_attr[axis-type=' + axis_type + ']').val();
		...
		hideFromToArea: function(axis_type) {
			this.$('#ca_MDCMV0180_' + axis_type + '_fromto').empty();
			this.$('.ca_MDCMV0180_' + axis_type + '_fromto_div').addClass('dispn');
		},
		...
		showFromToArea: function(fromtolist, axis_type) {
			...
			this.$('#ca_MDCMV0180_' + axis_type + '_fromto').empty();
			this.$('.ca_MDCMV0180_' + axis_type + '_fromto_div').removeClass('dispn');
			...
			if (isAscend) {
				...
				for (var i = 0; i < max_rank; i++) {
					if (i >= fromtolist.length) {
						...
						this.$('#ca_MDCMV0180_fromto_tmp').tmpl(newdata).appendTo('#ca_MDCMV0180_' + axis_type + '_fromto');
					} else {
						var fromto = fromtolist[i];
						fromto.no = numbering[i];
						fromto.axis_type = axis_type;
						fromto.max_len = max_len;
						this.$('#ca_MDCMV0180_fromto_tmp').tmpl(fromto).appendTo('#ca_MDCMV0180_' + axis_type + '_fromto');
					}
				}
			} else {
				// 降順
		...
		setFirstRow: function(axis_type, attr) {
			if (this.chkAscend(attr)) {
				// 昇順
				var input = this.$('#ca_MDCMV0180_' + axis_type + '_fromto input:first');
				$(input).attr('disabled', 'disabled');
			} else {
				// 降順
				var input = this.$('#ca_MDCMV0180_' + axis_type + '_fromto input:last');
		...
		hideLastStoreArea: function(axis_type) {
			this.$('#ca_MDCMV0180_' + axis_type + '_laststore').empty();
			this.$('.ca_MDCMV0180_' + axis_type + '_laststore_div').addClass('dispn');
		},
		...
		showLastStoreArea: function(fromtolist, axis_type) {
			...
			this.$('#ca_MDCMV0180_' + axis_type + '_laststore').empty();
			this.$('.ca_MDCMV0180_' + axis_type + '_laststore_div').removeClass('dispn');

			this.$('#ca_MDCMV0180_laststore_tmp').tmpl({
				axis_type : axis_type
			}).appendTo('#ca_MDCMV0180_' + axis_type + '_laststore');
			clutil.datepicker(this.$('#ca_MDCMV0180_' + axis_type + '_laststore').find('input'));

			// 編集データがある場合は編集データを表示する
			if (fromtolist != null && fromtolist.length == 1) {
				this.$('#ca_MDCMV0180_' + axis_type + '_laststore').find('input[name=val]').val(
						clutil.dateFormat(fromtolist[0].val, 'yyyy/mm/dd')
				);
				this.$('#ca_MDCMV0180_' + axis_type + '_laststore').find('input[name=val2]').val(
		...
		_onAxisDispClick: function(e) {
			...
			// どのボタンが押されたか取得
			var axis_type = $(e.target).attr('axis-type');
			var $select = this.$("select.ca_MDCMV0180_axis_kind[axis-type='" + axis_type + "']");
			...
			// TODO サブ画面を表示
			this.MDCMV0180_MDCMV0190Selector.show(this.mstitem_list[axis_type], true, kind, func_id, name);	// TODO 選択済みリスト

			// TODO 戻ってきたときのコールバック設定
			this.MDCMV0180_MDCMV0190Selector.okProc = function(data) {
				...
				if (data != null) {
					...
					var $input = _this.$("input.input_MDCNV0180_mastitem[axis-type='" + axis_type + "']");
					$input.val(txt);
...
//				$(e.target).focus();
		...
		_onFromAmFocusout: function(e) {
			var axis_type = $(e.target).closest('div.ca_MDCMV0180_fromto').attr('axis-type');
			var attr = this.$('select.ca_MDCMV0180_axis_attr[axis-type=' + axis_type + ']').val();
			var fromtolist = this.getFromToList(axis_type);

			// 領域のエラークリア
			var rank_validator = clutil.validator(this.$('#ca_MDCMV0180_' + axis_type + '_fromto'), {
				echoback		: $('.cl_echoback')
			});
		...
		checkNumber: function(fromtolist, axis_type) {
			var inputlist = this.$('#ca_MDCMV0180_' + axis_type + '_fromto input[name=val]');
			var attr = this.$('select.ca_MDCMV0180_axis_attr[axis-type=' + axis_type + ']').val();
			...
			if (fromtolist == null || fromtolist.length <= 1) {
				...
				if (this.chkAscend(attr)) {
					...
					for (var i = 0; i <= last-1; i++) {
						...
						if (clutil.cStr(rank1.val) != "") {
							...
							if (this.chkSumrt(attr) && Number(rank1.val) >= 100) {
								this.validator.setErrorMsg($(inputlist[i]), clmsg.ca_MDCMV0180_0004);
								isAscending = false;
							}
							...
							for (var j = i+1; j <= last; j++) {
								...
								if (clutil.cStr(rank2.val) != "") {
									if (Number(rank1.val) >= Number(rank2.val)) {
										// エラーの設定
										this.validator.setErrorMsg($(inputlist[i]), clmsg.ca_MDCMV0180_0001);
										this.validator.setErrorMsg($(inputlist[j]), clmsg.ca_MDCMV0180_0001);
										isAscending = false;
									}
									break;
								}
					...
					if (this.chkSumrt(attr) && Number(fromtolist[last].val) >= 100) {
						this.validator.setErrorMsg($(inputlist[last]), clmsg.ca_MDCMV0180_0004);
						isAscending = false;
					}
					...
					if (!isAscending) {
						...
						// フォーカスの設定
						var err = $('#ca_MDCMV0180_' + axis_type + '_fromto input.cl_error_field:first[name=val]');
						$(err).focus();
						return false;
					}
				} else {
						...
						if (clutil.cStr(rank1.val) != "") {
							...
							for (var j = i-1; j >= last; j--) {
								...
								if (clutil.cStr(rank2.val) != "") {
									if (Number(rank1.val) >= Number(rank2.val)) {
										// エラーの設定
										this.validator.setErrorMsg($(inputlist[i]), clmsg.ca_MDCMV0180_0006);
										this.validator.setErrorMsg($(inputlist[j]), clmsg.ca_MDCMV0180_0006);
										isDescending = false;
									}
									break;
								}
		...
		calcToAm: function(fromtolist, axis_type) {
			...
			// 終了値の再設定
			var inputlist = $('#ca_MDCMV0180_' + axis_type + '_fromto input[name=val2]');
		...
		checkLastStore : function(laststorelist, axis_type) {
			var inputfrom = this.$('#ca_MDCMV0180_' + axis_type + '_laststore input[name=val]');
			var inputto = this.$('#ca_MDCMV0180_' + axis_type + '_laststore input[name=val2]');
			var chkInfo = [];
		...
		getFromToList : function(axis_type, splice, attr){
			var divlist = this.$('div.ca_MDCMV0180_' + axis_type + '_fromto_elem');
		...
		getLastStoreList : function(axis_type){
			var divlist = this.$('div.ca_MDCMV0180_' + axis_type + '_laststore_elem');
		...
		chkData : function() {
			...
			for (var i = 1; i <= this.anadata.axis_num.v_axis_num; i++) {
				var axis = {};
				axis.axis_type = 'v' + i;
				var index = Number(this.$('select.ca_MDCMV0180_axis_kind[axis-type=' + axis.axis_type + ']').val()) - 1;
				if (index >= 0) {
					axis.kind = this.axisList[index].cond_kind;
				} else {
					axis.kind = 0;
				}
				axis.$kind_select = $(this.$('select.ca_MDCMV0180_axis_kind[axis-type=' + axis.axis_type + ']'));
			...
			if (!this.options.isMDBascket && AxisList.length > 1) {
						...
						case amanp_AnaDefs.AMAN_DEFS_KIND_DMRFM:
							...
							if (pAxis.axis_type[0] == nAxis.axis_type[0]) {
								...
								if ((Number(pAxis.kind) != 0) && (pAxis.kind == nAxis.kind)) {
									this.validator.setErrorMsg(pAxis.$kind_select, clmsg.ca_MDCMV0180_0002);
									this.validator.setErrorMsg(nAxis.$kind_select, clmsg.ca_MDCMV0180_0002);
									retStat = true;
								}
							} else {
								// 同一属性の場合はエラー
								if ((pAxis.kind == nAxis.kind) &&
									...
									this.validator.setErrorMsg(pAxis.$attr_select, clmsg.ca_MDCMV0180_0003);
									this.validator.setErrorMsg(nAxis.$attr_select, clmsg.ca_MDCMV0180_0003);
									retStat = true;
								}
							}
							break;
				...
				if( retStat != true ){
					...
					for (var i = 1; i <= this.anadata.axis_num.v_axis_num; i++) {
						var vaxis_type = 'v' + i;
						var vkind;
						var vindex = Number(this.$('select.ca_MDCMV0180_axis_kind[axis-type=' + vaxis_type + ']').val()) - 1;
						if (vindex >= 0) {
						...
						if (vkind == amanp_AnaDefs.AMAN_DEFS_KIND_ITGRP ||
							...
							vcc++;
							if( vcc > 1 ){
								this.validator.setErrorMsg(this.$('select.ca_MDCMV0180_axis_kind[axis-type=' + vaxis_type + ']'), clmsg.ca_MDCMV0180_0007);
								retStat = true;
							}
						}
					}
			    ...
			    case amanp_AnaDefs.AMAN_DEFS_KIND_PACKITEM:
					if ((Number(pAxis.kind) != 0) && (pAxis.attr == null)) {
						...
						if (pAxis.$attr_select != null &&
							...
							this.validator.setErrorMsg(pAxis.$attr_select, clmsg.ca_MDCMV0180_0005);
							retStat = true;
						}
					}
					break;
...
//				$(err).focus();
		...
		getAxisData : function() {
			var axisFolder = clutil.view2data(this.$('.ca_MDCMV0180_searchArea'), 'ca_MDCMV0180_');
			var vAxisList = [];
			var hAxisList = [];
			for (var i = 1; i <= this.anadata.axis_num.v_axis_num; i++) {
				var axis = {};
				var index = Number(this.$('select.ca_MDCMV0180_axis_kind[axis-type=v' + i + ']').val()) - 1;
			...
			for (var i = 1; i <= this.anadata.axis_num.h_axis_num; i++) {
				var axis = {};
				var index = Number(this.$('select.ca_MDCMV0180_axis_kind[axis-type=h' + i + ']').val()) - 1;
			...
			// MD商品分析版のABC軸基準設定
//			if(Ana.Config.cond.MDCMV0180.isMDABCAxis){
//				axisFolder.mdabc_type = this.$(':radio[name="ca_MDCMV0180_mdabc_type"]:checked').val();
//			}
