$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCMV1050SelectorView = Backbone.View.extend({
		itemsOnPage :30,
		...
		// 押下イベント
		events: {
			"change #ca_MDCMV1050_func_id"		:	"_onFuncSelect",		// 種別変更
//			"keyup #ca_MDCMV1050_func_id"		:	"_onFuncSelect",		// 種別変更
//			"keydown #ca_MDCMV1050_func_id"		:	"_onFuncSelect",		// 種別変更

			"keyup .ca_MDCMV1050_srch_input"	:	"_onSrchInputKeyPress",	// 検索条件キー入力
			"keydown .ca_MDCMV1050_item_srch_input"
												:	"_onItemSrchInputKeyPress",	// 商品用検索条件Enter,Tab入力

			"click .ca_MDCMV1050_node"			:	"_onNodeClick",			// ツリーノード押下
			"click .showmore"					:	"_onShowMoreClick",			// もっと見るボタン押下

//			"click #ca_MDCMV1050_search"		:	"_onSearchClick",		// 検索ボタン押下
//			"click #ca_MDCMV1050_showSearchArea":	"_onShowSearchAreaClick",	// 検索条件を再指定ボタン押下
			"click #ca_MDCMV1050_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_MDCMV1050_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_MDCMV1050_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_MDCMV1050_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_MDCMV1050_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_MDCMV1050_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV1050_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_MDCMV1050_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		 ...
		 * ・ymd				: 検索日
		 *						  指定がない場合は運用日
		 * ・max_level			: 最大階層Lv制限 amcm_type.AMCM_VAL_ITGRP_LEVEL_***
		 */
		...
		initUIelement: function() {
			...
			if (this.select_mode == clutil.cl_single_select) {
				this.$('.ca_MDCMV1050_multi').remove();
			}
			// 分析画面以外からのモード
			...
			// 種別が設定されている場合は種別変更不可
			if (this.func_id != -1) {
				this.$('#ca_MDCMV1050_func_id').attr('disabled', 'disabled');
			}
			clutil.cltypeselector2(this.$('#ca_MDCMV1050_func_id'), this.funclist, 0,
					null,
					'func_id', 'func_name', 'func_code');
			if (this.func_id != -1) {
				this.$('#ca_MDCMV1050_func_id').selectpicker('val', this.func_id);
			}

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_MDCMV1050_searchArea'),
					this.$('#ca_MDCMV1050_search'),
					this.$('#result'),
					this.$('#searchAgain'));
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		show: function(editList, isSubDialog, func_id, lvl, initData) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV1050_main'), {
			...
			// データを取得
			var uri = 'gsan_se_itgrptree_init';
			clutil.postAnaJSON(uri, req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (this.select_mode == clutil.cl_multiple_select &&
						...
						this.$('#ca_MDCMV1050_edit_tbl').html(html_source);
...
//						this.initData = null;
					...
					if (this.initData != null) {
						this.$('#ca_MDCMV1050_func_id').selectpicker('val', this.initData.func_id);
//						// 親組織=0で第1階層を取得
...
//					this.validator.setErrorInfo({_eb_: clutil.fmtargs(clutil.getclmsg(data.head.message), data.head.args)});
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_MDCMV1050_func_id'));
		},
		...
		_onNodeClick: function(e) {
			...
			// 商品の結果リストを削除
			if (!is_leaf) {
				this.removeItgrpList();
			}
			// 選択された内部レベルを取得
			var level = Number($(table).attr("box-level"));
			for (var i = this.level; i > level; i--) {
				// 内部レベルより大きいボックスは消去する
				this.$('#ca_MDCMV1050_box_' + i).closest('div.drilldownItem').remove();
			...
			if ($(table).find($('tr.selected')).length > 0) {
				...
				this.$('#ca_MDCMV1050_box_' + select_level).addClass('final');
			}
			...
			if (!is_leaf && $(table).find($('tr.selected')).length == 1) {
				...
				// 品種を選択した場合はもっと見るを表示する検索をする
				// 商品分類階層区分
				if(lvl_no >= amcm_type.AMCM_VAL_ITGRP_LEVEL_CTG){
					// 結果表示
					this.onItemSearchClick(parent_id);
				} else {
					// 結果表示
		...
		_onSrchInputKeyPress: function(e, ev) {
			...
			if (clutil.chkStr(srch_word)) {
				for (var i = 0; i < trlist.length; i++) {
					...
					if (word.indexOf(srch_word) == -1 && !$(tr).hasClass('ca_showmore_tr')) {
						$(tr).hide();
					}
				}
			}
		...
		onSearchClick: function(parent_id, itgrp_id) {
			...
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_MDCMV1050_searchArea'), 'ca_MDCMV1050_');
			...
			// 初期表示のみページャーを渡す
			if (searchData.f_upper == 1) {
				var pagedata = {
						start_record : 0,			// index番目のデータから(0～)
						page_size : this.itemsOnPage	// 1ページに表示する件数を要求
				};
				req.pagereq = pagedata;
			}

			// データを取得
			var uri = 'gsan_se_itgrptree_srch';
			clutil.postAnaJSON(uri, req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (itgrp_id != null) {
						...
						if (resultList == null || resultList.length == 0) {
							...
							// 取得したデータを表示する
							_this.idList = [];
							_this.makeTree(resultList, searchData.func_id, data.pagersp);
							for (var i = 0; i < _this.idList.length; i++) {
								var tr = $('#ca_MDCMV1050_itgrptree tr[id=' + _this.idList[i] + ']')
						...
						if (resultList == null || resultList.length == 0) {
							...
							var no = 0;	//表示番号セット
							// 取得したデータを表示する
							_.each(resultList, function(aNode){
								aNode.no = no++;
								if(!aNode.is_leaf){
									aNode.is_leaf = (aNode.lvl_no >= this.max_level) ? 1 : 0;
								}
							},this);
							// 階層名を取得
							...
							var lvl_obj = {
									...
									level : _this.level};
							// 取得したデータを表示する
							_this.$('#ca_MDCMV1050_box_tmp').tmpl(lvl_obj).appendTo('#ca_MDCMV1050_itgrptree');
							_this.$('#ca_MDCMV1050_node_tmp').tmpl(resultList).appendTo('#ca_MDCMV1050_box_' + _this.level);
						}


					}
					// スクロール
			        $('#ca_MDCMV1050_itgrptree').scrollTo('#ca_MDCMV1050_box_' + _this.level);

					clutil.initUIelement(_this.$el);
//					$('#ca_MDCMV1050_box_' + _this.level).closest('.yScroll').slimScroll({
//						color: '#ccc',
//					});
				} else {
					// ヘッダーにメッセージを表示
		 ...
		 * 商品用検索条件入力
		 */
		_onItemSrchInputKeyPress: function(e, ev) {
			var _this = this;
			...
			// 商品検索
			// EnterまたはTab押下時はサーバー接続
			if (key == 13 || key == 9) {
				// 検索条件：商品分類名
				var name = this.$('input.ca_MDCMV1050_item_srch_input').val();
				if (this.req.cond.name != name) {
					// 検索条件が変更されていたらサーバー接続
					// 商品の結果リストを削除
					this.removeItgrpList();
					this.onItemPageClick(this.pageNumber, this.itemsOnPage, true);
				}
			}
		},

		// 商品の結果リストを削除
		removeItgrpList: function() {
			if (this.$('input.ca_MDCMV1050_item_srch_input').length == 0) {
				return;
			} else if (this.resultList.length == amcm_type.AMCM_VAL_ITGRP_LEVEL_CTG) {
				return;
			}
			for (var i = this.resultList.length-1; i >= 0; i--) {
				this.resultList.splice(i, 1);
				this.pageNumber--;
		...
		// 結果表示（商品用）
		onItemSearchClick: function(parent_id) {
			var _this = this;
			...
			this.onItemPageClick(1, this.itemsOnPage);
		},

		// ページャークリック（商品用）
		onItemPageClick: function(pageNumber, itemsOnPage, isPager) {
			var _this = this;
			...
			// 検索条件：商品分類名
			var name = clutil.cStr(this.$('input.ca_MDCMV1050_item_srch_input').val());
			if (this.req.cond.name != name) {
				// 検索条件が変更されていたらページャー内容を変更する
				this.req.cond.name = name;
				this.req.pagereq.start_record = 0;
				// 商品の結果リストを削除
				this.removeItgrpList();
				this.$('#ca_MDCMV1050_box_' + _this.level).empty();
			}
			...
			clutil.postAnaJSON(uri, this.req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (resultList == null || resultList.length == 0) {
						...
						// 階層名を取得
						var lvllist = _this.levelList[this.req.cond.func_id];
						var lvl_obj = {
								lvl_name : lvllist[_this.level].lvl_name,
								level : _this.level};
						// 取得したデータを表示する
						if (!isPager) {
							_this.$('#ca_MDCMV1050_box_tmp').tmpl(lvl_obj).appendTo('#ca_MDCMV1050_itgrptree');
							// 検索inputに商品用クラスを追加
							_this.$('input.ca_MDCMV1050_srch_input[tgt-id=ca_MDCMV1050_box_' +
									_this.level + ']').addClass('ca_MDCMV1050_item_srch_input');

//							var $input = _this.$('input.ca_MDCMV1050_srch_input[tgt-id=ca_MDCMV1050_box_' +
//									_this.level + ']');
//							$input.blur(function(e){
//								_this.pageNumber = 1;
//								_this.onItemPageClick(_this.pageNumber, _this.itemsOnPage);
//							});
						}
						_this.$('#ca_MDCMV1050_node_tmp').tmpl(resultList).appendTo('#ca_MDCMV1050_box_' + _this.level);

						// ページ情報からもっと見るボタンを設定する
						_this.setShowMore(data.pagersp);
					}
					// スクロール
			        $('#ca_MDCMV1050_itgrptree').scrollTo('#ca_MDCMV1050_box_' + _this.level);
...
//					});
		 ...
		 * もっと見るボタンの設定
		 */
		setShowMore: function(pagersp) {
			// 総ページ数
			var page_num = pagersp.page_num;

			// もっと見るボタンを一旦非表示
			this.$('#ca_MDCMV1050_box_' + this.level).find('tr.ca_showmore_tr').remove();

			if (this.pageNumber < page_num) {
				// もっと見るボタンを表示
				this.$('#ca_MDCMV1050_item_node_tmp').tmpl({}).appendTo('#ca_MDCMV1050_box_' + this.level);
			}
		},
		 ...
		 * ツリーを作成する
		 */
		makeTree: function(list, func_id, pagersp) {
			...
			var no = 0;	//表示番号セット
			var lvl_no = 0; // レベル番号を取得
			// 取得したデータを表示する
			_.each(list, function(aNode) {
				aNode.no = no++;
				lvl_no = this.lvl_no;
				if(!aNode.is_leaf){
					aNode.is_leaf = (aNode.lvl_no >= this.max_level) ? 1 : 0;
			...
			this.$('#ca_MDCMV1050_box_tmp').tmpl(lvl_obj).appendTo('#ca_MDCMV1050_itgrptree');
			this.$('#ca_MDCMV1050_node_tmp').tmpl(list).appendTo('#ca_MDCMV1050_box_' + this.level);

			// 商品の場合はもっと見るを表示する 4/29
			if(lvl_no >= amcm_type.AMCM_VAL_ITGRP_LEVEL_ITEM){
				// 検索inputに商品用クラスを追加
				this.$('input.ca_MDCMV1050_srch_input[tgt-id=ca_MDCMV1050_box_' +
						this.level + ']').addClass('ca_MDCMV1050_item_srch_input');
				// ページ情報からもっと見るボタンを設定する
				this.setShowMore(pagersp);
			}
			...
			if (childlist != null && childlist.length > 0) {
				this.level++;
				this.makeTree(childlist, func_id, pagersp);
			}
		...
		clearResult: function() {
			// 結果表示エリアをクリア
			this.$('#ca_MDCMV1050_itgrptree').empty();
			// validatorの初期化
			this.validator.clear();
			// 内部用レベルを初期化
			this.level = 0;
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_MDCMV1050_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onAddClick:function(e) {
			...
			var _this = this;
			var lvl_flag = false;
			$.each(this.$("#ca_MDCMV1050_itgrptree table.final tr.selected"), function() {
				if (_this.lvl_id != -1 &&
			...
			if (chkId.length > 0) {
				...
				$.each(this.$('#ca_MDCMV1050_edit_tbl li'), function() {
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					// 種別を挿入する
					newdata.kind = amgbp_AnaDefs.AMGBA_DEFS_KIND_ITGRP;
					// 階層種別
					newdata.func_id = this.$('#ca_MDCMV1050_func_id').val();

					html_source += this.makeEditTmp(newdata);
					num++;
					editlist.push(newdata);
				}
				this.$('#ca_MDCMV1050_edit_tbl').append(html_source);
			} else {
				// なにもしない
			}
			// アニメーション表示のための初期化
		...
		makeEditTmp: function(data) {
			...
			html_source += data.code + '</span><span class="btn-delete ca_MDCMV1050_edit_del"></span>'
		...
		_onEditDelAllClick: function() {
			...
			var idlist = [];
			var html_source = '';
			this.$('#ca_MDCMV1050_edit_tbl').html("");
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_MDCMV1050_edit_tbl').children());
			return editlist;
		},
		...
		_onCommitClick: function() {
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_MDCMV1050_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
