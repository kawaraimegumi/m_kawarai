$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCMV1370SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			//"change #ca_MDCMV1370_dispgroup"	:	"_onDispGroupSelect",	// 分類項目変更
			"click #ca_MDCMV1370_main .ca_MDCMV1370_addall"
												:	"_onAddAllClick",		// 全選択ボタン押下
			"click #ca_MDCMV1370_main .ca_MDCMV1370_add"
												:	"_onAddClick",			// 選択ボタン押下

			"click #ca_MDCMV1370_edit_tbody .ca_MDCMV1370_sort"
												:	"_onSortClick",			// ソートボタン押下

			"click #ca_MDCMV1370_main .ca_MDCMV1370_dispitem"
												:	"_onDispItemClick",		// 表示項目押下
			"click #ca_MDCMV1370_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下
			"click #ca_MDCMV1370_main .ca_MDCMV1370_edit_delete"
												:	"_onEditDeleteClick",	// 単一削除ボタン押下
			"click #ca_MDCMV1370_up"			:	"_onUpClick",			// ↑ボタン押下
			"click #ca_MDCMV1370_down"			:	"_onDownClick",			// ↓ボタン押下

			"click #ca_MDCMV1370_commit"		:	"_onCommitClick",		// 確定ボタン押下
			"click #ca_MDCMV1370_main .close"	:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV1370_cancel"		:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_MDCMV1370_main .modalBK"	:	"_onCancelClick"	// 枠外押下時
		},
...
//					'dispgroup');
			...
			if (editList != null && editList.length > 0) {
				this.$('#ca_MDCMV1370_edit_tbody_tmp').tmpl(editList).appendTo('#ca_MDCMV1370_edit_tbody');
			}
			...
			if (vsortlist.length > 0) {
				...
				var tr = this.$('#ca_MDCMV1370_edit_tbody').find('tr[id=' + sortkey.dispitem_id + ']');
				var order = sortkey.order == amgbp_AnaSortKey.AMGBP_ANA_SORTKEY_ORDER_ASCENDING ?
						'asc' : 'dsc';
...
//			url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		show: function(editList, isSubDialog, kind, func_id, name) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV1370_main'), {
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_MDCMV1370_dispgroup'));
		},
		...
		_onDispGroupSelect: function(e) {
			...
			this.$('#ca_MDCMV1370_tbody').empty();
			this.$('#ca_MDCMV1370_tbody_tmp').tmpl(mstitemList).appendTo('#ca_MDCMV1370_tbody');

			clutil.initUIelement(this.$('#ca_MDCMV1370_tbody'));
		},
		...
		getDispItem: function() {
			...
			var mastitem_list = clcom.getStoredValue('mstitem_list_gba');
			var mstitemList = [];
        ...
        getSelectedDispitemIdMap: function(){
            var map = {};
            this.$('#ca_MDCMV1370_edit_tbody tr').each(function(idx, elem){
                var $tr = $(elem);
...
//				});
		 ...
		 */
		setButtons: function() {
			var $buttons = this.$('button.ca_MDCMV1370_add');
			// 一旦すべて使用可にする
		...
		_onSortClick: function(e) {
			...
			// すべてのソート解除
			this.$('#ca_MDCMV1370_edit_tbody').find('span.asc').removeClass('active');
			this.$('#ca_MDCMV1370_edit_tbody').find('span.dsc').removeClass('active');
		...
		_onEditDelAllClick: function() {
			this.$('#ca_MDCMV1370_edit_tbody').empty();
		},
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_MDCMV1370_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onAddAllClick: function(e) {
			var $rPool = this.$("#ca_MDCMV1370_edit_tbody");
			var $lastTR = null;

			var map = this.getSelectedDispitemIdMap();
			this.$('#ca_MDCMV1370_tbl tr').each(_.bind(function(idx, elem){
				var $tr = $(elem);
				var dispitem_id = $tr.find('input[name="dispitem_id"]').val();
				var name = $tr.find('input[name="name"]').val();
				var dto = {
				...
				var $addTR = this.$('#ca_MDCMV1370_edit_tbody_tmp').tmpl(dto);
				$rPool.append($addTR);
				$lastTR = $addTR;
            },this));
			// 末尾へスクロール
		...
		_onAddClick:function(e) {
			...
			//var $tr = this.$('#ca_MDCMV1370_tbl tr.selected');
			_.each(this.$('#ca_MDCMV1370_tbl tr.selected'), _.bind(function(tr) {
				var $tr = $(tr);
				var dispitem_id = $tr.find('input[name="dispitem_id"]').val();
				var name = $tr.find('input[name="name"]').val();
				var dto = {
		...
		_onUpClick: function(e) {
			var trlist = this.$('#ca_MDCMV1370_edit_tbl').find('tr.selected');
			...
			var $tr_up = null;
			var trlist_edit = this.$('#ca_MDCMV1370_edit_tbl').find('tr');
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_MDCMV1370_edit_tbody').children());
		...
		getSortkey : function() {
			// ソートキー設定
			var sortkey = null;
			var $span = this.$('#ca_MDCMV1370_edit_tbody').find('span.active');
			if ($span.length > 0) {
