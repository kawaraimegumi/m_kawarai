$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCMV1080SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_MDCMV1080_search"					:	"_onSearchClick",		// 検索ボタン押下
			"click #searchAgain"							:	"_onSearchAgainClick",	// 検索条件を再指定ボタン押下
			"click #ca_MDCMV1080_add"						:	"_onAddClick",			// 追加ボタン押下

			'click #ca_MDCMV1080_edit_del_all'				:	'_onEditDelAllClick', 	// 全削除ボタン押下
			"click .ca_MDCMV1080_edit_del"					:	"_onEditDelClick",		// 単一削除ボタン押下

			//			"click #ca_MDCMV1080_main .ca_MDCMV1080_tr"		:	"_onTrClick",			// テーブル行選択
			"click #ca_MDCMV1080_clear"						:	"_onClearClick",		// クリアボタン押下
			"click #ca_MDCMV1080_commit"					:	"_onCommitClick",		// 確定ボタン押下

			"change #ca_MDCMV1080_elem_func"				:	"_onElemFuncSelect",	// 種別変更
//			"keyup #ca_MDCMV1080_elem_func"					:	"_onElemFuncSelect",	// 種別変更
//			"keydown #ca_MDCMV1080_elem_func"				:	"_onElemFuncSelect",	// 種別変更

			"click #ca_MDCMV1080_main th.ca_th_sort"	:	"_onThSortClick",			// テーブルヘッダ押下

			"click #ca_MDCMV1080_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV1080_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_MDCMV1080_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},

		/**
		 * 引数
		 * ・$parentView		: 親画面のjQueryオブジェクト (例：$('#ca_main'))
		 * ・$dialog_main		: 自身のjQueryオブジェクト (例：$('#ca_MDCMV1080_dialog'))
		...
		initUIelement: function() {
			...
			if (this.isAnalyse_mode && this.select_mode != clutil.cl_single_select){
				...
				this.addtoSelected = clutil.addtoSelected(
						this.$('#ca_MDCMV1080_add'),
						this.$('#selected'),
						this.$('#mainColumn'));
			}

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_MDCMV1080_searchArea'),
					this.$('#ca_MDCMV1080_search'),
					this.$('#result'),
					this.$('#searchAgain'));
		...
		initTypeSelector: function() {
			clutil.cltypeselector(this.$('#ca_MDCMV1080_f_open'), amcm_type.AMCM_TYPE_OPEN, 1);
			clutil.cltypeselector2(this.$('#ca_MDCMV1080_elem_func'), this.funclist, 1,
					null,
					'func_id', 'func_name', 'func_code');
			clutil.initcltypeselector2(
					$('#ca_MDCMV1080_elem_lvl_div'), null, 1, null,
					null, null, null,
					{id : "ca_MDCMV1080_elem_lvl", name : "info"}, "mbn wt280 flleft");
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		initPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_MDCMV1080_pager1').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_MDCMV1080_pager_displaypanel1'),
			...
			this.$('#ca_MDCMV1080_pager2').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_MDCMV1080_pager_displaypanel2'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
					_this.onPageClick(pageNumber, itemsOnPage, true);
				},
		...
		initElemPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_MDCMV1080_elem_pager').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_MDCMV1080_elem_pager_displaypanel'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
		...
		show: function(editList, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV1080_main'), {
			...
			clutil.postAnaJSON(uri, req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					// 種別から階層リストを作成する
					for (var i = 0; i < _this.funclist.length; i++) {
						var func = _this.funclist[i];
						_this.levelList[func.func_id] = [];
					}
					for (var i = 0; i < _this.lvllist.length; i++) {
						var lvl = _this.lvllist[i];
						_this.levelList[lvl.lvl_func_id].push(lvl);
					}
					...
					if (editList != null && editList.length > 0) {
						...
						this.$('#ca_MDCMV1080_edit_tbl').html(html_source);
						// // サーバーに検索
...
//					_this.validator.setErrorInfo({_eb_: clutil.fmtargs(clutil.getclmsg(data.head.message), data.head.args)});
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_MDCMV1080_elem_func'));
		},
		...
		_onElemFuncSelect: function(e) {
			...
			clutil.initcltypeselector2(
					$('#ca_MDCMV1080_elem_lvl_div'),this.levelList[func_id], 1, null,
		...
		setSort: function(req) {
			...
			var th = this.$('#ca_MDCMV1080_tbl').find('th[key=' + sortreq.key + ']');
		...
		_onSearchClick: function() {
			...
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_MDCMV1080_searchArea'), 'ca_MDCMV1080_');
			searchData['f_genlist'] = am_proto_defs.AMDB_DEFS_F_GENLIST_ITEM;
			this.req = {
					cond : searchData
			};
			...
			clutil.postAnaJSON(uri, this.req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (_this.resultList == null || _this.resultList.length == 0) {
						...
						_this.$('#ca_MDCMV1080_tbody_tmp').tmpl(_this.resultList).appendTo('#ca_MDCMV1080_tbody');

						// ラジオボタンの初期化
						clutil.initUIelement(_this.$('#ca_MDCMV1080_tbl'));
						...
						if (_this.isEdit_id != null) {
							var tr = _this.$('#ca_MDCMV1080_tbl tr[id=' + _this.isEdit_id + ']')
							$(tr).find('[data-toggle="radio"]').radio('check');
//							tr.addClass('ca_MDCMV1080_select');
...
//							_this.onPageElemClick(1, clcom.itemsOnPage);
					...
					if (_this.$('#searchAgain').css('display') == 'none') {
						// 検索ボタンにフォーカスする
						_this.$('#ca_MDCMV1080_search').focus();
					} else {
						// 再検索ボタンにフォーカスする
...
//			$('#ca_MDCMV1080_elem_tbody').empty();
		...
		clearElemResult: function() {
			// テーブルをクリア
			this.$('#ca_MDCMV1080_elem_tbody').empty();
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onClearClick: function() {
			// 階層リストはクリアしておく
			$('#ca_MDCMV1080_elem_lvl_div').html('');

			clutil.viewClear(this.$('#ca_MDCMV1080_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onTrClick:function(e) {
			...
			// 既に選択されている場合はなにもしない
			if (tr.hasClass('ca_MDCMV1080_select')) {
				return;
			}
			this.$('#ca_MDCMV1080_tbl tr.ca_MDCMV1080_select').css('background-color', '');
			this.$('#ca_MDCMV1080_tbl tr.ca_MDCMV1080_select').removeClass('ca_MDCMV1080_select');
		...
		onPageElemClick: function(pageNumber, itemsOnPage, isPager) {
			...
			clutil.postAnaJSON(uri, this.elemreq, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (genlist != null) {
						...
						if (resultElemList == null || resultElemList.length == 0) {
							...
							_this.$('#ca_MDCMV1080_elem_tbody_tmp').tmpl(resultElemList).appendTo('#ca_MDCMV1080_elem_tbody');
				...
				if (_this.$('#ca_MDCMV1080_showSearchArea').css('display') == 'none') {
					// 検索ボタンにフォーカスする
					_this.$('#ca_MDCMV1080_search').focus();
				} else {
					// 検索条件を再指定ボタンにフォーカスする
					_this.$('#ca_MDCMV1080_showSearchArea').focus();
				}
			}, this));
		},
		...
		_onAddClick:function() {
			...
			var chkId = [];
			$.each(this.$('#ca_MDCMV1080_tbody').find("[name=ca_MDCMV1080_chk]:checked"), function() {
				var tr = $(this).closest('tr');
				chkId.push(tr.get(0).id);
			});

			if (chkId.length > 0) {
				var addedId = _.map(this.$('#ca_MDCMV1080_edit_tbl li'), function(li) {
					return Number(li.id);
				});
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					if (_.contains(addedId, select.genlist_id)) {
						continue;
					}
...
//					newdata.code = select.code;
					...
					// 種別を挿入する
					newdata.kind = amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMLIST;

					html_source += this.makeEditTmp(newdata);
				}
				// 編集領域を上書きする
				// this.$('#ca_MDCMV1080_edit_tbl').html();
				// this.$('#ca_MDCMV1080_edit_tbl').html(html_source);
				this.$('#ca_MDCMV1080_edit_tbl').append(html_source);
			} else {
				// なにもしない
			}
			// アニメーション表示のための初期化
		...
		makeEditTmp: function(data) {
			...
			html_source += '</span><span class="btn-delete ca_MDCMV1080_edit_del"></span>'
		...
		_onEditDelAllClick: function () {
			this.$('#ca_MDCMV1080_edit_tbl').empty();
		},
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_MDCMV1080_edit_tbl').children());
			return editlist;
		},
