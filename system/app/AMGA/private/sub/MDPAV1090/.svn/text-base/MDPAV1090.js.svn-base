$(function() {
	//////////////////////////////////////////////
	// View
	MDPAV1090PanelView = Backbone.View.extend({
		// 押下イベント
		events: {
		},
		...
		render: function() {
			...
			if (clcom.userInfo.user_typeid == amcm_type.AMCM_VAL_USER_STORE && clcom.srcId.slice(0, 4) == 'AMDB') {
				// 店舗ユーザーがMDBから遷移してきたときは「この条件で分析」のみ可
				this.$result_view.append(
					this.navResultItemView.render().$el.css('width', '100%')
				);
			}
			else if (this.anaProc.catalog_id != 0) {
				this.$result_view
		...
		initUIelement: function() {
			...
			// カタログ登録画面
			this.MDCTV1010Selector = new  MDCTV1010SelectorView({
				el : $('#ca_MDCTV1010_dialog'),	// 配置場所
...
//				select_mode		: clutil.cl_multiple_select,	// 複数選択モード
			...
			if(this.anaProc){
				...
				deferred.done(_.bind(function(data){
					if(_.isObject(data.head) && _.isNumber(data.head.status)
						...
						console.log('### MDPAV1090: anaProc.doSearch(), OK ###');
				...
				deferred.fail(_.bind(function(data){
					...
					if(_.isObject(data.head) && _.isNumber(data.head.status)
							&& data.head.status !== am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK){
						console.log('### MDPAV1090: anaProc.doSearch(), status[' + data.head.status + '] failed[' + data.head.message + '] ###');
		...
		navSaveCondItemClick: function(e) {
			...
			this.MDCTV1010Selector.show();

			//サブ画面復帰後処理
			this.MDCTV1010Selector.okProc = _.bind(function(data) {
				// ボタンにフォーカスする
				$(e.target).focus();
		...
		isValidCond: function(anaproc){
			var anaProc = this.anaProc;
			var cond = anaProc.cond;
			var vAxisList = cond.vAxisList;
			var hAxisList = cond.hAxisList;
			if (!vAxisList.length) {
				this.validator.setErrorHeader('縦軸を指定してください。');
				return false;
			}
			if (_.where(_.union(vAxisList, hAxisList), {kind: amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMLIST}).length) {
				if (!anaProc.getFocus1({kind: amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMLIST}).length) {
					this.validator.setErrorHeader('商品リストを指定してください。');
					return false;
				}
			}
			if (!cond.dispitemlist.length) {
				this.validator.setErrorHeader('表示項目を指定してください。');
				return false;
			}
			return true;
		}
