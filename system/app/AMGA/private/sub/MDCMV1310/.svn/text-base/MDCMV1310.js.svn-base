$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCMV1310SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"change #ca_MDCMV1310_kind"		:	"_onKindSelect",		// 種別変更
//			"keyup #ca_MDCMV1310_kind"		:	"_onKindSelect",		// 種別変更
//			"keydown #ca_MDCMV1310_kind"		:	"_onKindSelect",		// 種別変更

			"keyup .ca_MDCMV1310_srch_input"	:	"_onSrchInputKeyPress",	// テーブル上でリスト名編集確定

			"click .ca_MDCMV1310_node"			:	"_onNodeClick",			// ツリーノード押下

//			"click #ca_MDCMV1310_search"		:	"_onSearchClick",		// 検索ボタン押下
//			"click #ca_MDCMV1310_showSearchArea":	"_onShowSearchAreaClick",	// 検索条件を再指定ボタン押下
			"click #ca_MDCMV1310_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_MDCMV1310_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_MDCMV1310_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_MDCMV1310_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_MDCMV1310_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_MDCMV1310_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV1310_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_MDCMV1310_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function() {
			...
			if (this.select_mode == clutil.cl_single_select) {
				this.$('.ca_MDCMV1310_multi').remove();
			}
			// 分析画面以外からのモード
			...
			// 属性リストを取得
			this.kindlist =  clcom.getItemSpecList();

			// コンボボックスの中身を作成
			clutil.cltypeselector2(this.$('#ca_MDCMV1310_kind'), this.kindlist, 0,
					1, 'attr');
			if (this.kindlist != null && this.kindlist.length > 0) {
				this.$('#ca_MDCMV1310_kind').selectpicker('val', this.kindlist[0]);
			}
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		show: function(editList, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV1310_main'), {
			...
			if (this.select_mode == clutil.cl_multiple_select &&
				...
				this.$('#ca_MDCMV1310_edit_tbl').html(html_source);
			} else {
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_MDCMV1310_kind'));
		},
		...
		_onNodeClick: function(e) {
			...
			for (var i = this.level; i > level; i--) {
				// 内部レベルより大きいボックスは消去する
				this.$('#ca_MDCMV1310_box_' + i).closest('div.drilldownItem').remove();
			...
			if ($(table).find($('tr.selected')).length > 0) {
				...
				this.$('#ca_MDCMV1310_box_' + select_level).addClass('final');
			}
		...
		_onKindSelect: function(e) {
			...
			this.clearResult();
            //選択されている仕様種別番号取得
            var spec_id = $(e.target).val();
			//要素を取得
			this.onSearchClick(spec_id, 1);
		},
		...
		onSearchClick: function(parent_id, lvl_no) {
			...
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_MDCMV1310_searchArea'), 'ca_MDCMV1310_');
			...
			// データを取得
			var uri = 'gsan_se_itemspec_srch';
			if(Ana.Config.cond.MDCMV1310 && !_.isEmpty(Ana.Config.cond.MDCMV1310.itemspec_srch_resId)){
				uri = Ana.Config.cond.MDCMV1310.itemspec_srch_resId;
			}
			clutil.postAnaJSON(uri, req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {

					// 確定時用にデータは保存しておく
					var resultList = data.itemspeclist;
					if(uri != 'gsan_se_itemspec_srch' && data.body && _.isArray(data.body.list)){
					...
					if (resultList == null || resultList.length == 0) {
						...
						var lvl_obj = {
								level : _this.level};
						// 取得したデータを表示する
						_this.$('#ca_MDCMV1310_box_tmp').tmpl(lvl_obj).appendTo('#ca_MDCMV1310_itemspectree');
						_this.$('#ca_MDCMV1310_node_tmp').tmpl(resultList).appendTo('#ca_MDCMV1310_box_' + _this.level);
					}
					// スクロール
			        $('#ca_MDCMV1310_itemspectree').scrollTo('#ca_MDCMV1310_box_' + _this.level);
					clutil.initUIelement(_this.$el);
//					$('#ca_MDCMV1310_box_' + _this.level).closest('.yScroll').slimScroll({
//						color: '#ccc',
//					});
				} else {
					// ヘッダーにメッセージを表示
		...
		clearResult: function() {
			// 結果表示エリアをクリア
			this.$('#ca_MDCMV1310_itemspectree').empty();
			// validatorの初期化
			this.validator.clear();
			// 内部用レベルを初期化
			this.level = 0;
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_MDCMV1310_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onAddClick:function(e) {
			...
			var chkId = [];
			var _this = this;
			$.each(this.$("#ca_MDCMV1310_itemspectree table.final tr.selected"), function() {
				chkId.push(this.id);
			});
			...
			if (chkId.length > 0) {
				...
				var idlist = [];
				var html_source = '';
				//var kind = this.$('#ca_MDCMV1310_kind').val();
				var kind = amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMSPEC;

				$.each(this.$('#ca_MDCMV1310_edit_tbl li'), function() {
					//var edit_kind = $(this).find('input[name=kind]').val()
					var edit_kind = amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMSPEC;
					if (kind == edit_kind) {
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					newdata.attr = select.specitem_id;
					newdata.name2 = this.$('#ca_MDCMV1310_kind').find('option[value=' +
							this.$('#ca_MDCMV1310_kind').val() + ']').html();
				...
				this.$('#ca_MDCMV1310_edit_tbl').append(html_source);
			} else {
				// なにもしない
			}
			// アニメーション表示のための初期化
		...
		makeEditTmp: function(data) {
			...
			html_source += data.name2 + '</span><span class="btn-delete ca_MDCMV1310_edit_del"></span>'
		...
		_onEditDelAllClick: function() {
			...
			var idlist = [];
			var html_source = '';
			this.$('#ca_MDCMV1310_edit_tbl').html("");
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_MDCMV1310_edit_tbl').children());
			return editlist;
		},
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					for (var i = 0; i < selectlist.length; i++) {
						...
						// 種別を挿入する
						newdata.kind = this.$('#ca_MDCMV1310_kind').val();
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_MDCMV1310_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
