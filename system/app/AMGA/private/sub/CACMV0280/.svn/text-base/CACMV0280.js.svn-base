$(function() {
	...
	//////////////////////////////////////////////
	// View
	CACMV0280SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_CACMV0280_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_CACMV0280_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_CACMV0280_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_CACMV0280_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_CACMV0280_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_CACMV0280_show_CACMV0030"
												:	"_onShowCACMV0030Click",		// 店舗選択ボタン押下

			"change #ca_CACMV0280_anaattr"		:	"_onAnaAttrSelect",		// 属性値変更
//			"keyup #ca_CACMV0280_anaattr"		:	"_onAnaAttrSelect",		// 属性値変更
//			"keydown #ca_CACMV0280_anaattr"		:	"_onAnaAttrSelect",		// 属性値変更

			"click #ca_CACMV0280_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_CACMV0280_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_CACMV0280_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function() {
			...
			if (this.select_mode == clutil.cl_single_select) {
				this.$('.ca_CACMV0280_multi').remove();
				this.$('#ca_CACMV0280_chkall').remove();
			}
			// 分析画面以外からのモード
			...
			this.addtoSelected = clutil.addtoSelected(
					this.$('#ca_CACMV0280_add'),
					this.$('#selected'),
					this.$('#mainColumn'));

			// 属性リストを取得
			this.attrlist =  clcom.getTranAttrList();
			...
			if (clcom.pageId == 'CAANV0090' &&
				...
				for (var i = this.attrlist.length-1; i >= 0; i--) {
					if (this.attrlist[i].kind == amanp_AnaDefs.AMAN_DEFS_KIND_TRANATTR_NEWMEMB) {
			...
			// コンボボックスの中身を作成
			clutil.cltypeselector2(this.$('#ca_CACMV0280_anaattr'), this.attrlist, 0,
					1, 'kind', 'name');
...
//			url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		show: function(editList, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_CACMV0280_main'), {
			...
			if (this.select_mode == clutil.cl_multiple_select &&
				...
				this.$('#ca_CACMV0280_edit_tbl').html(html_source);
			} else {
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_CACMV0280_anaattr'));
		},
		 ...
		 */
		getAnaAttr: function() {
			var kind = this.$('#ca_CACMV0280_anaattr').val();
			var anaattr = {};
		...
		showData: function(anaattr) {
			...
			case am_proto_defs.AM_PROTO_ACTYPE_AGETYPE:			// MD商品分析 - 客層
				template = "ca_CACMV0280_table_template";
//				template = "ca_CACMV0280_CACMV0030_template";
				break;
			case am_proto_defs.AM_PROTO_ACTYPE_ORG:
				template = "ca_CACMV0280_org_template";
				break;
			default:
				break;
			}

			this.$('#ca_CACMV0280_result').empty();
			this.$('#' + template).tmpl().appendTo('#ca_CACMV0280_result');

			switch (Number(anaattr.actype)) {
			case am_proto_defs.AM_PROTO_ACTYPE_TEXT:
				// 全角
				this.$('#ca_CACMV0280_val').addClass("ime-active");
				this.$('#ca_CACMV0280_val').attr("data-limit", "len:" + maxlen);
//				// 半角カナチェックボックス表示 2013/09/06
////				this.$('.ca_CACMV0280_check').show();
				break;
			case am_proto_defs.AM_PROTO_ACTYPE_TYPE:
			case am_proto_defs.AM_PROTO_ACTYPE_CDNAME:
			case am_proto_defs.AM_PROTO_ACTYPE_STAFFCDNAME:
			...
			case am_proto_defs.AM_PROTO_ACTYPE_AGETYPE:			// MD商品分析 - 客層
				...
				case am_proto_defs.AM_PROTO_ACTYPE_TYPE:
					...
					this.$('#ca_CACMV0280_tbody_type_tmp').tmpl(this.typelist).appendTo('#ca_CACMV0280_tbody');
					break;
				case am_proto_defs.AM_PROTO_ACTYPE_CDNAME:
					this.typelist = clutil.getcdnamelist(anaattr.xtype);
					this.$('#ca_CACMV0280_tbody_cdname_tmp').tmpl(this.typelist).appendTo('#ca_CACMV0280_tbody');
					break;
				case am_proto_defs.AM_PROTO_ACTYPE_STAFFCDNAME:
				...
				case am_proto_defs.AM_PROTO_ACTYPE_MARKETTYPE:	// MD商品分析 - マーケット
					...
					this.$('#ca_CACMV0280_tbody_cdname2_tmp').tmpl(this.typelist).appendTo('#ca_CACMV0280_tbody');
					break;
				case am_proto_defs.AM_PROTO_ACTYPE_AGETYPE:		// MD商品分析 - 客層
				...
				var _this = this;
				// 全選択チェックボックスを初期化
				this.chkall = clutil.checkallbox(this.$('#ca_CACMV0280_chkall'),
						this.$('.ca_CACMV0280_tbl'),
						this.$('#ca_CACMV0280_tbody')
						);
				break;
				$(window).scrollTo(1);	// ダブルスクロール防止
//			case am_proto_defs.AM_PROTO_ITEM_TYPE_TEXTHAN:
//				// 半角
//				this.$('#ca_CACMV0280_text').addClass("ime-disabled");
//				this.$('#ca_CACMV0280_text').attr("data-limit", "len:" + maxlen);
//				break;
//			case am_proto_defs.AM_PROTO_ITEM_TYPE_TEXTNUM:
//				// 半角数値
//				this.$('#ca_CACMV0280_text').addClass("ime-disabled");
//				this.$('#ca_CACMV0280_text').attr("data-limit", "len:" + maxlen + " digit");
//				break;
			case am_proto_defs.AM_PROTO_ACTYPE_YMDRANGE:
				clutil.datepicker(this.$("#ca_CACMV0280_val"));
				clutil.datepicker(this.$("#ca_CACMV0280_val2"));
				break;
			default:
				break;
			}
			...
			// 単位の表示
			this.$('#ca_CACMV0280_item_unit').html(clutil.cStr(anaattr.item_unit));
		...
		_onShowCACMV0030Click: function(e) {
			...
			this.CACMV0280_CACMV0030Selector.show(null, true);
			//サブ画面復帰後処理
			this.CACMV0280_CACMV0030Selector.okProc = function(data) {
				if(data != null) {
					_this.$('#ca_CACMV0280_name').val(data[0].code + ":" + data[0].name);
					_this.$('#ca_CACMV0280_val').val(data[0].val);
				}
				// ボタンにフォーカスする
				$(e.target).focus();
			}
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_CACMV0280_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onAddClick:function() {
			...
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_CACMV0280_result'), 'ca_CACMV0280_');

			var anaattr = this.getAnaAttr();

			// 編集領域のIDリストを取得する
			var idlist = {};
			$.each(this.$('#ca_CACMV0280_edit_tbl li'), function() {
			...
			// 現在の要素数を取得
			var num = this.$('#ca_CACMV0280_edit_tbl li').length;
			...
			case am_proto_defs.AM_PROTO_ACTYPE_YMDRANGE:
				...
				case am_proto_defs.AM_PROTO_ACTYPE_NUMRANGE100:
					...
					var chkInfo = [];
					chkInfo.push({
						stval : 'ca_CACMV0280_val',
						edval : 'ca_CACMV0280_val2'
					});
				...
				this.$('#ca_CACMV0280_edit_tbl').append(this.makeEditTmp(newdata));
				break;
			case am_proto_defs.AM_PROTO_ACTYPE_ONOFF:
				break;
...
//				break;
			...
			case am_proto_defs.AM_PROTO_ACTYPE_AGETYPE:			// MD商品分析 - 客層
				var chkId = [];
				$.each(this.$('#ca_CACMV0280_tbody').find("[name=ca_CACMV0280_chk]:checked"), function() {
				...
				if (chkId.length > 0) {
					...
					this.$('#ca_CACMV0280_edit_tbl').append(html_source);
				} else {
					// なにもしない
				}
				// 全選択チェックボックスを初期化
		...
		makeEditTmp: function(data) {
			...
			html_source += data.name2 + '</span><span class="btn-delete ca_CACMV0280_edit_del"></span>'
...
//			html_source += '<input type="hidden" name="attr" value="' + data.attr + '">';
		...
		_onEditDelAllClick: function() {
			...
			var idlist = [];
			var html_source = '';
			this.$('#ca_CACMV0280_edit_tbl').html("");
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_CACMV0280_edit_tbl').children());
			return editlist;
		},
