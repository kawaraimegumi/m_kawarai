$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCMV0170SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"change #ca_MDCMV0170_func_id"		:	"_onFuncSelect",		// 種別変更
//			"keyup #ca_MDCMV0170_func_id"		:	"_onFuncSelect",		// 種別変更
//			"keydown #ca_MDCMV0170_func_id"		:	"_onFuncSelect",		// 種別変更

			"keyup .ca_MDCMV0170_srch_input"	:	"_onSrchInputKeyPress",	// テーブル上でリスト名編集確定

			"click .ca_MDCMV0170_node"			:	"_onNodeClick",			// ツリーノード押下

//			"click #ca_MDCMV0170_search"		:	"_onSearchClick",		// 検索ボタン押下
//			"click #ca_MDCMV0170_showSearchArea":	"_onShowSearchAreaClick",	// 検索条件を再指定ボタン押下
			"click #ca_MDCMV0170_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_MDCMV0170_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_MDCMV0170_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_MDCMV0170_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_MDCMV0170_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_MDCMV0170_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV0170_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_MDCMV0170_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function() {
			...
			if (this.select_mode == clutil.cl_single_select) {
				this.$('.ca_MDCMV0170_multi').remove();
			}
			// 分析画面以外からのモード
			...
			// 種別が設定されている場合は種別変更不可
			if (this.func_id != -1) {
				this.$('#ca_MDCMV0170_func_id').attr('disabled', 'disabled');
			}
			clutil.cltypeselector2(this.$('#ca_MDCMV0170_func_id'), this.funclist, 0,
					null,
					'func_id', 'func_name', 'func_code');
			if (this.func_id != -1) {
				this.$('#ca_MDCMV0170_func_id').selectpicker('val', this.func_id);
			}

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_MDCMV0170_searchArea'),
					this.$('#ca_MDCMV0170_search'),
					this.$('#result'),
					this.$('#searchAgain'));
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		 ...
		 * initData		: 初期表示条件 org_id, func_idを設定する
		 * r_org_id		: r_org_idを設定した場合、必ずこの配下からの表示にする
		 *
		 */
		show: function(editList, isSubDialog, func_id, lvl, initData, r_org_id) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV0170_main'), {
			...
			clutil.postAnaJSON(uri, req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (this.select_mode == clutil.cl_multiple_select &&
						...
						this.$('#ca_MDCMV0170_edit_tbl').html(html_source);
...
//						this.initData = null;
					...
					if (this.initData != null) {
						this.$('#ca_MDCMV0170_func_id').selectpicker('val', this.initData.func_id);
					} else {
						// 引数がない場合のみ検索
						if (this.func_id == -1) {
							// 親組織=0で第1階層を取得
...
//					_this.validator.setErrorInfo({_eb_: clutil.fmtargs(clutil.getclmsg(data.head.message), data.head.args)});
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_MDCMV0170_func_id'));
		},
		...
		_onNodeClick: function(e) {
			...
			for (var i = this.level; i > level; i--) {
				// 内部レベルより大きいボックスは消去する
				this.$('#ca_MDCMV0170_box_' + i).closest('div.drilldownItem').remove();
			...
			if ($(table).find($('tr.selected')).length > 0) {
				...
				this.$('#ca_MDCMV0170_box_' + select_level).addClass('final');
			}
		...
		onSearchClick: function(parent_id, r_org_id) {
			...
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_MDCMV0170_searchArea'), 'ca_MDCMV0170_');
					...
					if (org_id != null) {
						...
						if (resultList == null || resultList.length == 0) {
							...
							for (var i = 0; i < _this.idList.length; i++) {
								var tr = $('#ca_MDCMV0170_orgtree tr[id=' + _this.idList[i] + ']')
						...
						if (resultList == null || resultList.length == 0) {
							...
							var lvl_obj = {
									...
									level : _this.level};
							// 取得したデータを表示する
							_this.$('#ca_MDCMV0170_box_tmp').tmpl(lvl_obj).appendTo('#ca_MDCMV0170_orgtree');
							_this.$('#ca_MDCMV0170_node_tmp').tmpl(resultList).appendTo('#ca_MDCMV0170_box_' + _this.level);
						}
					}
					// スクロール
			        $('#ca_MDCMV0170_orgtree').scrollTo('#ca_MDCMV0170_box_' + _this.level);

					clutil.initUIelement(_this.$el);
//					$('#ca_MDCMV0170_box_' + _this.level).closest('.yScroll').slimScroll({
//						color: '#ccc',
//					});
				} else {
					// ヘッダーにメッセージを表示
		...
		makeTree: function(list, func_id) {
			...
			this.$('#ca_MDCMV0170_box_tmp').tmpl(lvl_obj).appendTo('#ca_MDCMV0170_orgtree');
			this.$('#ca_MDCMV0170_node_tmp').tmpl(list).appendTo('#ca_MDCMV0170_box_' + this.level);
		...
		clearResult: function() {
			// 結果表示エリアをクリア
			this.$('#ca_MDCMV0170_orgtree').empty();
			// validatorの初期化
			this.validator.clear();
			// 内部用レベルを初期化
			this.level = 0;
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_MDCMV0170_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onAddClick:function(e) {
			...
			var _this = this;
			var lvl_flag = false;
			$.each(this.$("#ca_MDCMV0170_orgtree table.final tr.selected"), function() {
				if (_this.lvl_id != -1 &&
			...
			if (chkId.length > 0) {
				...
				$.each(this.$('#ca_MDCMV0170_edit_tbl li'), function() {
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					// 種別を挿入する
					newdata.kind = amgbp_AnaDefs.AMGBA_DEFS_KIND_ORG;
					// 階層種別
					newdata.func_id = this.$('#ca_MDCMV0170_func_id').val();

					html_source += this.makeEditTmp(newdata);
					num++;
					editlist.push(newdata);
				}
				this.$('#ca_MDCMV0170_edit_tbl').append(html_source);
			} else {
				// なにもしない
			}
			// アニメーション表示のための初期化
		...
		makeEditTmp: function(data) {
			...
			html_source += data.code + '</span><span class="btn-delete ca_MDCMV0170_edit_del"></span>'
		...
		_onEditDelAllClick: function() {
			...
			var idlist = [];
			var html_source = '';
			this.$('#ca_MDCMV0170_edit_tbl').html("");
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_MDCMV0170_edit_tbl').children());
			return editlist;
		},
		...
		_onCommitClick: function() {
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_MDCMV0170_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
