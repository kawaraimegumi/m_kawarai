$(function() {
	...
	//////////////////////////////////////////////
	// View
	CACMV0270SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_CACMV0270_search"		:	"_onSearchClick",		// 検索ボタン押下
			"click #searchAgain"				:	"_onSearchAgainClick",	// 検索条件を再指定ボタン押下
			"click #ca_CACMV0270_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_CACMV0270_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_CACMV0270_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_CACMV0270_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_CACMV0270_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_CACMV0270_show_CACMV0260"		:	"_onShowCACMV0260Click",		// DM企画グループ選択ボタン押下

			"click #ca_CACMV0270_main th.ca_th_sort"	:	"_onThSortClick",	// テーブルヘッダ押下

			"click #ca_CACMV0270_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_CACMV0270_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_CACMV0270_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function() {
			...
			clutil.initbuinfoselector(
					$('#ca_CACMV0270_bu_id_div'), 0, null,
					{id : "ca_CACMV0270_bu_id", name : "info"}, "mbn wt280 flleft");

//			// DM企画種別区分selector作成
//			clutil.initcltypeselector(
//					$('#ca_CACMV0270_f_dmprom_div'), gsdb_defs.MTTYPETYPE_F_DMPROM, 1, null,
//					{id : "ca_CACMV0270_f_dmprom",
//					 name : "info"}, "mbn wt280 flleft");

			// 単一選択モード
			if (this.select_mode == clutil.cl_single_select) {
				this.$('.ca_CACMV0270_multi').remove();
//				this.$('#ca_CACMV0270_chkall').remove();
			}
...
//			// 全選択チェックボックスを初期化
//			this.chkall = clutil.checkallbox(this.$('#ca_CACMV0270_chkall'),
//					this.$('#ca_CACMV0270_tbl'),
//					this.$('#ca_CACMV0270_tbody')
//					);
			...
			if (this.isAnalyse_mode && this.select_mode != clutil.cl_single_select){
				...
				this.addtoSelected = clutil.addtoSelected(
						this.$('#ca_CACMV0270_add'),
						this.$('#selected'),
						this.$('#mainColumn'));
			}

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_CACMV0270_searchArea'),
					this.$('#ca_CACMV0270_search'),
					this.$('#result'),
					this.$('#searchAgain'));

			// DM企画グループ選択画面
			this.CACMV0270_CACMV0260Selector = new  CACMV0260SelectorView({
				el : this.$('#ca_CACMV0270_CACMV0260_dialog'),	// 配置場所
				$parentView		: this.$('#ca_CACMV0270_main'),
				isAnalyse_mode	: this.isAnalyse_mode,			// 分析ユースかどうかフラグ？？？
//				ymd				: null,			// 検索日
				select_mode		: clutil.cl_single_select,	// 単一選択モード
				anaProc			: this.anaProc
			});
			this.CACMV0270_CACMV0260Selector.render();
		},
...
//			url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		initPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_CACMV0270_pager1').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0270_pager_displaypanel1'),
			...
			this.$('#ca_CACMV0270_pager2').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0270_pager_displaypanel2'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
					_this.onPageClick(pageNumber, itemsOnPage, true);
				},
		...
		show: function(editList, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_CACMV0270_main'), {
			...
			if (this.select_mode == clutil.cl_multiple_select &&
				...
				this.$('#ca_CACMV0270_edit_tbl').html(html_source);
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_CACMV0270_code'));
		},

		/**
		 * DM企画グループ選択ボタン押下
		 */
		_onShowCACMV0260Click: function(e) {
			var _this = this;
			...
			this.CACMV0270_CACMV0260Selector.show(null, true);
			//サブ画面復帰後処理
			this.CACMV0270_CACMV0260Selector.okProc = function(data) {
				if(data != null && data.length > 0) {
					_this.$('#ca_CACMV0270_dmpromgrpname').val(data[0].code + ":" + data[0].name);
					_this.$('#ca_CACMV0270_dmpromgrp_id').val(data[0].val);
				}
				// ボタンにフォーカスする
				$(e.target).focus();
			}
		...
		_onThSortClick: function(e) {
			...
			// ソートキーを挿入
			this.req['sortreq'] = {
					sort_key : gsan_cp_dmprom_srch_if['GSAN_PROTO_SORT_KEY_' + key],
		...
		setSort: function(req) {
			...
			var th = this.$('#ca_CACMV0270_tbl').find('th[key=' + sortreq.key + ']');
		...
		_onSearchClick: function() {
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_CACMV0270_searchArea'), 'ca_CACMV0270_');
			// 全角、半角対応
...
//							$select_template = _this.$('#ca_CACMV0270_tbody_multiple_tmp');
//							break;
//						case clutil.cl_single_select:
//						default:
							$select_template = _this.$('#ca_CACMV0270_tbody_single_tmp');
//							break;
//						}
						// 取得したデータを表示する
						$select_template.tmpl(_this.resultList).appendTo('#ca_CACMV0270_tbody');

						clutil.initUIelement(_this.$('#ca_CACMV0270_tbl'));
				...
				if (_this.$('#searchAgain').css('display') == 'none') {
					// 検索ボタンにフォーカスする
					_this.$('#ca_CACMV0270_search').focus();
				} else {
					// 再検索ボタンにフォーカスする
...
//			this.chkall.init();
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_CACMV0270_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onAddClick:function() {
			...
			var chkId = [];
			$.each(this.$('#ca_CACMV0270_tbody').find("[name=ca_CACMV0270_chk]:checked"), function() {
			...
			if (chkId.length > 0) {
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					// 種別を挿入する
					newdata.kind = gsanp_AnaDefs.GSAN_DEFS_KIND_DMPROM;

					html_source += this.makeEditTmp(newdata);
				}
				// 編集領域を上書きする
				this.$('#ca_CACMV0270_edit_tbl').html();
				this.$('#ca_CACMV0270_edit_tbl').html(html_source);
			} else {
				// なにもしない
			}
		...
		makeEditTmp: function(data) {
			...
			html_source += data.code + '</span><span class="btn-delete ca_CACMV0270_edit_del"></span>'
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_CACMV0270_edit_tbl').children());
			return editlist;
		},
		...
		_onCommitClick: function() {
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_CACMV0270_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
