$(function(){
	...
	var EditView = Backbone.View.extend({
		...
		initUIelement: function() {
			...
			clutil.initcltypeselector($('#ca_f_open_div'), gsdb_defs.MTTYPETYPE_F_OPEN,
					0, null,
		...
		showData: function(appdata) {
			...
			$.each(f_anacond_chklist, function(key, value) {
				if ((appdata.f_anacond & gsdb_defs['MTTYPE_F_ANACOND_' + key]) == 
						gsdb_defs['MTTYPE_F_ANACOND_' + key]) {
					f_anacond[key] = 1;
				} else {
					f_anacond[key] = 0;
				}
			...
			if (Number(appdata.anakind) == gsdb_defs.MTTYPE_F_ANAKIND_CUST_DMPROMGRP ||
					Number(appdata.anakind) == gsdb_defs.MTTYPE_F_ANAKIND_CUST_DMPROM) {
				$('.ca_period_area').remove();
				return;
			}
			...
			if (appdata.f_period_type == CATALOG_PERIOD_BEFORE) {
				...
				case gsanp_AnaPeriod.GSANP_ANA_PERIOD_TYPE_YM:
					// 月単位
					...
					break;
				case gsanp_AnaPeriod.GSANP_ANA_PERIOD_TYPE_YW:
					// 週単位
					...
					break;
				case gsanp_AnaPeriod.GSANP_ANA_PERIOD_TYPE_YMD:
					// 日単位
					break;
				default:
					break;
		...
		getFAnacondData : function(resultData){
			...
			$.each(f_anacond_chklist, function(key, value) {
				if (value) {
					f_anacond = f_anacond | gsdb_defs['MTTYPE_F_ANACOND_' + key];
				}
			});
	...
	clutil.getIniJSON(null, null, _.bind(function(data, dataType) {
		...
		headerview.render(function() {
			...
			if(clcom.srcId == "CACTV0020"){
				...
				// 画面起動モードを新規作成にする
				editView.ope_mode = gs_proto_defs.GS_PROTO_COMMON_RTYPE_NEW;
				// 更新権限
				editView.is_upd = 1;
			}
			// データを取得して画面に表示する
