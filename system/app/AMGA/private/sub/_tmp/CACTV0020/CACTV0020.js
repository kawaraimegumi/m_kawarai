$(function(){
	...
	var ListView = Backbone.View.extend({
		...
		onClickContextmenu: function(val, id) {
			var is_upd = Number($('#ca_tbody').find('tr[id=' + id + ']').attr('is_upd'));
			
			switch (val) {
			case 'ca_edit' :
				this.showEditpage(gs_proto_defs.GS_PROTO_COMMON_RTYPE_UPD, id, is_upd);
				break;
			case 'ca_catalog' :
				this.showCatalogpage(id);
				break;
			case 'ca_del' :
				if (is_upd == 1) {
					//更新権限がある場合
		...
		_onAnaProcClick: function(e) {
			...
			this.showCatalogpage(catalog_id);
		},
		 ...
		 * カタログを実行する
		 */
		showCatalogpage : function(catalog_id){
			...
			clcom.pushPage(clcom.appRoot + '/' + catalog.func_code + '/' + catalog.func_code + '.html',
					args, null, null, true);
		},
		...
		_onEditClick: function(e) {
			...
			this.showEditpage(gs_proto_defs.GS_PROTO_COMMON_RTYPE_UPD, catalog_id, is_upd);
		},
		...
		showEditpage : function(ope_mode, catalog_id, is_upd){
			if (catalog_id == null && ope_mode != gs_proto_defs.GS_PROTO_COMMON_RTYPE_NEW) {
				// エラー
				return;
			}
			var args = {
		...
		delOkcallback: function(obj) {
			...
			var req = {
					rtype : gs_proto_defs.GS_PROTO_COMMON_RTYPE_DEL,
					catalog : {
						id : obj.catalog_id
					}
			};
