$(function() {
	...
	//////////////////////////////////////////////
	// View
	CACMV0040SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_CACMV0040_search"					:	"_onSearchClick",		// 検索ボタン押下
			"click #searchAgain"							:	"_onSearchAgainClick",	// 検索条件を再指定ボタン押下
			"click #ca_CACMV0040_add"						:	"_onAddClick",			// 追加ボタン押下

			"click .ca_CACMV0040_edit_del"					:	"_onEditDelClick",		// 単一削除ボタン押下

			//			"click #ca_CACMV0040_main .ca_CACMV0040_tr"		:	"_onTrClick",			// テーブル行選択
			"click #ca_CACMV0040_clear"						:	"_onClearClick",		// クリアボタン押下
			"click #ca_CACMV0040_commit"					:	"_onCommitClick",		// 確定ボタン押下

			"change #ca_CACMV0040_elem_func"				:	"_onElemFuncSelect",	// 種別変更
//			"keyup #ca_CACMV0040_elem_func"					:	"_onElemFuncSelect",	// 種別変更
//			"keydown #ca_CACMV0040_elem_func"				:	"_onElemFuncSelect",	// 種別変更

			"click #ca_CACMV0040_main th.ca_th_sort"		:	"_onThSortClick",				// テーブルヘッダ押下

			"click #ca_CACMV0040_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_CACMV0040_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_CACMV0040_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},

		/**
		 * 引数
		 * ・$parentView		: 親画面のjQueryオブジェクト (例：$('#ca_main'))
		 * ・$dialog_main		: 自身のjQueryオブジェクト (例：$('#ca_CACMV0040_dialog'))
		...
		initUIelement: function() {
			...
			if (this.isAnalyse_mode && this.select_mode != clutil.cl_single_select){
				...
				this.addtoSelected = clutil.addtoSelected(
						this.$('#ca_CACMV0040_add'),
						this.$('#selected'),
						this.$('#mainColumn'));
			}

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_CACMV0040_searchArea'),
					this.$('#ca_CACMV0040_search'),
					this.$('#result'),
					this.$('#searchAgain'));
		...
		initTypeSelector: function() {
			clutil.cltypeselector(this.$('#ca_CACMV0040_f_open'), amcm_type.AMCM_TYPE_OPEN, 1);
			clutil.cltypeselector2(this.$('#ca_CACMV0040_elem_func'), this.funclist, 1,
					null,
					'func_id', 'func_name', 'func_code');
			clutil.initcltypeselector2(
					$('#ca_CACMV0040_elem_lvl_div'), null, 1, null,
					null, null, null,
					{id : "ca_CACMV0040_elem_lvl", name : "info"}, "mbn wt280 flleft");
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		initPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_CACMV0040_pager1').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0040_pager_displaypanel1'),
			...
			this.$('#ca_CACMV0040_pager2').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0040_pager_displaypanel2'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
					_this.onPageClick(pageNumber, itemsOnPage, true);
				},
		...
		initElemPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_CACMV0040_elem_pager').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0040_elem_pager_displaypanel'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
		...
		show: function(editList, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_CACMV0040_main'), {
			...
			clutil.postAnaJSON(uri, req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (editList != null && editList.length > 0) {
						...
						this.$('#ca_CACMV0040_edit_tbl').html(html_source);
						// サーバーに検索
...
//					this.validator.setErrorInfo({_eb_: clutil.fmtargs(clutil.getclmsg(data.head.message), data.head.args)});
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_CACMV0040_elem_func'));
		},
		...
		_onElemFuncSelect: function(e) {
			...
			clutil.initcltypeselector2(
					$('#ca_CACMV0040_elem_lvl_div'),this.levelList[func_id], 1, null,
		...
		setSort: function(req) {
			...
			var th = this.$('#ca_CACMV0040_tbl').find('th[key=' + sortreq.key + ']');
		...
		_onSearchClick: function() {
			...
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_CACMV0040_searchArea'), 'ca_CACMV0040_');
			...
			clutil.postAnaJSON(uri, this.req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (_this.resultList == null || _this.resultList.length == 0) {
						...
						_this.$('#ca_CACMV0040_tbody_tmp').tmpl(_this.resultList).appendTo('#ca_CACMV0040_tbody');

						// ラジオボタンの初期化
						clutil.initUIelement(_this.$('#ca_CACMV0040_tbl'));
						...
						if (_this.isEdit_id != null) {
							var tr = _this.$('#ca_CACMV0040_tbl tr[id=' + _this.isEdit_id + ']')
							$(tr).find('[data-toggle="radio"]').radio('check');
//							tr.addClass('ca_CACMV0040_select');
...
//							_this.onPageElemClick(1, clcom.itemsOnPage);
					...
					if (_this.$('#searchAgain').css('display') == 'none') {
						// 検索ボタンにフォーカスする
						_this.$('#ca_CACMV0040_search').focus();
					} else {
						// 再検索ボタンにフォーカスする
...
//			$('#ca_CACMV0040_elem_tbody').empty();
		...
		clearElemResult: function() {
			// テーブルをクリア
			this.$('#ca_CACMV0040_elem_tbody').empty();
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onClearClick: function() {
			// 階層リストはクリアしておく
			$('#ca_CACMV0040_elem_lvl_div').html('');

			clutil.viewClear(this.$('#ca_CACMV0040_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onTrClick:function(e) {
			...
			// 既に選択されている場合はなにもしない
			if (tr.hasClass('ca_CACMV0040_select')) {
				return;
			}
			this.$('#ca_CACMV0040_tbl tr.ca_CACMV0040_select').css('background-color', '');
			this.$('#ca_CACMV0040_tbl tr.ca_CACMV0040_select').removeClass('ca_CACMV0040_select');
		...
		onPageElemClick: function(pageNumber, itemsOnPage, isPager) {
			...
			clutil.postAnaJSON(uri, this.elemreq, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (genlist != null) {
						...
						if (resultElemList == null || resultElemList.length == 0) {
							...
							_this.$('#ca_CACMV0040_elem_tbody_tmp').tmpl(resultElemList).appendTo('#ca_CACMV0040_elem_tbody');
				...
				if (_this.$('#ca_CACMV0040_showSearchArea').css('display') == 'none') {
					// 検索ボタンにフォーカスする
					_this.$('#ca_CACMV0040_search').focus();
				} else {
					// 検索条件を再指定ボタンにフォーカスする
					_this.$('#ca_CACMV0040_showSearchArea').focus();
				}
			}, this));
		},
		...
		_onAddClick:function() {
			...
			var chkId = [];
			$.each(this.$('#ca_CACMV0040_tbody').find("[name=ca_CACMV0040_chk]:checked"), function() {
...
//					newdata.code = select.code;
					...
					// 種別を挿入する
					newdata.kind = amanp_AnaDefs.AMAN_DEFS_KIND_STORELIST;

					html_source += this.makeEditTmp(newdata);
				}
				// 編集領域を上書きする
				this.$('#ca_CACMV0040_edit_tbl').html();
				this.$('#ca_CACMV0040_edit_tbl').html(html_source);
			} else {
				// なにもしない
			}
		...
		makeEditTmp: function(data) {
			...
			html_source += '</span><span class="btn-delete ca_CACMV0040_edit_del"></span>'
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_CACMV0040_edit_tbl').children());
			return editlist;
		},
