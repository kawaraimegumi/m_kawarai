$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCMV0160SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_MDCMV0160_search"		:	"_onSearchClick",		// 検索ボタン押下
			"click #searchAgain"				:	"_onSearchAgainClick",	// 検索条件を再指定ボタン押下
			"click #ca_MDCMV0160_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_MDCMV0160_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_MDCMV0160_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_MDCMV0160_main th.ca_th_sort"	:	"_onThSortClick",				// テーブルヘッダ押下

			"click #ca_MDCMV0160_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_MDCMV0160_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_MDCMV0160_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV0160_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_MDCMV0160_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function() {
			...
			if (this.select_mode == clutil.cl_single_select) {
				this.$('.ca_MDCMV0160_multi').remove();
				this.$('#ca_MDCMV0160_chkall').remove();
			}
			...
			// 全選択チェックボックスを初期化
			this.chkall = clutil.checkallbox(this.$('#ca_MDCMV0160_chkall'),
					this.$('#ca_MDCMV0160_tbl'),
					this.$('#ca_MDCMV0160_tbody')
					);
			...
			if (this.isAnalyse_mode && this.select_mode != clutil.cl_single_select){
				...
				this.addtoSelected = clutil.addtoSelected(
						this.$('#ca_MDCMV0160_add'),
						this.$('#selected'),
						this.$('#mainColumn'));
			}

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_MDCMV0160_searchArea'),
					this.$('#ca_MDCMV0160_search'),
					this.$('#result'),
					this.$('#searchAgain'));
		},
...
//			url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		initPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_MDCMV0160_pager1').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_MDCMV0160_pager_displaypanel1'),
			...
			this.$('#ca_MDCMV0160_pager2').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_MDCMV0160_pager_displaypanel2'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
					_this.onPageClick(pageNumber, itemsOnPage, true);
				},
		...
		show: function(editList, isSubDialog, maker_id) {
			var _this = this;
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV0160_main'), {
			...
			if (this.select_mode == clutil.cl_multiple_select &&
				...
				this.$('#ca_MDCMV0160_edit_tbl').html(html_source);
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_MDCMV0160_code'));
		},
		...
		_onThSortClick: function(e) {
			...
			// ソートキーを挿入
			this.req['sortreq'] = {
					sort_key : aman_se_maker_srch_if['AMGBA_PROTO_SORT_KEY_' + key],
		...
		setSort: function(req) {
			...
			var th = this.$('#ca_MDCMV0160_tbl').find('th[key=' + sortreq.key + ']');
		...
		_onSearchClick: function() {
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_MDCMV0160_searchArea'), 'ca_MDCMV0160_');
			// 全角、半角対応
			...
			if (this.maker_id != null) {
				searchData.maker_id = this.maker_id;
			}
		...
		onPageClick: function(pageNumber, itemsOnPage, isPager) {
			...
			clutil.postAnaJSON(uri, this.req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (_this.resultList == null || _this.resultList.length == 0) {
						...
						case clutil.cl_multiple_select:
							$select_template = _this.$('#ca_MDCMV0160_tbody_multiple_tmp');
							break;
						case clutil.cl_single_select:
						default:
							$select_template = _this.$('#ca_MDCMV0160_tbody_single_tmp');
							break;
						}
						// 取得したデータを表示する
						$select_template.tmpl(_this.resultList).appendTo('#ca_MDCMV0160_tbody');

						clutil.initUIelement(_this.$('#ca_MDCMV0160_tbl'));
				...
				if (_this.$('#searchAgain').css('display') == 'none') {
					// 検索ボタンにフォーカスする
					_this.$('#ca_MDCMV0160_search').focus();
				} else {
					// 再検索ボタンにフォーカスする
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_MDCMV0160_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onAddClick:function() {
			...
			var chkId = [];
			$.each(this.$('#ca_MDCMV0160_tbody').find("[name=ca_MDCMV0160_chk]:checked"), function() {
			...
			if (chkId.length > 0) {
				...
				$.each(this.$('#ca_MDCMV0160_edit_tbl li'), function() {
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					// 種別を挿入する
					newdata.kind = amgbp_AnaDefs.AMGBA_DEFS_KIND_MAKER;

					html_source += this.makeEditTmp(newdata);
					num++;
					editlist.push(newdata);
				}
				this.$('#ca_MDCMV0160_edit_tbl').append(html_source);
			} else {
				// なにもしない
			}
			// 全選択チェックボックスを初期化
...
//			$(e.target).closest("li").fadeOut(300);
		...
		makeEditTmp: function(data) {
			...
			html_source += data.code + '</span><span class="btn-delete ca_MDCMV0160_edit_del"></span>'
		...
		_onEditDelAllClick: function() {
			...
			var idlist = [];
			var html_source = '';
			this.$('#ca_MDCMV0160_edit_tbl').html("");
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_MDCMV0160_edit_tbl').children());
			return editlist;
		},
		...
		_onCommitClick: function() {
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					// ヘッダーにメッセージを表示  - 「取引先を選択してください。」
					this.validator.setErrorInfo({_eb_: clmsg.ca_MDCMV0160_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
