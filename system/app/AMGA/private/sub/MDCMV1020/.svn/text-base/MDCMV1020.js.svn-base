$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCMV1020SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_MDCMV1020_commit"			:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_MDCMV1020_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV1020_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_MDCMV1020_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		typeRadioViews: [
			{
				options: {
					...
					anaFocusKind: amgbp_AnaDefs.AMGBA_DEFS_KIND_STOREATTR_CLOSED
				},
				view: null
			},
			{
				options: {
					...
					anaFocusKind: amgbp_AnaDefs.AMGBA_DEFS_KIND_STORE_SORT
				},
				view: null
			},
			{
				options: {
					...
					anaFocusKind: amgbp_AnaDefs.AMGBA_DEFS_KIND_NEWSTORE_LOGIC
				},
				view: null
			},
			{
				options: {
					...
					anaFocusKind: amgbp_AnaDefs.AMGBA_DEFS_KIND_NEWSTORE_COND
				},
				view: null
			},
			{
				options: {
					...
					anaFocusKind: amgbp_AnaDefs.AMGBA_DEFS_KIND_STOREATTR_SMX
				},
				view: null
			}
		],
		...
		initUIelement: function(editList) {
			...
			if (this.anaProc.func_code == "AMGAV2140") {
				this.typeRadioViews = this.typeRadioView_memb;
			}
			...
			// 流入・流出分析では基準日は不要
			if (this.anaProc.func_code == "AMGAV2140") {
				$("#ca_MDCMV1020_dateArea").hide();
			} else {
				$("#ca_MDCMV1020_dateArea").show();
			}
...
//			this.$el.html('');
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV1020_main'), {
		...
		// Idより選択されたデータを取得
		getData : function(){
			var viewData = clutil.view2data($('#ca_MDCMV1020_main'));
			return {
				anaFocus: _.reduce(this.typeRadioViews, function(list, typeRadioView){
