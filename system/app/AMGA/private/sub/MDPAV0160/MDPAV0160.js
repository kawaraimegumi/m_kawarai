...
$(function() {
	//////////////////////////////////////////////
	// View
	MDPAV0160PanelView = Backbone.View.extend({
		id: 'ca_MDPAV0160_view',
		tagName: 'table',
		className: 'tree mrgb10',
		template: _.template('<tr id="ca_MDPAV0160_showbuttons"><th colspan="2"><span class="treeClose"></span>取引先</th></tr>'),
		...
		initialize: function(opt) {
			...
			// Lv2: 取引先のメニューアイテム
			this.navMDCMV0160View = new AnaNaviItemView({
				title: '取引先',
				tr : {id: 'ca_navItemMDCMV0160'}
			}).on('onNaviItemClick', this.navItemMDCMV0160Click);

			// -----------------------------
			// 各セレクタView

			// 取引先選択画面
			this.MDCMV0160Selector = new MDCMV0160SelectorView({
				el : $('#ca_MDCMV0160_dialog'),	// 配置場所
...
//				ymd				: null,			// 検索日
		...
		render: function() {
			...
			// 取引先ボタン群
			var $navViewContainer = $('#ca_MDPAV0160_view');
			$navViewContainer.append(this.navMDCMV0160View.render().$el);

			this.MDCMV0160Selector.render();
		...
		initUIelement: function() {
			...
			// 取引先押下
			$("#ca_MDPAV0160_showbuttons").click(function(e){
				...
				if ($(span).hasClass('treeClose')) {
					...
					$('#ca_MDPAV0160_view').find('tr.ca_button').hide();
					$('#ca_MDPAV0160_view').find('tr.ca_condview').hide();
				} else {
					$(span).removeClass('treeOpen');
					$(span).addClass('treeClose');
					$('#ca_MDPAV0160_view').find('tr.ca_button').show();
					$('#ca_MDPAV0160_view').find('tr.ca_condview').show();
				}
			});
			//////////////////////////
			// 設定済条件の表示
			...
			if (this.anaProc.catalog != null) {
				...
				if ((f_anacond & amcm_type.AMCM_VAL_ANACOND_MAKER) == amcm_type.AMCM_VAL_ANACOND_MAKER) {
					var th = $("#ca_MDPAV0160_view").find('th.ca_th_edit');
					$(th).removeClass('ca_th_edit');
					var span = $("#ca_MDPAV0160_view").find('span.edit');
					$(span).remove();
					var span = $("#ca_MDPAV0160_view").find('span.ca_title');
