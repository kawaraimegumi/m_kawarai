$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCMV1030SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_MDCMV1030_search"		:	"_onSearchClick",		// 検索ボタン押下
			"click #searchAgain"				:	"_onSearchAgainClick",	// 検索条件を再指定ボタン押下
			"click #ca_MDCMV1030_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_MDCMV1030_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_MDCMV1030_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_MDCMV1030_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_MDCMV1030_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_MDCMV1030_show_MDCMV0170"		:	"_onShowMDCMV0170Click",		// 組織選択ボタン押下

			"click #ca_MDCMV1030_main th.ca_th_sort"	:	"_onThSortClick",				// テーブルヘッダ押下

			"click #ca_MDCMV1030_main .close"	:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV1030_cancel"		:	"_onCancelClick",	// キャンセルボタン押下時
		},
		...
		initUIelement: function() {
			...
			if (this.select_mode == clutil.cl_single_select) {
				...
				this.$('#ca_MDCMV1030_chkall').remove();
			}
			//選択した内容のスクロール
			this.$('#innerScroll').perfectScrollbar();

			// カレンダー
			clutil.datepicker(this.$("#ca_MDCMV1030_open_iymd_from"));
			clutil.datepicker(this.$("#ca_MDCMV1030_open_iymd_to"));
			clutil.datepicker(this.$("#ca_MDCMV1030_close_iymd_from"));
			clutil.datepicker(this.$("#ca_MDCMV1030_close_iymd_to"));
			// カレンダーの日付をクリア
			clutil.viewClear(this.$('#ca_MDCMV1030_searchArea'), false);

			clutil.initUIelement(this.$el);

			// 全選択チェックボックスを初期化
			this.chkall = clutil.checkallbox(this.$('#ca_MDCMV1030_chkall'),
					this.$('#ca_MDCMV1030_tbl'),
					this.$('#ca_MDCMV1030_tbody')
					);
			...
			if (this.isAnalyse_mode && this.select_mode != clutil.cl_single_select){
				...
				this.addtoSelected = clutil.addtoSelected(
						this.$('#ca_MDCMV1030_add'),
						this.$('#selected'),
						this.$('#mainColumn'));
			}

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_MDCMV1030_searchArea'),
					this.$('#ca_MDCMV1030_search'),
					this.$('#result'),
					this.$('#searchAgain'));

			// 組織選択画面
			this.MDCMV1030_MDCMV0170Selector = new  MDCMV0170SelectorView({
				el : this.$('#ca_MDCMV1030_MDCMV0170_dialog'),	// 配置場所
				$parentView		: this.$('#ca_MDCMV1030_main'),
				isAnalyse_mode	: this.isAnalyse_mode,			// 分析ユースかどうかフラグ？？？
//				ymd				: null,			// 検索日
				select_mode		: clutil.cl_single_select,	// 単一選択モード
				anaProc			: this.anaProc
			});
			this.MDCMV1030_MDCMV0170Selector.render();

			// エキスパンダ
			this.$('.expandFieldUnits').click(function(){
				$('.fieldUnitsHidden').slideToggle();
				$(this).find('span').fadeToggle();
				$("div#ca_MDCMV1030_searchArea").parent().css('height', 'auto');
			});

			// 組織区分
			clutil.checkall(this.$('#ca_MDCMV1030_orgkind_chkall'), this.$('#ca_MDCMV1030_orgkind'));
		},
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		initPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_MDCMV1030_pager1').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_MDCMV1030_pager_displaypanel1'),
			...
			this.$('#ca_MDCMV1030_pager2').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_MDCMV1030_pager_displaypanel2'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
		...
		show: function(editList, isSubDialog) {
			...
			// 組織区分の選択肢を再構築
			if(true){
				var $ul = this.$('#ca_MDCMV1030_orgkind');
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV1030_main'), {
			...
			if (this.select_mode == clutil.cl_multiple_select &&
				...
				this.$('#ca_MDCMV1030_edit_tbl').html(html_source);
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_MDCMV1030_code'));
		},
		...
		_onShowMDCMV0170Click: function(e) {
			...
			// 選択された情報を初期値として検索する
			var initData = {};
			initData.func_id = this.$('#ca_MDCMV1030_func_id').val();
			initData.org_id = this.$('#ca_MDCMV1030_org_id').val();
			...
			this.MDCMV1030_MDCMV0170Selector.show(null, true, null, null, initData);
			//サブ画面復帰後処理
			this.MDCMV1030_MDCMV0170Selector.okProc = function(data) {
				if(data != null && data.length > 0) {
					_this.$('#ca_MDCMV1030_orgname').val(data[0].code + ":" + data[0].name);
					_this.$('#ca_MDCMV1030_org_id').val(data[0].val);
					_this.$('#ca_MDCMV1030_func_id').val(data[0].func_id);
				}
				// ボタンにフォーカスする
				$(e.target).focus();
			}
		...
		setSort: function(req) {
			...
			var th = this.$('#ca_MDCMV1030_tbl').find('th[key=' + sortreq.key + ']');
		...
		_onSearchClick: function() {
			...
			var chkInfo = [];
			chkInfo.push({
				stval : 'ca_MDCMV1030_open_iymd_from',
				edval : 'ca_MDCMV1030_open_iymd_to'
			});
			chkInfo.push({
				stval : 'ca_MDCMV1030_close_iymd_from',
				edval : 'ca_MDCMV1030_close_iymd_to'
			});
			...
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_MDCMV1030_searchArea'), 'ca_MDCMV1030_');
			// 全角、半角対応
			...
			if(true){
				var selectedOrgIds = [];
				this.$('#ca_MDCMV1030_orgkind input:checked').each(function(){
		...
		onPageClick: function(pageNumber, itemsOnPage, isPager) {
			...
			clutil.postAnaJSON(uri, this.req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (this.resultList == null || this.resultList.length == 0) {
						...
						case clutil.cl_multiple_select:
							$select_template = this.$('#ca_MDCMV1030_tbody_multiple_tmp');
							break;
						case clutil.cl_single_select:
						default:
							$select_template = this.$('#ca_MDCMV1030_tbody_single_tmp');
							break;
						}
						// 取得したデータを表示する
						$select_template.tmpl(this.resultList).appendTo('#ca_MDCMV1030_tbody');

						clutil.initUIelement(this.$('#ca_MDCMV1030_tbl'));
				...
				if (this.$('#searchAgain').css('display') == 'none') {
					// 検索ボタンにフォーカスする
					this.$('#ca_MDCMV1030_search').focus();
				} else {
					// 再検索ボタンにフォーカスする
		...
		_onAddClick:function() {
			...
			var chkId = [];
			$.each(this.$('#ca_MDCMV1030_tbody').find("[name=ca_MDCMV1030_chk]:checked"), function() {
			...
			if (chkId.length > 0) {
				...
				$.each(this.$('#ca_MDCMV1030_edit_tbl li'), function() {
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					// 種別を挿入する
					newdata.kind = amgbp_AnaDefs.AMGBA_DEFS_KIND_STORE;

					html_source += this.makeEditTmp(newdata);
					num++;
					editlist.push(newdata);
				}
				this.$('#ca_MDCMV1030_edit_tbl').append(html_source);
			} else {
				// なにもしない
			}
			// 全選択チェックボックスを初期化
		...
		makeEditTmp: function(data) {
			...
			html_source += data.code + '</span><span class="btn-delete ca_MDCMV1030_edit_del"></span>'
		...
		_onEditDelAllClick: function() {
			...
			var idlist = [];
			var html_source = '';
			this.$('#ca_MDCMV1030_edit_tbl').html("");
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_MDCMV1030_edit_tbl').children());
			return editlist;
		},
		...
		_onCommitClick: function() {
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_MDCMV1030_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
