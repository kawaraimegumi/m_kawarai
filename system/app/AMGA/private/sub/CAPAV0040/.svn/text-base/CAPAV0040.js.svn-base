$(function() {
	//////////////////////////////////////////////
	// View
	CAPAV0040PanelView = Backbone.View.extend({
		id: 'ca_CAPAV0040_view',
		tagName: 'table',
		className: 'tree mrgb10',
		template: _.template(''
				+ '<tr id="ca_CAPAV0040_showbuttons">'
				+ '<th colspan="2"><span class="treeClose ca_CAPAV0040_tree"></span>会員'
				+ '<label class="checkbox ca_CAPAV0040_custall" for="ca_CAPAV0040_custall_chk">'
				+ '<input type="checkbox" class="ca_CAPAV0040_custall" id="ca_CAPAV0040_custall_chk" data-toggle="checkbox">全顧客'
				+ '</label>'
				+ '</th>'
				+ '</tr>'),
		...
		filter_ADDR : {
			kind : amanp_AnaDefs.AMAN_DEFS_KIND_ADDR
		},
		filter_ADDRLIST : {
			kind : amanp_AnaDefs.AMAN_DEFS_KIND_ADDRLIST
		},
		filter_MEMBLIST : {
			kind : amanp_AnaDefs.AMAN_DEFS_KIND_MEMBLIST
		},
		filter_MEMBATTR : {
			kind : amanp_AnaDefs.AMAN_DEFS_KIND_MEMBATTR
		},
		panelId : 'CAPAV0040',
...
//			}).on('onNaviItemClick', this.navItemCACMV0100Click);
			...
			// Lv2: 住所のメニューアイテム
			this.navCACMV0110View = new AnaNaviItemView({
				title: '住所',
				tr : {id: 'ca_navItemCACMV0110'}
			}).on('onNaviItemClick', this.navItemCACMV0110Click);
			...
			// Lv2: 会員リストのメニューアイテム
			this.navCACMV0130View = new AnaNaviItemView({
				title: '会員リスト',
				tr : {id: 'ca_navItemCACMV0130'}
			}).on('onNaviItemClick', this.navItemCACMV0130Click);

			// Lv2: 会員属性のメニューアイテム
			this.navCACMV0140View = new AnaNaviItemView({
				title: '会員属性',
				tr : {id: 'ca_navItemCACMV0140'}
			}).on('onNaviItemClick', this.navItemCACMV0140Click);
...
//			});
			...
			// 住所選択画面
			this.CACMV0110Selector = new CACMV0110SelectorView({
				el			: $('#ca_CACMV0110_dialog'),	// 配置場所
				$parentView	: this.$parentView,
//				isAnalyse_mode	: true,			// 分析ユースかどうかフラグ？？？
//				ymd				: null,			// 検索日
				select_mode		: clutil.cl_multiple_select,	// 複数選択モード
...
//				ymd				: null,			// 検索日
			...
			// 会員リスト選択画面
			this.CACMV0130Selector = new CACMV0130SelectorView({
				el : $('#ca_CACMV0130_dialog'),	// 配置場所
				$parentView	: this.$parentView,
//				isAnalyse_mode	: true,			// 分析ユースかどうかフラグ？？？
//				ymd				: null,			// 検索日
				select_mode		: clutil.cl_multiple_select,	// 複数選択モード
			...
			// 会員属性選択画面
			this.CACMV0140Selector = new CACMV0140SelectorView({
				el : $('#ca_CACMV0140_dialog'),	// 配置場所
				$parentView	: this.$parentView,
//				isAnalyse_mode	: true,			// 分析ユースかどうかフラグ？？？
//				ymd				: null,			// 検索日
				select_mode		: clutil.cl_multiple_select,	// 複数選択モード
...
//			.append(this.navCACMV0100View.render().$el)
			.append(this.navCACMV0110View.render().$el)
			.append(this.navCACMV0120View.render().$el)
			.append(this.navCACMV0130View.render().$el)
			.append(this.navCACMV0140View.render().$el)
//			.append(this.navCACMV0150View.render().$el);

//			this.CACMV0100Selector.render();
			this.CACMV0110Selector.render();
			this.CACMV0120Selector.render();
			this.CACMV0130Selector.render();
			this.CACMV0140Selector.render();
//			this.CACMV0150Selector.render();
		...
		initUIelement: function() {
			clutil.inputlimiter($('#ca_CAPAV0040_view'));
			clutil.initUIelement($('#ca_CAPAV0040_view'));

			var _this = this;

			// 会員押下
			$("#ca_CAPAV0040_showbuttons").click(function(e){
				// チェックボックス押下時はなにもしない
				if ($(e.target).closest('label').hasClass('ca_CAPAV0040_custall')) {
					return;
				}
				...
				var span = $(tr_button).find('span.ca_CAPAV0040_tree');

				if ($(span).hasClass('treeClose')) {
					$(span).removeClass('treeClose');
					$(span).addClass('treeOpen');
					$('#ca_CAPAV0040_view').find('tr.ca_button').hide();
					$('#ca_CAPAV0040_view').find('tr.ca_condview').hide();
				} else {
					$(span).removeClass('treeOpen');
					$(span).addClass('treeClose');
					$('#ca_CAPAV0040_view').find('tr.ca_button').show();
					$('#ca_CAPAV0040_view').find('tr.ca_condview').show();
				}
			});
			...
			// 全顧客チェックボックス押下
			$("#ca_CAPAV0040_view").delegate(':checkbox', 'toggle', function (e) {
				var tr_button = $(e.target).closest('tr');
				var span = $(tr_button).find('span.ca_CAPAV0040_tree');
				...
				if ($(this).prop('checked')) {
					...
					_this.setMembCond(amanp_AnaHead.AMANP_ANA_REQ_MEMBCOND_ALL);

					// 条件リストを保存してクリアする
					_this.condList = {};

					_this.condList['filter_ADDR'] = _this.anaProc.getFocus1(_this.filter_ADDR);
					_this.condList['filter_ADDRLIST'] = _this.anaProc.getFocus1(_this.filter_ADDRLIST);
					_this.condList['filter_MEMBLIST'] = _this.anaProc.getFocus1(_this.filter_MEMBLIST);
					_this.condList['filter_MEMBATTR'] = _this.anaProc.getFocus1(_this.filter_MEMBATTR);
					...
					// 全顧客タイプに設定
					_this.anaProc.cond.extra.membcond = amanp_AnaHead.AMANP_ANA_REQ_MEMBCOND_ALL;
				} else {
					_this.setMembCond(amanp_AnaHead.AMANP_ANA_REQ_MEMBCOND_MEMB);
					...
					_this.condList = {};
					// 会員タイプに設定
					_this.anaProc.cond.extra.membcond = amanp_AnaHead.AMANP_ANA_REQ_MEMBCOND_MEMB;
				}
			});
			...
			if (this.anaProc.catalog != null) {
				...
				if ((f_anacond & amcm_type.AMCM_VAL_ANACOND_MEMB) == amcm_type.AMCM_VAL_ANACOND_MEMB) {
					var th = $("#ca_CAPAV0040_view").find('th.ca_th_edit');
					$(th).removeClass('ca_th_edit');
					var span = $("#ca_CAPAV0040_view").find('span.edit');
					$(span).remove();
					var span = $("#ca_CAPAV0040_view").find('span.ca_title');
					$(th).removeClass('category');
					$(th).addClass('ca_th_disabled');
					$(span).addClass('ca_span_disabled');
					// 全顧客チェックボックスを使用不可にする
					clutil.viewReadonly($('#ca_CAPAV0040_showbuttons'));
				}
			}

			// 全顧客指定不可能の場合はチェックボックスを削除
			if (this.disableMemb) {
				$('.ca_CAPAV0040_custall').remove();
			}
		},

		// 会員タイプで表示方法を変える
		setMembCond: function(membcond) {
			var span = $('#ca_CAPAV0040_view').find('span.ca_CAPAV0040_tree');
			var tr = $('#ca_CAPAV0040_view').find('tr');

			if (membcond == amanp_AnaHead.AMANP_ANA_REQ_MEMBCOND_ALL) {
				// エキスパンダを閉じる
				$(span).removeClass('treeClose');
				$(span).addClass('treeOpen');
				...
				// 全顧客タイプに設定
				$('#ca_CAPAV0040_custall_chk').checkbox('check');
				$('#ca_CAPAV0040_label').removeClass('ca_memball');
				$('#ca_CAPAV0040_label').addClass('ca_custall');
			} else {
				// エキスパンダを開く
				$(span).removeClass('treeOpen');
				$(span).addClass('treeClose');
				...
				// 会員タイプに設定
				$('#ca_CAPAV0040_label').removeClass('ca_custall');
				$('#ca_CAPAV0040_label').addClass('ca_memball');
			}
		},
...
//		},
		...
		// 住所
		navItemCACMV0110Click: function(e) {
			var _this = this;
			e.srcBackboneView.setActive();

			var list = this.anaProc.getFocus1(this.filter_ADDR);
			this.CACMV0110Selector.show(list);

			//サブ画面復帰後処理
			this.CACMV0110Selector.okProc = function(data) {
				if(data != null) {
					_this.anaProc.removeFocus1(_this.filter_ADDR);
					_this.anaProc.pushFocus1(data);
					_this.anaProc.fireAnaCondUpdated({
						id : 'ca_navItemCACMV0110',
		...
		// 会員リスト
		navItemCACMV0130Click: function(e) {
			var _this = this;
			e.srcBackboneView.setActive();

			var list = this.anaProc.getFocus1(this.filter_MEMBLIST);
			this.CACMV0130Selector.show(list);

			//サブ画面復帰後処理
			this.CACMV0130Selector.okProc = function(data) {
				if(data != null) {
					_this.anaProc.removeFocus1(_this.filter_MEMBLIST);
					_this.anaProc.pushFocus1(data);
					_this.anaProc.fireAnaCondUpdated({
		...
		// 会員属性
		navItemCACMV0140Click: function(e) {
			var _this = this;
			e.srcBackboneView.setActive();

			var list = this.anaProc.getFocus1(this.filter_MEMBATTR);
			this.CACMV0140Selector.show(list);

			//サブ画面復帰後処理
			this.CACMV0140Selector.okProc = function(data) {
				if(data != null) {
					_this.anaProc.removeFocus1(_this.filter_MEMBATTR);
					_this.anaProc.pushFocus1(data);
					_this.anaProc.fireAnaCondUpdated({
						id : 'ca_navItemCACMV0140',
