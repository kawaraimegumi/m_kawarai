$(function() {
	...
	//////////////////////////////////////////////
	// View
	CACMV0130SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_CACMV0130_search"					:	"_onSearchClick",		// 検索ボタン押下
			"click #searchAgain"							:	"_onSearchAgainClick",	// 検索条件を再指定ボタン押下
			"click #ca_CACMV0130_add"						:	"_onAddClick",			// 追加ボタン押下

			"click .ca_CACMV0130_edit_del"					:	"_onEditDelClick",		// 単一削除ボタン押下

			//			"click #ca_CACMV0130_main .ca_CACMV0130_tr"		:	"_onTrClick",			// テーブル行選択
			"click #ca_CACMV0130_clear"						:	"_onClearClick",		// クリアボタン押下
			"click #ca_CACMV0130_commit"					:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_CACMV0130_main th.ca_th_sort"		:	"_onThSortClick",				// テーブルヘッダ押下

			"click #ca_CACMV0130_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_CACMV0130_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_CACMV0130_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function() {
			...
			clutil.cltypeselector(this.$('#ca_CACMV0130_f_open'), gsdb_defs.MTTYPETYPE_F_OPEN, 1);
			clutil.cltypeselector(this.$('#ca_CACMV0130_f_listuse'), gsdb_defs.MTTYPETYPE_F_LISTUSE, 1);

			clutil.initUIelement(this.$el);

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_CACMV0130_searchArea'),
					this.$('#ca_CACMV0130_search'),
					this.$('#result'),
					this.$('#searchAgain'));

			// 分析画面以外からのモード
			if (this.isAnalyse_mode == false) {
				this.$('.ca_CACMV0130_multi').remove();
...
//			url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		initPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_CACMV0130_pager1').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0130_pager_displaypanel1'),
			...
			this.$('#ca_CACMV0130_pager2').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0130_pager_displaypanel2'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
					_this.onPageClick(pageNumber, itemsOnPage, true);
				},
		...
		initElemPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_CACMV0130_elem_pager').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0130_elem_pager_displaypanel'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
		...
		show: function(editList, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_CACMV0130_main'), {
			...
			if (editList != null && editList.length > 0) {
				...
				this.$('#ca_CACMV0130_edit_tbl').html(html_source);
				// サーバーに検索
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_CACMV0130_name'));
		},
		...
		setSort: function(req) {
			...
			var th = this.$('#ca_CACMV0130_tbl').find('th[key=' + sortreq.key + ']');
		...
		_onSearchClick: function() {
			...
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_CACMV0130_searchArea'), 'ca_CACMV0130_');
			...
			clutil.postAnaJSON(uri, this.req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (_this.resultList == null || _this.resultList.length == 0) {
						...
						_this.$('#ca_CACMV0130_tbody_tmp').tmpl(_this.resultList).appendTo('#ca_CACMV0130_tbody');

						// ラジオボタンの初期化
						clutil.initUIelement(_this.$('#ca_CACMV0130_tbl'));
						...
						if (_this.isEdit_id != null) {
							var tr = _this.$('#ca_CACMV0130_tbl tr[id=' + _this.isEdit_id + ']')
							$(tr).find('[data-toggle="radio"]').radio('check');
//							tr.addClass('ca_CACMV0130_select');
...
//							_this.onPageElemClick(1, clcom.itemsOnPage);
					...
					if (_this.$('#searchAgain').css('display') == 'none') {
						// 検索ボタンにフォーカスする
						_this.$('#ca_CACMV0130_search').focus();
					} else {
						// 再検索ボタンにフォーカスする
...
//			$('#ca_CACMV0130_elem_tbody').empty();
		...
		clearElemResult: function() {
			// テーブルをクリア
			this.$('#ca_CACMV0130_elem_tbody').empty();
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_CACMV0130_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onTrClick:function(e) {
			...
			// 既に選択されている場合はなにもしない
			if (tr.hasClass('ca_CACMV0130_select')) {
				return;
			}
			this.$('#ca_CACMV0130_tbl tr.ca_CACMV0130_select').css('background-color', '');
			this.$('#ca_CACMV0130_tbl tr.ca_CACMV0130_select').removeClass('ca_CACMV0130_select');
		...
		onPageElemClick: function(pageNumber, itemsOnPage, isPager) {
			...
			clutil.postAnaJSON(uri, this.elemreq, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (genlist != null) {
						...
						if (resultElemList == null || resultElemList.length == 0) {
							...
							_this.$('#ca_CACMV0130_elem_tbody_tmp').tmpl(resultElemList).appendTo('#ca_CACMV0130_elem_tbody');
				...
				if (_this.$('#ca_CACMV0130_showSearchArea').css('display') == 'none') {
					// 検索ボタンにフォーカスする
					_this.$('#ca_CACMV0130_search').focus();
				} else {
					// 検索条件を再指定ボタンにフォーカスする
					_this.$('#ca_CACMV0130_showSearchArea').focus();
				}
			}, this));
		},
		...
		_onAddClick:function() {
			...
			var chkId = [];
			$.each(this.$('#ca_CACMV0130_tbody').find("[name=ca_CACMV0130_chk]:checked"), function() {
...
//					newdata.code = select.code;
					...
					// 種別を挿入する
					newdata.kind = amanp_AnaDefs.AMAN_DEFS_KIND_MEMBLIST;

					html_source += this.makeEditTmp(newdata);
				}
				// 編集領域を上書きする
				this.$('#ca_CACMV0130_edit_tbl').html();
				this.$('#ca_CACMV0130_edit_tbl').html(html_source);
			} else {
				// なにもしない
			}
		...
		makeEditTmp: function(data) {
			...
			html_source += '</span><span class="btn-delete ca_CACMV0130_edit_del"></span>'
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_CACMV0130_edit_tbl').children());
			return editlist;
		},
		...
		_onCommitClick: function() {
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_CACMV0130_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
