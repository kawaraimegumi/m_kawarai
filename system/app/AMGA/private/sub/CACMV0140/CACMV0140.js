$(function() {
	...
	//////////////////////////////////////////////
	// View
	CACMV0140SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_CACMV0140_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_CACMV0140_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_CACMV0140_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_CACMV0140_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_CACMV0140_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_CACMV0140_show_CACMV0030"
												:	"_onShowCACMV0030Click",		// 店舗選択ボタン押下

			"change #ca_CACMV0140_anaattr"		:	"_onAnaAttrSelect",		// 属性値変更
//			"keyup #ca_CACMV0140_anaattr"		:	"_onAnaAttrSelect",		// 属性値変更
//			"keydown #ca_CACMV0140_anaattr"		:	"_onAnaAttrSelect",		// 属性値変更

			"click #ca_CACMV0140_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_CACMV0140_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_CACMV0140_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function() {
			...
			if (this.select_mode == clutil.cl_single_select) {
				this.$('.ca_CACMV0140_multi').remove();
				this.$('#ca_CACMV0140_chkall').remove();
			}
			// 分析画面以外からのモード
...
//			// 組織選択画面
//			this.CACMV0140_CACMV0020Selector = new  CACMV0020SelectorView({
//				el : this.$('#ca_CACMV0140_CACMV0020_dialog')	// 配置場所
//			});
//			this.CACMV0140_CACMV0020Selector.render(
//					this.$('#ca_CACMV0140_main'),
//            		clutil.cl_single_select,	// 単一数選択モード
//            		null						// 検索日
//            );

			// 店舗選択画面
			this.CACMV0140_CACMV0030Selector = new  CACMV0030SelectorView({
				el : this.$('#ca_CACMV0140_CACMV0030_dialog'),	// 配置場所
				$parentView		: this.$('#ca_CACMV0140_main'),
				isAnalyse_mode	: this.isAnalyse_mode,			// 分析ユースかどうかフラグ？？？
//				ymd				: null,			// 検索日
				select_mode		: clutil.cl_single_select,	// 複数選択モード
				anaProc			: this.anaProc
			});
			this.CACMV0140_CACMV0030Selector.render();

			// コンボボックスの中身を作成
			clutil.cltypeselector2(this.$('#ca_CACMV0140_anaattr'), this.attrlist, 0,
					1, 'attr', 'name');
...
//			url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		show: function(editList, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_CACMV0140_main'), {
			...
			if (this.select_mode == clutil.cl_multiple_select &&
				...
				this.$('#ca_CACMV0140_edit_tbl').html(html_source);
			} else {
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_CACMV0140_anaattr'));
		},
		 ...
		 */
		getAnaAttr: function() {
			var attr = this.$('#ca_CACMV0140_anaattr').val();
			var anaattr = {};
		...
		showData: function(anaattr) {
			...
			switch (anaattr.attr) {
			case amanp_AnaDefs.AMAN_DEFS_ATTR_MEMBATTR_AGE:
				// 年齢の場合は3桁
				anaattr.maxlen = 3;
				break;
			case amanp_AnaDefs.AMAN_DEFS_ATTR_MEMBATTR_R:
			case amanp_AnaDefs.AMAN_DEFS_ATTR_MEMBATTR_F:
			case amanp_AnaDefs.AMAN_DEFS_ATTR_MEMBATTR_M:
				// RFMの場合は3桁
				anaattr.maxlen = 1;
				break;
			}
			...
			case am_proto_defs.AM_PROTO_ACTYPE_STAFFCDNAME:
				template = "ca_CACMV0140_table_template";
//				template = "ca_CACMV0140_CACMV0030_template";
				break;
			case am_proto_defs.AM_PROTO_ACTYPE_ORG:
				template = "ca_CACMV0140_CACMV0030_template";
				break;
			case am_proto_defs.AM_PROTO_ACTYPE_BIRTH_MONTH:
				template = "ca_CACMV0140_birth_month_template";
				break;
			default:
				break;
			}
			this.$('#ca_CACMV0140_result').empty();
			this.$('#' + template).tmpl(anaattr).appendTo('#ca_CACMV0140_result');

			switch (Number(anaattr.actype)) {
			case am_proto_defs.AM_PROTO_ACTYPE_TEXT:
				// 全角
				this.$('#ca_CACMV0140_val').addClass("ime-active");
//				// 半角カナチェックボックス表示 2013/09/06
////				this.$('.ca_CACMV0140_check').show();
				break;
			case am_proto_defs.AM_PROTO_ACTYPE_TYPE:
			case am_proto_defs.AM_PROTO_ACTYPE_CDNAME:
			case am_proto_defs.AM_PROTO_ACTYPE_STAFFCDNAME:
				...
				case am_proto_defs.AM_PROTO_ACTYPE_TYPE:
					...
					this.$('#ca_CACMV0140_tbody_type_tmp').tmpl(this.typelist).appendTo('#ca_CACMV0140_tbody');
					break;
				case am_proto_defs.AM_PROTO_ACTYPE_CDNAME:
					this.typelist = clutil.getcdnamelist(anaattr.xtype);
					this.$('#ca_CACMV0140_tbody_cdname_tmp').tmpl(this.typelist).appendTo('#ca_CACMV0140_tbody');
					break;
				case am_proto_defs.AM_PROTO_ACTYPE_STAFFCDNAME:
				...
				var _this = this;
				// 全選択チェックボックスを初期化
				this.chkall = clutil.checkallbox(this.$('#ca_CACMV0140_chkall'),
						this.$('.ca_CACMV0140_tbl'),
						this.$('#ca_CACMV0140_tbody')
						);
				break;
				$(window).scrollTo(1);	// ダブルスクロール防止
//			case am_proto_defs.AM_PROTO_ITEM_TYPE_TEXTHAN:
//				// 半角
//				this.$('#ca_CACMV0140_text').addClass("ime-disabled");
//				this.$('#ca_CACMV0140_text').attr("data-limit", "len:" + maxlen);
//				break;
//			case am_proto_defs.AM_PROTO_ITEM_TYPE_TEXTNUM:
//				// 半角数値
//				this.$('#ca_CACMV0140_text').addClass("ime-disabled");
//				this.$('#ca_CACMV0140_text').attr("data-limit", "len:" + maxlen + " digit");
//				break;
			case am_proto_defs.AM_PROTO_ACTYPE_YMDRANGE:
				// 生年月日の場合は1900/01/02～
				if (anaattr.attr == amanp_AnaDefs.AMAN_DEFS_ATTR_MEMBATTR_BIRTHDAY) {
					clutil.datepicker(this.$("#ca_CACMV0140_val"), 19000102);
					clutil.datepicker(this.$("#ca_CACMV0140_val2"), 19000102);
				} else {
					clutil.datepicker(this.$("#ca_CACMV0140_val"));
					clutil.datepicker(this.$("#ca_CACMV0140_val2"));
				}
				break;
			case am_proto_defs.AM_PROTO_ACTYPE_BIRTH_MONTH:
				// すべて選択の初期化
				this.birth_month_chkall = clutil.checkall(
						this.$('#ca_CACMV0140_birth_month_chkall'),
						this.$('#ca_CACMV0140_birth_month_chk')
				);
				break;
			default:
				break;
			...
			// 単位の表示
			this.$('#ca_CACMV0140_item_unit').html(clutil.cStr(anaattr.item_unit));
		...
		_onShowCACMV0030Click: function(e) {
			...
			this.CACMV0140_CACMV0030Selector.show(null, true);
			//サブ画面復帰後処理
			this.CACMV0140_CACMV0030Selector.okProc = function(data) {
				if(data != null) {
					_this.$('#ca_CACMV0140_dispname').val(data[0].code + ":" + data[0].name);
					_this.$('#ca_CACMV0140_name').val(data[0].name);
					_this.$('#ca_CACMV0140_code').val(data[0].code);
					_this.$('#ca_CACMV0140_val').val(data[0].val);
				}
				// ボタンにフォーカスする
				$(e.target).focus();
			}
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_CACMV0140_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onAddClick:function() {
			...
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_CACMV0140_result'), 'ca_CACMV0140_');

			var anaattr = this.getAnaAttr();

			// 編集領域のIDリストを取得する
			var idlist = {};
			$.each(this.$('#ca_CACMV0140_edit_tbl li'), function() {
			...
			// 現在の要素数を取得
			var num = this.$('#ca_CACMV0140_edit_tbl li').length;
			...
			case am_proto_defs.AM_PROTO_ACTYPE_YMDRANGE:
				...
				case am_proto_defs.AM_PROTO_ACTYPE_NUMRANGE100:
					...
					var chkInfo = [];
					chkInfo.push({
						stval : 'ca_CACMV0140_val',
						edval : 'ca_CACMV0140_val2'
					});
				...
				this.$('#ca_CACMV0140_edit_tbl').append(this.makeEditTmp(newdata));
				break;
			case am_proto_defs.AM_PROTO_ACTYPE_ONOFF:
				if(!retStat) {
					return;
				...
				newdata.attr = anaattr.attr;
				newdata.val = searchData.val;
				newdata.name = searchData.val == '1' ? anaattr.on_name : anaattr.off_name;
				this.$('#ca_CACMV0140_edit_tbl').append(this.makeEditTmp(newdata));
				break;
...
//				break;
			...
			case am_proto_defs.AM_PROTO_ACTYPE_STAFFCDNAME:
				var chkId = [];
				$.each(this.$('#ca_CACMV0140_tbody').find("[name=ca_CACMV0140_chk]:checked"), function() {
				...
				if (chkId.length > 0) {
					...
					for (var i = 0; i < selectlist.length; i++) {
						...
						newdata.name2 = anaattr.name;
						// 種別を挿入する
						newdata.kind = amanp_AnaDefs.AMAN_DEFS_KIND_MEMBATTR;

						html_source += this.makeEditTmp(newdata, idname);
						num++;
					}
					this.$('#ca_CACMV0140_edit_tbl').append(html_source);
				} else {
					// なにもしない
				}
				// 全選択チェックボックスを初期化
		...
		makeEditTmp: function(data) {
			...
			html_source += data.name2 + '</span><span class="btn-delete ca_CACMV0140_edit_del"></span>'
		...
		_onEditDelAllClick: function() {
			...
			var idlist = [];
			var html_source = '';
			this.$('#ca_CACMV0140_edit_tbl').html("");
		...
		getEditList : function() {
			var editlist = clutil.tableview2data(this.$('#ca_CACMV0140_edit_tbl').children());
			return editlist;
		},
