$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCMV1060SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_MDCMV1060_search"		:	"_onSearchClick",		// 検索ボタン押下
			"click #searchAgain"				:	"_onSearchAgainClick",	// 検索条件を再指定ボタン押下
			"click #ca_MDCMV1060_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_MDCMV1060_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_MDCMV1060_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_MDCMV1060_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_MDCMV1060_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_MDCMV1060_main .ca_MDCMV1060_dispitem"
												:	"_onDispItemClick",		// 表示項目押下
			"click #ca_MDCMV1060_up"			:	"_onUpClick",			// ↑ボタン押下
			"click #ca_MDCMV1060_down"			:	"_onDownClick",			// ↓ボタン押下

			"click #ca_MDCMV1060_show_MDCMV1050"		:	"_onShowMDCMV1050Click",		// 商品分類選択ボタン押下

			"click #ca_MDCMV1060_main th.ca_th_sort"	:	"_onThSortClick",

			"click #ca_MDCMV1060_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV1060_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_MDCMV1060_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function() {
			...
			if (this.select_mode == clutil.cl_single_select) {
				this.$('.ca_MDCMV1060_multi').remove();
				this.$('#ca_MDCMV1060_chkall').remove();
			}
			clutil.initUIelement(this.$el);

			// 全選択チェックボックスを初期化
			this.chkall = clutil.checkallbox(this.$('#ca_MDCMV1060_chkall'),
					this.$('#ca_MDCMV1060_tbl'),
					this.$('#ca_MDCMV1060_tbody')
					);
			...
			if (this.isAnalyse_mode && this.select_mode != clutil.cl_single_select){
				...
				this.addtoSelected = clutil.addtoSelected(
						this.$('#ca_MDCMV1060_add'),
						this.$('#selected'),
						this.$('#mainColumn'));
			}

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_MDCMV1060_searchArea'),
					this.$('#ca_MDCMV1060_search'),
					this.$('#result'),
					this.$('#searchAgain'));

			// 商品分類選択画面
			this.MDCMV1060_MDCMV1050Selector = new  MDCMV1050SelectorView({
				el : this.$('#ca_MDCMV1060_MDCMV1050_dialog'),	// 配置場所
				$parentView		: this.$('#ca_MDCMV1060_main'),
				isAnalyse_mode	: this.isAnalyse_mode,			// 分析ユースかどうかフラグ？？？
//				ymd				: null,			// 検索日
				select_mode		: clutil.cl_single_select,	// 単一選択モード
				anaProc			: this.anaProc
			});
			this.MDCMV1060_MDCMV1050Selector.render();
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		initPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_MDCMV1060_pager1').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_MDCMV1060_pager_displaypanel1'),
			...
			this.$('#ca_MDCMV1060_pager2').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_MDCMV1060_pager_displaypanel2'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
					_this.onPageClick(pageNumber, itemsOnPage, true);
				},
		...
		show: function(editList, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV1060_main'), {
			...
			if (this.select_mode == clutil.cl_multiple_select &&
				...
				this.$('#ca_MDCMV1060_edit_tbody').html(html_source);
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_MDCMV1060_code'));
		},

		/**
		 * 商品分類選択ボタン押下
		 */
		_onShowMDCMV1050Click: function(e) {
			var _this = this;

			// 選択された情報を初期値として検索する
			var initData = {};
			initData.func_id = this.$('#ca_MDCMV1060_itgrpfunc_id').val();
			initData.itgrp_id = this.$('#ca_MDCMV1060_itgrp_id').val();
			...
			this.MDCMV1060_MDCMV1050Selector.show(null, true, null, null, initData);
			//サブ画面復帰後処理
			this.MDCMV1060_MDCMV1050Selector.okProc = function(data) {
				if(data != null && data.length > 0) {
					_this.$('#ca_MDCMV1060_itgrpname').val(data[0].code + ":" + data[0].name);
					_this.$('#ca_MDCMV1060_itgrp_id').val(data[0].val);
					_this.$('#ca_MDCMV1060_itgrpfunc_id').val(data[0].func_id);
					_this.$('#ca_MDCMV1060_itgrplvl_id').val(data[0].lvl_id);
				}
				// ボタンにフォーカスする
				$(e.target).focus();
			}
		...
		setSort: function(req) {
			...
			var th = this.$('#ca_MDCMV1060_tbl').find('th[key=' + sortreq.key + ']');
		...
		_onSearchClick: function() {
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_MDCMV1060_searchArea'), 'ca_MDCMV1060_');
			// 全角、半角対応
		...
		onPageClick: function(pageNumber, itemsOnPage, isPager) {
			...
			clutil.postAnaJSON(uri, this.req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (_this.resultList == null || _this.resultList.length == 0) {
						...
						case clutil.cl_multiple_select:
							$select_template = _this.$('#ca_MDCMV1060_tbody_multiple_tmp');
							break;
						case clutil.cl_single_select:
						default:
							$select_template = _this.$('#ca_MDCMV1060_tbody_single_tmp');
							break;
						}
						// 取得したデータを表示する
						$select_template.tmpl(_this.resultList).appendTo('#ca_MDCMV1060_tbody');

						clutil.initUIelement(_this.$('#ca_MDCMV1060_tbl'));
				...
				if (_this.$('#searchAgain').css('display') == 'none') {
					// 検索ボタンにフォーカスする
					_this.$('#ca_MDCMV1060_search').focus();
				} else {
					// 再検索ボタンにフォーカスする
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_MDCMV1060_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
...
//				});
		...
		_onAddClick:function() {
			...
			var chkId = [];
			$.each(this.$('#ca_MDCMV1060_tbody').find("[name=ca_MDCMV1060_chk]:checked"), function() {
			...
			if (chkId.length > 0) {
				...
				$.each(this.$('#ca_MDCMV1060_edit_tbl tr'), function() {
					idlist.push(Number(this.id));
				});
				/* liタグの場合
				$.each(this.$('#ca_MDCMV1060_edit_tbl li'), function() {
					idlist.push(Number(this.id));
				})
				*/;
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					// 種別を挿入する
					newdata.kind = amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEM;

					html_source += this.makeEditTmp(newdata);
					num++;
					editlist.push(newdata);
				}
				this.$('#ca_MDCMV1060_edit_tbody').append(html_source);
			} else {
				// なにもしない
			}
			// 全選択チェックボックスを初期化
		 ...
		 */
		_onUpClick: function() {
			var trlist = this.$('#ca_MDCMV1060_edit_tbl').find('tr.selected');
			...
			var $tr_up = null;
			var trlist_edit = this.$('#ca_MDCMV1060_edit_tbl').find('tr');
		...
		makeEditTmp: function(data) {
			var html_source = '';
			html_source += '<tr id="' + data.val + '" class="ca_MDCMV1060_dispitem deletable order">';
			html_source += '<td height="40px">';
			html_source += '<span class="data ca_MDCMV1060_dispitem_name flleft mrgb10 mrgr10" style="margin-top:3px">' + data.name + '</span>';
			html_source += '<div class="clear"></div><span class="code data ca_MDCMV1060_dispitem_name flleft">' + data.code + '</span>';
			html_source += '<div class="clear"></div><span style="margin-right:10px" class="btn-delete flright ca_MDCMV1060_edit_del mrgr10"></span></td>';
		...
		makeEditTmp_old: function(data) {
			...
			html_source += data.code + '</span><span class="btn-delete ca_MDCMV1060_edit_del"></span>';
			html_source += '<input type="hidden" name="kind" value="' + data.kind + '">';
			html_source += '<input type="hidden" name="val" value="' + data.val + '">';
			html_source += '<input type="hidden" name="code" value="' + data.code + '">';
			html_source += '<input type="hidden" name="name" value="' + data.name + '">';
		...
		_onEditDelAllClick: function() {
			this.$('#ca_MDCMV1060_edit_tbody').html("");
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_MDCMV1060_edit_tbody').children());
			_.each(editlist, _.bind(function(item, i) {
				item.seq = i+1;
			}, this));
			return editlist;
//			var editlist = [];
//			editlist = clutil.tableview2data(this.$('#ca_MDCMV1060_edit_tbl').children());
//			return editlist;
		},
		...
		_onCommitClick: function() {
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_MDCMV1060_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
