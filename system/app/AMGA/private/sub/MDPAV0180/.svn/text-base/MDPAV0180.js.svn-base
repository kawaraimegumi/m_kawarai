$(function() {
	//////////////////////////////////////////////
	// View
	MDPAV0180PanelView = Backbone.View.extend({
		id: 'ca_MDPAV0180_view',
		tagName: 'table',
		className: 'tree mrgb10',
		template: _.template(''
				+ '<tr id="ca_MDPAV0180_showbuttons">'
		...
		initUIelement: function() {
			...
			// 軸押下
			$("#ca_MDPAV0180_showbuttons").click(function(e){
				...
				if ($(span).hasClass('treeClose')) {
					...
					$('#ca_MDPAV0180_view').find('tr.ca_button').hide();
					$('#ca_MDPAV0180_view').find('tr.ca_condview').hide();
				} else {
					$(span).removeClass('treeOpen');
					$(span).addClass('treeClose');
					$('#ca_MDPAV0180_view').find('tr.ca_button').show();
					$('#ca_MDPAV0180_view').find('tr.ca_condview').show();
				}
			});
			...
			if (this.anaProc.catalog != null) {
				...
				if ((f_anacond & amcm_type.AMCM_VAL_ANACOND_AXIS) == amcm_type.AMCM_VAL_ANACOND_AXIS) {
					var th = $("#ca_MDPAV0180_view").find('th.ca_th_edit');
					$(th).removeClass('ca_th_edit');
					var span = $("#ca_MDPAV0180_view").find('span.edit');
					$(span).remove();
					var span = $("#ca_MDPAV0180_view").find('span.ca_title');
		...
		navItemClick: function(e) {
        	...
        	this.MDCMV0180Selector.show(this.getModel());

			//サブ画面復帰後処理
        	this.MDCMV0180Selector.okProc = _.bind(function(data) {
				if(data != null) {
					_this.setModel(data);
					_this.anaProc.fireAnaCondUpdated({
						id : 'ca_navItemMDCMV0180',
...
//				cond.disply = fixedDto.disply;
		...
		onCondUpdated: function(anaproc, from){
			...
			// MD商品分析 - ABC分析基準
//			if(Ana.Config.cond.MDCMV0180.isMDABCAxis && anafocus.mdabc_type){
//				var typlabel = "";
//				switch(anafocus.mdabc_type){
//				case amgbp_AnaHead.AMGBP_ANA_REQ_ABC_TYPE_QY:
//					typlabel = '売上数';
//					break;
//				case amgbp_AnaHead.AMGBP_ANA_REQ_ABC_TYPE_AM:
//					typlabel = '売上高';
//					break;
//				case amgbp_AnaHead.AMGBP_ANA_REQ_ABC_TYPE_SUPPORT_RT:
//					typlabel = '支持率(選択期間内)';
//					break;
//				case amgbp_AnaHead.AMGBP_ANA_REQ_ABC_TYPE_SUPPORT_RT_CM:
//					typlabel = '支持率(累計)';
//					break;
//				}
