$(function() {
	...
	//////////////////////////////////////////////
	// View
	CACMV0240SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_CACMV0240_search"		:	"_onSearchClick",		// 検索ボタン押下
			"click #searchAgain"				:	"_onSearchAgainClick",	// 検索条件を再指定ボタン押下
			"click #ca_CACMV0240_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_CACMV0240_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_CACMV0240_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_CACMV0240_main th.ca_th_sort"	:	"_onThSortClick",	// テーブルヘッダ押下

			"click #ca_CACMV0240_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_CACMV0240_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_CACMV0240_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_CACMV0240_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_CACMV0240_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function() {
			...
			if (this.select_mode == clutil.cl_single_select) {
				this.$('.ca_CACMV0240_multi').remove();
				this.$('#ca_CACMV0240_chkall').remove();
			}
			clutil.initUIelement(this.$el);

			// 全選択チェックボックスを初期化
			this.chkall = clutil.checkallbox(this.$('#ca_CACMV0240_chkall'),
					this.$('#ca_CACMV0240_tbl'),
					this.$('#ca_CACMV0240_tbody')
					);
			...
			if (this.isAnalyse_mode && this.select_mode != clutil.cl_single_select){
				...
				this.addtoSelected = clutil.addtoSelected(
						this.$('#ca_CACMV0240_add'),
						this.$('#selected'),
						this.$('#mainColumn'));
			}

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_CACMV0240_searchArea'),
					this.$('#ca_CACMV0240_search'),
					this.$('#result'),
					this.$('#searchAgain'));
		},
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		initPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_CACMV0240_pager1').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0240_pager_displaypanel1'),
			...
			this.$('#ca_CACMV0240_pager2').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0240_pager_displaypanel2'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
					_this.onPageClick(pageNumber, itemsOnPage, true);
				},
		...
		show: function(editList, isSubDialog) {
			...
			// タイトル
			if(window.Ana && Ana.Config && Ana.Config.cond && Ana.Config.cond.CACMV0240){
				if(!_.isEmpty(Ana.Config.cond.CACMV0240.navMenuTitle)){
					var title = Ana.Config.cond.CACMV0240.navMenuTitle;
					this.$('#title > h2').text(title);
				}
				if(Ana.Config.cond.CACMV0240.inputJanCode){
					this.$('#ca_CACMV0240_code_fieldUnit').show();
				}
				if(!_.isEmpty(Ana.Config.cond.CACMV0240.tblCodeLabel)){
					this.$('#ca_CACMV0240_tblCodeLabel').text(Ana.Config.cond.CACMV0240.tblCodeLabel);
				}
			}
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_CACMV0240_main'), {
			...
			if (this.select_mode == clutil.cl_multiple_select &&
				...
				this.$('#ca_CACMV0240_edit_tbl').html(html_source);
		...
		setFocus: function() {
			// フォーカスの設定
			var $janInput = this.$('#ca_CACMV0240_code'), $input = $janInput;
			if(!$input.is(':visible')){
				$input = this.$('#ca_CACMV0240_old_code');
			}
		...
		setSort: function(req) {
			...
			var th = this.$('#ca_CACMV0240_tbl').find('th[key=' + sortreq.key + ']');
		...
		_onSearchClick: function() {
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_CACMV0240_searchArea'), 'ca_CACMV0240_');
			// 全角、半角対応
		...
		onPageClick: function(pageNumber, itemsOnPage, isPager) {
			...
			clutil.postAnaJSON(uri, this.req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (_this.resultList == null || _this.resultList.length == 0) {
						...
						case clutil.cl_multiple_select:
							$select_template = _this.$('#ca_CACMV0240_tbody_multiple_tmp');
							break;
						case clutil.cl_single_select:
						default:
							$select_template = _this.$('#ca_CACMV0240_tbody_single_tmp');
							break;
						}
						// 取得したデータを表示する
						$select_template.tmpl(_this.resultList).appendTo('#ca_CACMV0240_tbody');

						clutil.initUIelement(_this.$('#ca_CACMV0240_tbl'));
				...
				if (_this.$('#searchAgain').css('display') == 'none') {
					// 検索ボタンにフォーカスする
					_this.$('#ca_CACMV0240_search').focus();
				} else {
					// 再検索ボタンにフォーカスする
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_CACMV0240_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onAddClick:function() {
			...
			var chkId = [];
			$.each(this.$('#ca_CACMV0240_tbody').find("[name=ca_CACMV0240_chk]:checked"), function() {
			...
			if (chkId.length > 0) {
				...
				$.each(this.$('#ca_CACMV0240_edit_tbl li'), function() {
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					// 種別を挿入する
					newdata.kind = amanp_AnaDefs.AMAN_DEFS_KIND_COLORSIZEITEM;

					html_source += this.makeEditTmp(newdata);
					num++;
					editlist.push(newdata);
				}
				this.$('#ca_CACMV0240_edit_tbl').append(html_source);
			} else {
				// なにもしない
			}
			// 全選択チェックボックスを初期化
		...
		makeEditTmp: function(data) {
			...
			html_source += data.code + '</span><span class="btn-delete ca_CACMV0240_edit_del"></span>'
		...
		_onEditDelAllClick: function() {
			...
			var idlist = [];
			var html_source = '';
			this.$('#ca_CACMV0240_edit_tbl').html("");
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_CACMV0240_edit_tbl').children());
			return editlist;
		},
		...
		_onCommitClick: function() {
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_CACMV0240_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
