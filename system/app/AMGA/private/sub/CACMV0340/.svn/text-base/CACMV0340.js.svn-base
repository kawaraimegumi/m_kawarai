$(function() {
	...
	//////////////////////////////////////////////
	// View
	CACMV0340SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_CACMV0340_search"		:	"_onSearchClick",		// 検索ボタン押下
			"click #searchAgain"				:	"_onSearchAgainClick",	// 検索条件を再指定ボタン押下
			"click #ca_CACMV0340_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_CACMV0340_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_CACMV0340_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_CACMV0340_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_CACMV0340_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_CACMV0340_show_CACMV0050"		:	"_onShowCACMV0050Click",		// 商品分類選択ボタン押下

			"click #ca_CACMV0340_main th.ca_th_sort"	:	"_onThSortClick",

			"click #ca_CACMV0340_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_CACMV0340_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_CACMV0340_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function() {
			...
			if (this.select_mode == clutil.cl_single_select) {
				this.$('.ca_CACMV0340_multi').remove();
				this.$('#ca_CACMV0340_chkall').remove();
			}
			clutil.initUIelement(this.$el);

			// 全選択チェックボックスを初期化
			this.chkall = clutil.checkallbox(this.$('#ca_CACMV0340_chkall'),
					this.$('#ca_CACMV0340_tbl'),
					this.$('#ca_CACMV0340_tbody')
					);
			...
			if (this.isAnalyse_mode && this.select_mode != clutil.cl_single_select){
				...
				this.addtoSelected = clutil.addtoSelected(
						this.$('#ca_CACMV0340_add'),
						this.$('#selected'),
						this.$('#mainColumn'));
			}

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_CACMV0340_searchArea'),
					this.$('#ca_CACMV0340_search'),
					this.$('#result'),
					this.$('#searchAgain'));

			// 商品分類選択画面
			this.CACMV0340_CACMV0050Selector = new  CACMV0050SelectorView({
				el : this.$('#ca_CACMV0340_CACMV0050_dialog'),	// 配置場所
				$parentView		: this.$('#ca_CACMV0340_main'),
...
//				ymd				: null,			// 検索日
			...
			this.CACMV0340_CACMV0050Selector.render();
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		initPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_CACMV0340_pager1').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0340_pager_displaypanel1'),
			...
			this.$('#ca_CACMV0340_pager2').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0340_pager_displaypanel2'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
					_this.onPageClick(pageNumber, itemsOnPage, true);
				},
		...
		show: function(editList, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_CACMV0340_main'), {
			...
			if (this.select_mode == clutil.cl_multiple_select &&
				...
				this.$('#ca_CACMV0340_edit_tbl').html(html_source);
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_CACMV0340_code'));
		},
		...
		_onShowCACMV0050Click: function(e) {
			...
			// 選択された情報を初期値として検索する
			var initData = {};
			initData.func_id = this.$('#ca_CACMV0340_itgrpfunc_id').val();
			initData.itgrp_id = this.$('#ca_CACMV0340_itgrp_id').val();
			...
			this.CACMV0340_CACMV0050Selector.show(null, true, null, null, initData);
			//サブ画面復帰後処理
			this.CACMV0340_CACMV0050Selector.okProc = function(data) {
				if(data != null && data.length > 0) {
					_this.$('#ca_CACMV0340_itgrpname').val(data[0].code + ":" + data[0].name);
					_this.$('#ca_CACMV0340_itgrp_id').val(data[0].val);
					_this.$('#ca_CACMV0340_itgrpfunc_id').val(data[0].func_id);
					_this.$('#ca_CACMV0340_itgrplvl_id').val(data[0].lvl_id);
				}
				// ボタンにフォーカスする
				$(e.target).focus();
			}
		...
		_onThSortClick: function(e) {
			...
			// ソートキーを挿入
			this.req['sortreq'] = {
					sort_key : aman_se_packitem_srch_req['AMAN_PROTO_SORT_KEY_' + key],
		...
		setSort: function(req) {
			...
			var th = this.$('#ca_CACMV0340_tbl').find('th[key=' + sortreq.key + ']');
		...
		_onSearchClick: function() {
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_CACMV0340_searchArea'), 'ca_CACMV0340_');
		...
		onPageClick: function(pageNumber, itemsOnPage, isPager) {
			...
			clutil.postAnaJSON(uri, this.req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (_this.resultList == null || _this.resultList.length == 0) {
						...
						case clutil.cl_multiple_select:
							$select_template = _this.$('#ca_CACMV0340_tbody_multiple_tmp');
							break;
						case clutil.cl_single_select:
						default:
							$select_template = _this.$('#ca_CACMV0340_tbody_single_tmp');
							break;
						}
						// 取得したデータを表示する
						$select_template.tmpl(_this.resultList).appendTo('#ca_CACMV0340_tbody');

						clutil.initUIelement(_this.$('#ca_CACMV0340_tbl'));
				...
				if (_this.$('#searchAgain').css('display') == 'none') {
					// 検索ボタンにフォーカスする
					_this.$('#ca_CACMV0340_search').focus();
				} else {
					// 再検索ボタンにフォーカスする
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_CACMV0340_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onAddClick:function() {
			...
			var chkId = [];
			$.each(this.$('#ca_CACMV0340_tbody').find("[name=ca_CACMV0340_chk]:checked"), function() {
			...
			if (chkId.length > 0) {
				...
				$.each(this.$('#ca_CACMV0340_edit_tbl li'), function() {
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					// 種別を挿入する
					newdata.kind = amanp_AnaDefs.AMAN_DEFS_KIND_PACKITEM;

					html_source += this.makeEditTmp(newdata);
					num++;
					editlist.push(newdata);
				}
				this.$('#ca_CACMV0340_edit_tbl').append(html_source);
			} else {
				// なにもしない
			}
			// 全選択チェックボックスを初期化
		...
		makeEditTmp: function(data) {
			...
			html_source += data.code + '</span><span class="btn-delete ca_CACMV0340_edit_del"></span>'
		...
		_onEditDelAllClick: function() {
			...
			var idlist = [];
			var html_source = '';
			this.$('#ca_CACMV0340_edit_tbl').html("");
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_CACMV0340_edit_tbl').children());
			return editlist;
		},
		...
		_onCommitClick: function() {
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_CACMV0340_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
