$(function() {
	...
	//////////////////////////////////////////////
	// View
	CACMV0030SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_CACMV0030_search"		:	"_onSearchClick",		// 検索ボタン押下
			"click #searchAgain"				:	"_onSearchAgainClick",	// 検索条件を再指定ボタン押下
			"click #ca_CACMV0030_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_CACMV0030_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_CACMV0030_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_CACMV0030_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_CACMV0030_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_CACMV0030_show_CACMV0020"		:	"_onShowCACMV0020Click",		// 組織選択ボタン押下

			"click #ca_CACMV0030_main th.ca_th_sort"	:	"_onThSortClick",				// テーブルヘッダ押下

			"click #ca_CACMV0030_main .close"	:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_CACMV0030_cancel"		:	"_onCancelClick",	// キャンセルボタン押下時
		},
		...
		initUIelement: function() {
			...
			// カレンダー
			clutil.datepicker(this.$("#ca_CACMV0030_open_iymd_from"));
			clutil.datepicker(this.$("#ca_CACMV0030_open_iymd_to"));
			clutil.datepicker(this.$("#ca_CACMV0030_close_iymd_from"));
			clutil.datepicker(this.$("#ca_CACMV0030_close_iymd_to"));
			// カレンダーの日付をクリア
			clutil.viewClear(this.$('#ca_CACMV0030_searchArea'), false);

			clutil.initUIelement(this.$el);

			// 全選択チェックボックスを初期化
			this.chkall = clutil.checkallbox(this.$('#ca_CACMV0030_chkall'),
					this.$('#ca_CACMV0030_tbl'),
					this.$('#ca_CACMV0030_tbody')
					);
			...
			if (this.isAnalyse_mode && this.select_mode != clutil.cl_single_select){
				...
				this.addtoSelected = clutil.addtoSelected(
						this.$('#ca_CACMV0030_add'),
						this.$('#selected'),
						this.$('#mainColumn'));
			}

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_CACMV0030_searchArea'),
					this.$('#ca_CACMV0030_search'),
					this.$('#result'),
					this.$('#searchAgain'));
			...
			this.CACMV0030_CACMV0020Selector = new  CACMV0020SelectorView({
				...
				$parentView		: this.$('#ca_CACMV0030_main'),
...
//				ymd				: null,			// 検索日
			...
			this.$('.expandFieldUnits').click(function(){
				...
				$("div#ca_CACMV0030_searchArea").parent().css('height', 'auto');
			});

			// 組織区分
			clutil.checkall(this.$('#ca_CACMV0030_orgkind_chkall'), this.$('#ca_CACMV0030_orgkind'));
		},
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		initPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_CACMV0030_pager1').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0030_pager_displaypanel1'),
			...
			this.$('#ca_CACMV0030_pager2').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0030_pager_displaypanel2'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
		...
		show: function(editList, isSubDialog) {
			...
			// 組織区分の選択肢を再構築
			if(true){
				var $ul = this.$('#ca_CACMV0030_orgkind');
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_CACMV0030_main'), {
			...
			if (this.select_mode == clutil.cl_multiple_select &&
				...
				this.$('#ca_CACMV0030_edit_tbl').html(html_source);
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_CACMV0030_code'));
		},
		...
		setSort: function(req) {
			...
			var th = this.$('#ca_CACMV0030_tbl').find('th[key=' + sortreq.key + ']');
		...
		_onSearchClick: function() {
			...
			var chkInfo = [];
			chkInfo.push({
				stval : 'ca_CACMV0030_open_iymd_from',
				edval : 'ca_CACMV0030_open_iymd_to'
			});
			chkInfo.push({
				stval : 'ca_CACMV0030_close_iymd_from',
				edval : 'ca_CACMV0030_close_iymd_to'
			});
			...
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_CACMV0030_searchArea'), 'ca_CACMV0030_');
			// 全角、半角対応
			...
			if(true){
				var selectedOrgIds = [];
				this.$('#ca_CACMV0030_orgkind input:checked').each(function(){
		...
		onPageClick: function(pageNumber, itemsOnPage, isPager) {
			...
			clutil.postAnaJSON(uri, this.req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (this.resultList == null || this.resultList.length == 0) {
						...
						case clutil.cl_multiple_select:
							$select_template = this.$('#ca_CACMV0030_tbody_multiple_tmp');
							break;
						case clutil.cl_single_select:
						default:
							$select_template = this.$('#ca_CACMV0030_tbody_single_tmp');
							break;
						}
						// 取得したデータを表示する
						$select_template.tmpl(this.resultList).appendTo('#ca_CACMV0030_tbody');

						clutil.initUIelement(this.$('#ca_CACMV0030_tbl'));
				...
				if (this.$('#searchAgain').css('display') == 'none') {
					// 検索ボタンにフォーカスする
					this.$('#ca_CACMV0030_search').focus();
				} else {
					// 再検索ボタンにフォーカスする
		...
		_onAddClick:function() {
			...
			var chkId = [];
			$.each(this.$('#ca_CACMV0030_tbody').find("[name=ca_CACMV0030_chk]:checked"), function() {
			...
			if (chkId.length > 0) {
				...
				$.each(this.$('#ca_CACMV0030_edit_tbl li'), function() {
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					// 種別を挿入する
					newdata.kind = amanp_AnaDefs.AMAN_DEFS_KIND_STORE;

					html_source += this.makeEditTmp(newdata);
					num++;
					editlist.push(newdata);
				}
				this.$('#ca_CACMV0030_edit_tbl').append(html_source);
			} else {
				// なにもしない
			}
			// 全選択チェックボックスを初期化
		...
		makeEditTmp: function(data) {
			...
			html_source += data.code + '</span><span class="btn-delete ca_CACMV0030_edit_del"></span>'
		...
		_onEditDelAllClick: function() {
			...
			var idlist = [];
			var html_source = '';
			this.$('#ca_CACMV0030_edit_tbl').html("");
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_CACMV0030_edit_tbl').children());
			return editlist;
		},
		...
		_onCommitClick: function() {
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_CACMV0030_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
