$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCMV1070SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"change #ca_MDCMV1070_kind"		:	"_onKindSelect",		// 種別変更
//			"keyup #ca_MDCMV1070_kind"		:	"_onKindSelect",		// 種別変更
//			"keydown #ca_MDCMV1070_kind"		:	"_onKindSelect",		// 種別変更

			"keyup .ca_MDCMV1070_srch_input"	:	"_onSrchInputKeyPress",	// テーブル上でリスト名編集確定

			"click .ca_MDCMV1070_node"			:	"_onNodeClick",			// ツリーノード押下

//			"click #ca_MDCMV1070_search"		:	"_onSearchClick",		// 検索ボタン押下
//			"click #ca_MDCMV1070_showSearchArea":	"_onShowSearchAreaClick",	// 検索条件を再指定ボタン押下
			"click #ca_MDCMV1070_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_MDCMV1070_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_MDCMV1070_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_MDCMV1070_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_MDCMV1070_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_MDCMV1070_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_MDCMV1070_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_MDCMV1070_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function() {
			...
			if (this.select_mode == clutil.cl_single_select) {
				this.$('.ca_MDCMV1070_multi').remove();
			}
			// 分析画面以外からのモード
			...
			// コンボボックスの中身を作成
			clutil.cltypeselector2(this.$('#ca_MDCMV1070_kind'), this.kindlist, 0,
					1, 'kind');
			if (this.kindlist != null && this.kindlist.length > 0) {
				this.$('#ca_MDCMV1070_kind').selectpicker('val', this.kindlist[0]);
			}
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		show: function(editList, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCMV1070_main'), {
			...
			if (this.select_mode == clutil.cl_multiple_select &&
				...
				this.$('#ca_MDCMV1070_edit_tbl').html(html_source);
			} else {
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_MDCMV1070_kind'));
		},
		...
		_onNodeClick: function(e) {
			...
			for (var i = this.level; i > level; i--) {
				// 内部レベルより大きいボックスは消去する
				this.$('#ca_MDCMV1070_box_' + i).closest('div.drilldownItem').remove();
			...
			if ($(table).find($('tr.selected')).length > 0) {
				...
				this.$('#ca_MDCMV1070_box_' + select_level).addClass('final');
			}
		...
		onSearchClick: function(parent_id, lvl_no) {
			...
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_MDCMV1070_searchArea'), 'ca_MDCMV1070_');

			if (searchData.kind == amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMATTR_SUBCLASS1
					|| searchData.kind == amgbp_AnaDefs.AMGBA_DEFS_KIND_ITEMATTR_SUBCLASS2) {
				// サブクラス1,2の場合は品種を取得する
				var ctg_list = _.filter(this.anaProc.cond.focuslist, function(o) {
					return o.kind == amgbp_AnaDefs.AMGBA_DEFS_KIND_ITGRP;
				});
			...
			if(Ana.Config.cond.MDCMV1070 && !_.isEmpty(Ana.Config.cond.MDCMV1070.itemattr_srch_resId)){
				uri = Ana.Config.cond.MDCMV1070.itemattr_srch_resId;
			}
				...
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (resultList == null || resultList.length == 0) {
						...
						var lvl_obj = {
								level : _this.level};
						// 取得したデータを表示する
						_this.$('#ca_MDCMV1070_box_tmp').tmpl(lvl_obj).appendTo('#ca_MDCMV1070_itemattrtree');
						_this.$('#ca_MDCMV1070_node_tmp').tmpl(resultList).appendTo('#ca_MDCMV1070_box_' + _this.level);
					}
					// スクロール
			        $('#ca_MDCMV1070_itemattrtree').scrollTo('#ca_MDCMV1070_box_' + _this.level);
					clutil.initUIelement(_this.$el);
//					$('#ca_MDCMV1070_box_' + _this.level).closest('.yScroll').slimScroll({
//						color: '#ccc',
//					});
				} else {
					// ヘッダーにメッセージを表示
		...
		clearResult: function() {
			// 結果表示エリアをクリア
			this.$('#ca_MDCMV1070_itemattrtree').empty();
			// validatorの初期化
			this.validator.clear();
			// 内部用レベルを初期化
			this.level = 0;
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_MDCMV1070_searchArea'));
			// validatorの初期化
			this.validator.clear();
		},
		...
		_onAddClick:function(e) {
			...
			var chkId = [];
			var _this = this;
			$.each(this.$("#ca_MDCMV1070_itemattrtree table.final tr.selected"), function() {
				chkId.push(this.id);
			});
			...
			if (chkId.length > 0) {
				...
				var idlist = [];
				var html_source = '';
				var kind = this.$('#ca_MDCMV1070_kind').val();

				$.each(this.$('#ca_MDCMV1070_edit_tbl li'), function() {
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					newdata.name2 = this.$('#ca_MDCMV1070_kind').find('option[value=' +
							this.$('#ca_MDCMV1070_kind').val() + ']').html();
				...
				this.$('#ca_MDCMV1070_edit_tbl').append(html_source);
			} else {
				// なにもしない
			}
			// アニメーション表示のための初期化
		...
		makeEditTmp: function(data) {
			...
			html_source += data.name2 + '</span><span class="btn-delete ca_MDCMV1070_edit_del"></span>'
		...
		_onEditDelAllClick: function() {
			...
			var idlist = [];
			var html_source = '';
			this.$('#ca_MDCMV1070_edit_tbl').html("");
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_MDCMV1070_edit_tbl').children());
			return editlist;
		},
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					for (var i = 0; i < selectlist.length; i++) {
						...
						// 種別を挿入する
						newdata.kind = this.$('#ca_MDCMV1070_kind').val();
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_MDCMV1070_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
