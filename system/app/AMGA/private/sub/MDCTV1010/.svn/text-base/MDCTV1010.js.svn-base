$(function() {
	...
	//////////////////////////////////////////////
	// View
	MDCTV1010SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"change #ca_MDCTV1010_ymd"				:	"_onYmdSelect",				// 日付単位変更

			"click #ca_MDCTV1010_CACTV0040_show"	:	"_onCACTV0040ShowClick",	// メニュー登録場所ボタン押下
			"click #ca_MDCTV1010_menutree_clear"	:	"_onMenuTreeClearClick",	// メニュー登録場所クリアボタン押下

			"click #ca_MDCTV1010_commit"			:	"_onCommitClick",			// 登録ボタン押下
			"click #ca_MDCTV1010_cancel"			:	"_onCancelClick"			// キャンセルボタン押下
		},
		...
		initUIelement: function() {
			...
			clutil.cltypeselector(this.$('#ca_MDCTV1010_f_open'), amcm_type.AMCM_TYPE_OPEN);

			// 分析メニュー選択画面
			this.MDCTV1010_CACTV0040Selector = new  CACTV0040SelectorView({
				el : this.$('#ca_MDCTV1010_CACTV0040_dialog')	// 配置場所
			});
			this.MDCTV1010_CACTV0040Selector.render(
					this.$('#ca_MDCTV1010_main'),
					true				// 分析モード
			);
...
//				this.$('.ca_MDCTV1010_period_area').remove();
//				clutil.initUIelement(this.$el);
//				return;
//			}

			// カレンダー
			clutil.datepicker(this.$("#ca_MDCTV1010_day_temp").find(".ca_MDCTV1010_opt"));
			// 週単位
			clutil.clweekselector(
					this.$("#ca_MDCTV1010_week_temp").find("#ca_MDCTV1010_week_y_period_val1"),
					this.$("#ca_MDCTV1010_week_temp").find("#ca_MDCTV1010_week_w_period_val1_div"),
					{id : "ca_MDCTV1010_week_w_period_val1"}, "flleft ca_MDCTV1010_opt wt180 mrgl10 cl_valid",
					this.period_list);
			clutil.clweekselector(
					this.$("#ca_MDCTV1010_week_temp").find("#ca_MDCTV1010_week_y_period_val2"),
					this.$("#ca_MDCTV1010_week_temp").find("#ca_MDCTV1010_week_w_period_val2_div"),
					{id : "ca_MDCTV1010_week_w_period_val2"}, "flleft ca_MDCTV1010_opt wt180 mrgl10 cl_valid",
					this.period_list);
			// 月単位
			clutil.clyearmonthselector(
					this.$("#ca_MDCTV1010_month_temp").find("#ca_MDCTV1010_month_y_period_val1"),
					this.$("#ca_MDCTV1010_month_temp").find("#ca_MDCTV1010_month_m_period_val1"),
					0, this.past_years, this.future_years); //10, 0);
			clutil.clyearmonthselector(
					this.$("#ca_MDCTV1010_month_temp").find("#ca_MDCTV1010_month_y_period_val2"),
					this.$("#ca_MDCTV1010_month_temp").find("#ca_MDCTV1010_month_m_period_val2"),
					0, this.past_years, this.future_years); //10, 0);

			// ラジオボタンクリック
			this.$el.delegate(':radio[name=ca_MDCTV1010_catalog_period]', 'toggle', function (ev) {
			...
			// リストに登録画面
			this.MDCTV1010_CACMV0300Selector = new CACMV0300SelectorView({
				el : this.$('#ca_MDCTV1010_CACMV0300_dialog'),	// 配置場所
				$parentView	: this.$('#ca_MDCTV1010_main'),
				isAnalyse_mode	: true		// 分析ユースかどうかフラグ？？？
			});
			this.MDCTV1010_CACMV0300Selector.render();
		},
...
//			url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		show: function(isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_MDCTV1010_base_form'), {
				echoback		: $('.cl_echoback')
			});
			// validatorエラー時の表示領域
			this.type_before_validator = clutil.validator(this.$('.ca_MDCTV1010_type_before_div'), {
				echoback		: $('.cl_echoback')
			});
			// validatorエラー時の表示領域
			this.unit_ymd_validator = clutil.validator(this.$('.ca_MDCTV1010_unit_ymd_div'), {
				echoback		: $('.cl_echoback')
			});
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_MDCTV1010_name'));
		},
		...
		_onYmdSelect : function(e) {
			// クラスに合わせて表示する内容を変更
			var ymd = this.$('#ca_MDCTV1010_ymd option:selected').attr('class')
			var unit = this.$('#ca_MDCTV1010_ymd option:selected').attr('unit');

			this.$('div.ca_MDCTV1010_unit_ymd_div').addClass('dispn');
			this.$('#' + ymd + '_temp').removeClass('dispn');
			// 単位表示
			this.$('.ca_MDCTV1010_unit').html(unit);
		},
		...
		onRadioClick: function(type) {
			clutil.viewRemoveReadonly(this.$('.ca_MDCTV1010_unit_ymd_div'));
			clutil.viewRemoveReadonly(this.$('.ca_MDCTV1010_type_before_div'));
			if (type == CATALOG_PERIOD_BEFORE) {
				// 相対指定
				clutil.viewReadonly(this.$('.ca_MDCTV1010_unit_ymd_div'));
			} else {
				// 絶対指定
				clutil.viewReadonly(this.$('.ca_MDCTV1010_type_before_div'));
			}
		},
		...
		_onCACTV0040ShowClick: function(e) {
			...
			this.MDCTV1010_CACTV0040Selector.show(true);
			//サブ画面復帰後処理
			this.MDCTV1010_CACTV0040Selector.okProc = function(data) {
				if(data != null) {
					_this.$('#ca_MDCTV1010_menutree').val(data.node_name);
					_this.$('#ca_MDCTV1010_anamenunode_id').val(data.node_id);
				}
				// ボタンにフォーカスする
				$(e.target).focus();
				// 画面位置修正
		...
		_onMenuTreeClearClick: function() {
			$('#ca_MDCTV1010_anamenunode_id').val("");
			$('#ca_MDCTV1010_menutree').val("");
		},

		// 期間データを取得
		getPeriodData : function(period_val, resultData){
			// 日付単位
			resultData.f_period_unit = this.$('#ca_MDCTV1010_ymd').val();
			// 絶対指定か相対指定か
			resultData.f_period_type = this.$(':radio[name=ca_MDCTV1010_catalog_period]:checked').val();
		...
		_onCommitClick: function(e) {
			...
			if (Number(this.$(':radio[name=ca_MDCTV1010_catalog_period]:checked').val()) ==
				CATALOG_PERIOD_BEFORE) {
				// 相対指定
				period_val = clutil.view2data($('.ca_MDCTV1010_type_before_div'), 'ca_MDCTV1010_');
				...
				var chkInfo = [];
				chkInfo.push({
					stval : 'ca_MDCTV1010_period_val2',
					edval : 'ca_MDCTV1010_period_val1'
				});
				...
				// 絶対指定
				var ymd = this.$('#ca_MDCTV1010_ymd option:selected').attr('class');
				var type = this.$('#ca_MDCTV1010_ymd').val();
				this.$('#' + ymd + '_temp').removeClass('dispn');
				...
				clutil.postAnaJSON(uri, req, _.bind(function(data, dataType) {
					...
					if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
						...
						if (appdata != null &&
							...
							// 仮会員リスト登録ダイアログを表示
							clutil.MessageDialog(clmsg.ca_MDCTV1010_0001, _this.genlistUpdOkcallback, anafocus);
		...
		genlistUpdOkcallback: function(anafocus) {
			...
			// リスト登録画面を表示
			this.MDCTV1010_CACMV0300Selector.show(true, req, uri);
    		// サブ画面復帰後処理
           	this.MDCTV1010_CACMV0300Selector.okProc = function(data) {
		...
		updGenlistOkcallback: function(obj) {
			// 画面の情報を取得する
			var resultdata = clutil.view2data($('#ca_MDCTV1010_base_form'), 'ca_MDCTV1010_');
