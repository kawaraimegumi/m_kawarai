$(function() {
	...
	//////////////////////////////////////////////
	// View
	CACMV0150SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_CACMV0150_search"		:	"_onSearchClick",		// 検索ボタン押下
			"click #searchAgain"				:	"_onSearchAgainClick",	// 検索条件を再指定ボタン押下
			"click #ca_CACMV0150_add"			:	"_onAddClick",			// 追加ボタン押下

			"click .ca_CACMV0150_edit_del"		:	"_onEditDelClick",		// 単一削除ボタン押下
			"click #ca_CACMV0150_edit_delall"	:	"_onEditDelAllClick",	// すべて削除ボタン押下

			"click #ca_CACMV0150_show_CACMV0140":	"_onShowCACMV0140Click",// 会員属性選択ボタン押下

			"click #ca_CACMV0150_main th.ca_th_sort"	:	"_onThSortClick",				// テーブルヘッダ押下

			"click #ca_CACMV0150_clear"			:	"_onClearClick",		// クリアボタン押下
			"click #ca_CACMV0150_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_CACMV0150_main .close"		:	"_onCancelClick",	// 閉じるボタン押下時
			"click #ca_CACMV0150_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
			"click #ca_CACMV0150_main .modalBK"		:	"_onCancelClick"	// 枠外押下時
		},
		...
		initUIelement: function() {
			...
			if (this.select_mode == clutil.cl_single_select) {
				this.$('.ca_CACMV0150_multi').remove();
				this.$('#ca_CACMV0150_chkall').remove();
			}
			...
			// 全選択チェックボックスを初期化
			this.chkall = clutil.checkallbox(this.$('#ca_CACMV0150_chkall'),
					this.$('#ca_CACMV0150_tbl'),
					this.$('#ca_CACMV0150_tbody')
					);
			...
			if (this.isAnalyse_mode && this.select_mode != clutil.cl_single_select){
				...
				this.addtoSelected = clutil.addtoSelected(
						this.$('#ca_CACMV0150_add'),
						this.$('#selected'),
						this.$('#mainColumn'));
			}

			// 検索条件を再指定ボタンを隠す
			this.srchArea = clutil.controlSrchArea(this.$('#ca_CACMV0150_searchArea'),
					this.$('#ca_CACMV0150_search'),
					this.$('#result'),
					this.$('#searchAgain'));

			// 会員属性選択画面
			this.CACMV0150_CACMV0140Selector = new  CACMV0140SelectorView({
				el : this.$('#ca_CACMV0150_CACMV0140_dialog'),	// 配置場所
				$parentView		: this.$('#ca_CACMV0150_main'),
				isAnalyse_mode	: this.isAnalyse_mode,			// 分析ユースかどうかフラグ？？？
//				ymd				: null,			// 検索日
				select_mode		: clutil.cl_multiple_select,	// 複数選択モード
				anaProc			: this.anaProc
			});
			this.CACMV0150_CACMV0140Selector.render();
...
//			url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		initPager: function(pageNumber, itemsOnPage, totalCount) {
			var _this = this;
			this.$('#ca_CACMV0150_pager1').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0150_pager_displaypanel1'),
			...
			this.$('#ca_CACMV0150_pager2').pagination({
				items : totalCount,
				itemsOnPage : itemsOnPage,
				currentPage: pageNumber,
				displaypanel : this.$('#ca_CACMV0150_pager_displaypanel2'),
				onPageClick: function(pageNumber, itemsOnPage) {
					// ページ変更時
					_this.onPageClick(pageNumber, itemsOnPage, true);
				},
		...
		show: function(editList, isSubDialog) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_CACMV0150_main'), {
			...
			if (this.select_mode == clutil.cl_multiple_select &&
				...
				this.$('#ca_CACMV0150_edit_tbl').html(html_source);
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_CACMV0150_code'));
		},

		/**
		 * 会員属性選択ボタン押下
		 */
		_onShowCACMV0140Click: function(e) {
			var _this = this;

			this.CACMV0150_CACMV0140Selector.show(null, true);
			//サブ画面復帰後処理
			this.CACMV0150_CACMV0140Selector.okProc = function(data) {
				if(data != null && data.length > 0) {
					var attrname = "";
					for (var i = 0; i < data.length; i++) {
						attrname = attrname + data[i].name + ':' + data[i].name2;
						if (i != data.length-1) {
							attrname = attrname + ', ';
						}
					}
					_this.$('#ca_CACMV0150_attrname').val(attrname);
					_this.membattr = data;
				}
				// ボタンにフォーカスする
				$(e.target).focus();
		...
		_onThSortClick: function(e) {
			...
			// ソートキーを挿入
			this.req['sortreq'] = {
					sort_key : gsan_se_memb_srch_if['GSAN_PROTO_SORT_KEY_' + key],
		...
		setSort: function(req) {
			...
			var th = this.$('#ca_CACMV0150_tbl').find('th[key=' + sortreq.key + ']');
		...
		_onSearchClick: function() {
			// 画面の情報を取得する
			var searchData = clutil.view2data(this.$('#ca_CACMV0150_searchArea'), 'ca_CACMV0150_');
			searchData['membattr'] = this.membattr;
		...
		onPageClick: function(pageNumber, itemsOnPage, isPager) {
			...
			clutil.postAnaJSON(uri, this.req, _.bind(function(data, dataType) {
				if (data.head.status == am_proto_defs.AM_PROTO_COMMON_RSP_STATUS_OK) {
					...
					if (_this.resultList == null || _this.resultList.length == 0) {
						...
						case clutil.cl_multiple_select:
							$select_template = _this.$('#ca_CACMV0150_tbody_multiple_tmp');
							break;
						case clutil.cl_single_select:
						default:
							$select_template = _this.$('#ca_CACMV0150_tbody_single_tmp');
							break;
						}
						// 取得したデータを表示する
						$select_template.tmpl(_this.resultList).appendTo('#ca_CACMV0150_tbody');

						clutil.initUIelement(_this.$('#ca_CACMV0150_tbl'));
				...
				if (_this.$('#searchAgain').css('display') == 'none') {
					// 検索ボタンにフォーカスする
					_this.$('#ca_CACMV0150_search').focus();
				} else {
					// 再検索ボタンにフォーカスする
		...
		_onClearClick: function() {
			clutil.viewClear(this.$('#ca_CACMV0150_searchArea'));
		...
		_onAddClick:function() {
			...
			var chkId = [];
			$.each(this.$('#ca_CACMV0150_tbody').find("[name=ca_CACMV0150_chk]:checked"), function() {
			...
			if (chkId.length > 0) {
				...
				$.each(this.$('#ca_CACMV0150_edit_tbl li'), function() {
				...
				for (var i = 0; i < selectlist.length; i++) {
					...
					// 重複チェック
					if(idlist.indexOf(select.memb_id) != -1){
						// 重複する場合は追加しない
						continue;
					}
					// 重複していない場合は追加
					var newdata = {};
					newdata.val = select.memb_id;
				...
				this.$('#ca_CACMV0150_edit_tbl').append(html_source);
			} else {
				// なにもしない
			}
			// 全選択チェックボックスを初期化
		...
		makeEditTmp: function(data) {
			...
			html_source += data.code + '</span><span class="btn-delete ca_CACMV0150_edit_del"></span>'
		...
		_onEditDelAllClick: function() {
			...
			var idlist = [];
			var html_source = '';
			this.$('#ca_CACMV0150_edit_tbl').html("");
		...
		getEditList : function() {
			var editlist = [];
			editlist = clutil.tableview2data(this.$('#ca_CACMV0150_edit_tbl').children());
			return editlist;
		},
		...
		_onCommitClick: function() {
			...
			if (this.select_mode == clutil.cl_single_select ||
				...
				if (chkId.length > 0) {
					...
					// ヘッダーにメッセージを表示
					this.validator.setErrorInfo({_eb_: clmsg.ca_CACMV0150_0001});
				}
			} else {
				// 編集リストを取得
				this.$parentView.show();
