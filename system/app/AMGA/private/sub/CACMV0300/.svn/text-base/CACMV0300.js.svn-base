$(function() {
	...
	//////////////////////////////////////////////
	// View
	CACMV0300SelectorView = Backbone.View.extend({
		...
		// 押下イベント
		events: {
			"click #ca_CACMV0300_commit"		:	"_onCommitClick",		// 確定ボタン押下

			"click #ca_CACMV0300_cancel"			:	"_onCancelClick",	// キャンセルボタン押下時
		},
		...
		initUIelement: function() {
			...
			if (this.isAnalyse_mode == false) {
				this.$('.ca_CACMV0300_multi').remove();
			...
			// 公開区分
			clutil.initcltypeselector($('#ca_CACMV0300_f_open_div'), gsdb_defs.MTTYPETYPE_F_OPEN,
					0, null,
					{id : "ca_CACMV0300_f_open", name : "info"}, "mbn wt280 flleft");
			// リスト利用区分
			clutil.initcltypeselector($('#ca_CACMV0300_f_listuse_div'), gsdb_defs.MTTYPETYPE_F_LISTUSE, 0, null,
					{id : "ca_CACMV0300_f_listuse", name : "info"}, "mbn wt280 flleft");
...
//			var url = clcom.urlRoot + "/system/app/" + this.screenId + "/" + this.screenId + ".html";
		...
		show: function(isSubDialog, req, uri) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_CACMV0300_main'), {
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_CACMV0300_listname'));
		},
		...
		updOkcallback: function(e) {
			...
			var inputData = clutil.view2data(this.$('#ca_CACMV0300_searchArea'), 'ca_CACMV0300_');
