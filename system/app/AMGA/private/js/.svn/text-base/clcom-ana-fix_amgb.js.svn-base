...
//			store.remove(storedKey);
	...
	_.extend(clutil, {
		...
		getAnaIniJSON: function(main_menu_id){
			...
			// 5. 分析初期情報を取得 - aman_ap_gbaproc_preinit
			// 会員属性、売上属性を上書きする
			var d = clutil.postAnaJSON('aman_ap_gbaproc_preinit', {
				cond: {
					main_menu_id: main_menu_id
				}
			}).done(function(data){
				_.each({
					memb_attr_list: data.memb_attr_list,
					tran_attr_list: data.tran_attr_list,
					mstitem_list_gba: data.mstitem_list
				}, function(anaprocVal, key){
					var storedKey = clcom.anaProcStoredKeys[key];
					if(_.isEmpty(storedKey)){
						return;
					}
					window.store.set(storedKey, anaprocVal);
				});
			});
			dd.push(d);
