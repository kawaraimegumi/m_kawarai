...
//						itgrp: true,
		...
		Const: {
			...
			// ---------------------------------------------------------------
			// amanp_AnaSortKey ソート順定義
			sortOrder: {
				'asc': {
					name: '昇順',
					className: 'sortAsc',
					value: amanp_AnaSortKey.AMANP_ANA_SORTKEY_ORDER_ASCENDING
				},
				'dsc': {
					name: '降順',
					className: 'sortDsc',
					value: amanp_AnaSortKey.AMANP_ANA_SORTKEY_ORDER_DESCENDING
				}
			},

			// --- amanp_AnaDefs#define --------------------------------------
			// 階層構造になれる軸種別
			HierarchicalAxisKindMap: function(){
				var map = new Object();
				map[amanp_AnaDefs.AMAN_DEFS_KIND_ORG]				 = '組織';			// 101
				map[amanp_AnaDefs.AMAN_DEFS_KIND_ITGRP]				 = '商品分類';		// 201
				// 商品属性系はカット #20150104 OTK
				//map[amanp_AnaDefs.AMAN_DEFS_KIND_ITEMATTR_SUBCLASS1] = 'サブクラス１';	// 211
				//map[amanp_AnaDefs.AMAN_DEFS_KIND_ITEMATTR_SUBCLASS2] = 'サブクラス２';	// 212
				//map[amanp_AnaDefs.AMAN_DEFS_KIND_ITEMATTR_BRAND]	 = 'ブランド';		// 213
				//map[amanp_AnaDefs.AMAN_DEFS_KIND_ITEMATTR_STYLE]	 = 'スタイル';		// 214
				//map[amanp_AnaDefs.AMAN_DEFS_KIND_ITEMATTR_DESIGN]	 = 'デザイン';		// 215
				//map[amanp_AnaDefs.AMAN_DEFS_KIND_ITEMATTR_MATERIAL]	 = '素材';			// 216
				//map[amanp_AnaDefs.AMAN_DEFS_KIND_ITEMATTR_COLOR]	 = 'カラー';		// 217
				//map[amanp_AnaDefs.AMAN_DEFS_KIND_ITEMATTR_T_COLOR]	 = '色系';			// 218
				//map[amanp_AnaDefs.AMAN_DEFS_KIND_ITEMATTR_K_SIZE]	 = '体型・サイズ';	// 219
				//map[amanp_AnaDefs.AMAN_DEFS_KIND_ITEMATTR_SEASON]	 = 'シーズン';		// 220
				//map[amanp_AnaDefs.AMAN_DEFS_KIND_ITEMATTR_USE]		 = '用途区分';		// 321
				//map[amanp_AnaDefs.AMAN_DEFS_KIND_MAKER]				 = 'メーカー';		// 301
				// シーズンは復活させる #20150324 OTK
				map[amanp_AnaDefs.AMAN_DEFS_KIND_ITEMATTR_SEASON]	 = 'シーズン';		// 220
				map[amanp_AnaDefs.AMAN_DEFS_KIND_ADDR]				 = '住所';			// 804
				map[amanp_AnaDefs.AMAN_DEFS_KIND_YMD]				 = '日付';
				return map;
			}(),
			_eof: 'Const//'
		},
		...
		Util: {
			...
			findPeriod: function(unit, yyyymmdd){
				var map = null;
				switch(unit){
				case amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_YW:	// [ 9] 年周: YYYYWW
					map = this.WeekOfYearMap;
					break;
				case amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_Y2:	// [10] 半期: YYYYNN
					map = this.HalfOfYearMap;
					break;
				case amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_Y4:	// [11] 四半期 YYYYNN
					map = this.QuarterOfYearMap;
					break;
				default:
				case amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_YM:	// [ 2] 年月：YYYYMM
				case amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_YMD:	// [ 3] 年月日: YYYYMMDD
					return null;
				}
				// 期間は降順ソートされている。
			...
			cellToString: function(cell, disp_amunit/*円,千円,万円*/){
				...
				var val = cell.value;
				switch(cell.kind){
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_UNKNOWN:
					text = '-';
					break;
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_STRING:
					text = cell.str;
					break;
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_AMOUNT:		// 金額（１倍値）
					// 円単位/千円単位/万円単位対応
					switch(disp_amunit){
					case amanp_AnaHead.AMANP_ANA_REQ_DISP_AMUNIT_1000:
						val = (val / 1000).toFixed(0);
						break;
					case amanp_AnaHead.AMANP_ANA_REQ_DISP_AMUNIT_10000:
						val = (val / 10000).toFixed(0);
						break;
					}
					// fall through
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_DECIMAL:		// 整数
					text = val.toString();
					text = this.numberFormat(text);
					break;
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_AMREAL:
					// 円単位/千円単位/万円単位対応
					switch(disp_amunit){
					case amanp_AnaHead.AMANP_ANA_REQ_DISP_AMUNIT_1000:
						val /= 1000;
					...
					// fall through
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_RATIO:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_TIME:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_REAL:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_REAL0:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_REAL2:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_QYREAL:
					...
					break;
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_DOUBLE:		// 実数
					text = cell.dvalue.toFixed(cell.decpoint);
					text = this.numberFormat(text);
					break;
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_IYMD:		// yyyymmdd --> 'yyyy/mm/dd'
					text = (val / 10000).toFixed(0)
						+ '/' + (val % 10000 / 100).toFixed(0)
						+ '/' + (val % 100).toFixed(0);
					break;
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_YMD:			// 内部日付
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_WEATHER:		// ???
					text = val.toString();
					break;
				}
				return text;
			...
			cellTextAlignment: function(cell){
				var alignment;
				switch(cell.kind){
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_DECIMAL:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_AMOUNT:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_DOUBLE:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_RATIO:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_TIME:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_REAL:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_REAL0:
				...
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_AMREAL:
					...
					break;
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_YMD:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_IYMD:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_WEATHER:
					alignment = 'center';
					break;
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_UNKNOWN:
    		 ...
    		 */
			 cellColor: function(cell, colorList){
				...
				var denom;             // 分母
				switch(cell.kind){
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_DECIMAL:   // 1:整数
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_AMOUNT:    // 2:金額1倍値
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_DOUBLE:    // 4:実数
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_AMREAL:    // 13:金額実数
				// case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_ABC:       // 15:ABC
				  denom = 1;
				  break;
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_RATIO:     // 5:比率
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_REAL:      // 8:実数
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_QYREAL:    // 12:数量実数
				  denom = cell.denominator;
				  break;
				// case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_DECIMAL100:        // 20:整数100倍
				// case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_AMOUNT100:         // 21:金額100倍
				//   denom = 100;
				//   break;
				// case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_AMOUNT10000:       // 22:金額10000倍
				//   denom = 10000;
				//   break;
				default:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_UNKNOWN:   // 0:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_STRING:    // 3:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_WEATHER:   // 6:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_TIME:      // 7:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_YMD:       // 10:
				case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_IYMD:      // 11:
				// case amanp_AnaCellData.AMANP_ANA_CELLDATA_KIND_YM:        // 14:
				  return null;
				}
			...
			axisNodeAndDataToString: function(axNode){
				...
				while(!node.isRoot()){
					var dt = node.data;
					if(_.isNullOrUndefined(dt) || dt.type !== amanp_AnaAxisElem.AMANP_ANA_AXISELEM_TYPE_DATA){
						break;
					}
			...
			_initialize: function(){
				...
				if(_.isArray(periodlist)){
					for(var i = 0; i < periodlist.length; i++) {
						...
						var dstmap = null;
						switch(p.type){
						case amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_Y4:	// 四半期: key = {201301, 201302}
							dstmap = this.QuarterOfYearMap;
							break;
						case amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_Y2:	// 半期: key = { 201301, 201302, 201303, 201304 }
							dstmap = this.HalfOfYearMap;
							break;
						case amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_YW:	// 年週: key = { 201301, 201302, ... 201353 }
			...
			/**
			 * 複合表示項目idから、AMAN_DI_I_MASK, AMAN_DI_G_MASK 成分でハッシュキーを生成する。
			 */
			dispItemIGKey: function(dispitem_id){
				var grpId  = dispitem_id & amanp_AnaDispItemDefs.AMAN_DI_G_MASK;
				var itemId = dispitem_id & amanp_AnaDispItemDefs.AMAN_DI_I_MASK;
	...
	_.extend(Ana.Util.TreeNode.prototype, /*Backbone.Events,*/ {
		...
		isAxLeaf: function(){
			...
			if(dt && dt.type) {
				switch(dt.type) {
				case amanp_AnaAxisElem.AMANP_ANA_AXISELEM_TYPE_TOTAL:
				case amanp_AnaAxisElem.AMANP_ANA_AXISELEM_TYPE_SUBTOTAL:
				case amanp_AnaAxisElem.AMANP_ANA_AXISELEM_TYPE_NORMAL:
					// 軸種別が合計、小計、ノーマルは Leaf に入れる。
					return true;
				case amanp_AnaAxisElem.AMANP_ANA_AXISELEM_TYPE_DATA:
					return false;
				default:
				}
			}
			...
			cond: {
				...
				anaPeriods: function(){
					...
					return [
						{	// [0] 対象期間
							q_type: 0,	// 指定なし
							type:   amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_YM,	// 年月指定
							p_from: yyyymm0,
							p_to:   yyyymm0,
				...
				vAxisList: [{kind:amanp_AnaDefs.AMAN_DEFS_KIND_YMD, attr:3,
									name2 : '日付', name : '日単位'}],	// 日付軸、日単位	// shimizu 追加
				hAxisList: [],
				vfzerosuppress: 0,			// 縦軸
				hfzerosuppress: 0,			// 横軸
				disp_ly: amanp_AnaHead.AMANP_ANA_REQ_DISP_LY_SAMEWEEK,		// 昨年：昨年同日、昨年同曜
				fromtoList: {},				// 閾値条件

				//C) 表示項目					AnaDispItemFolder
				dispitemlist: [
					{dispitem_id: 1118209, name: '購入点数',       name2: '購入点数'},
					{dispitem_id: 1118211, name: '購入金額(税抜)', name2: '購入金額(税抜)'}
				],		// 表示項目
				dispcolorlist: [],		// 色分け条件
				...
				focuslist: [
					{kind : amanp_AnaDefs.AMAN_DEFS_KIND_HOLIDAY, val : 1},	// 祝日：含む
					{kind : amanp_AnaDefs.AMAN_DEFS_KIND_STORE_SORT, val : amcm_type.AMCM_VAL_ANA_STORE_SORT_CODE},		// 店舗並び順: コード
					{kind : amanp_AnaDefs.AMAN_DEFS_KIND_NEWSTORE_LOGIC, val : amcm_type.AMCM_VAL_ANA_NEWSTORE_LOGIC_BAISOKU},	// 新店ロジック：売速
					{kind : amanp_AnaDefs.AMAN_DEFS_KIND_NEWSTORE_COND, val : amcm_type.AMCM_VAL_ANA_NEWSTORE_COND_NOP},		// 新店既存店: 絞込なし
					{kind : amanp_AnaDefs.AMAN_DEFS_KIND_STOREATTR_SMX, val : amcm_type.AMCM_VAL_ANA_STOREATTR_SMX_NOP},		// SMX店舗：SMX店舗絞込なし
					{kind : amanp_AnaDefs.AMAN_DEFS_KIND_STOREATTR_CLOSED, val : amcm_type.AMCM_VAL_ANA_STOREATTR_CLOSED_EXCLUDE},		// 閉店店舗：閉店店舗を含めない
					{kind : amanp_AnaDefs.AMAN_DEFS_KIND_ITEMATTR_SMXKT, val : amcm_type.AMCM_VAL_ANA_ITEMATTR_SMXKT_NOP}		// SMX・KT商品：SMX・KT商品絞込なし
				],					// 対象
				focuslist2: [],		// 比較
				...
				mstsrchdate: function(){
					...
					return {
						f_srchiymd_type: amanp_AnaHead.AMANP_ANA_REQ_F_SRCHIYMD_TYPE_COM,	// 検索日タイプ
				...
				//Z) その他の付加情報
				extra: {
					membcond: amanp_AnaHead.AMANP_ANA_REQ_MEMBCOND_ALL,	// 顧客条件
	...
	_.extend(Ana.Proc.prototype, Backbone.Events, {
		...
		getMstSrchDate: function(s_type) {
			...
			if (mstsrchdate.f_srchiymd_type == amanp_AnaHead.AMANP_ANA_REQ_F_SRCHIYMD_TYPE_COM) {
				// 共通指定
				...
				return clutil.postAnaJSON(uri, req).then(_.bind(function(data){
					...
					this.drilldownMaxlvMap = _.reduce(data.ana_cond_item_list, function(map, elem){
						if(elem.f_axis && Ana.Const.HierarchicalAxisKindMap[elem.cond_kind]){
							...
							if( elem.cond_kind == amanp_AnaDefs.AMAN_DEFS_KIND_YMD &&
								elem.cond_attr == 9 ){
								;
							} else {
					...
					this.dispItemIdMap = _.reduce(data.ana_disp_item_list, function(map, elem){
						var dispItemId = elem.dispitem_id & amanp_AnaDispItemDefs.AMAN_DI_I_MASK;
						if(dispItemId === 0){
							// フォルダ。グループ名を定義するエントリ。
						}else{
		...
		buildDispItem: function(gbit, dispItemId, sbit){
			...
			var label2 = null;
			switch(sbit){
			case amanp_AnaDispItemDefs.AMAN_DI_S_PLANRT:
				label2 = "計画比";
				break;
			case amanp_AnaDispItemDefs.AMAN_DI_S_PLANDF:
				label2 = "計画差";
				break;
			//case amanp_AnaDispItemDefs.AMAN_DI_S_LYRT:	label2 = "前年比";		break;
			case amanp_AnaDispItemDefs.AMAN_DI_S_LYRT:
				label2 = "前年比";
				break;
			case amanp_AnaDispItemDefs.AMAN_DI_S_LYDF:
				label2 = "前年差";
				break;
			//case amanp_AnaDispItemDefs.AMAN_DI_S_LYVAL:	label2 = "前年実績";	break;
			case amanp_AnaDispItemDefs.AMAN_DI_S_LYVAL:
				label2 = "前年";
				break;
			//case amanp_AnaDispItemDefs.AMAN_DI_S_VSUMRT:label2 = "縦軸構成比";	break;
			case amanp_AnaDispItemDefs.AMAN_DI_S_VSUMRT:
				label2 = "構成比";
				break;
			case amanp_AnaDispItemDefs.AMAN_DI_S_HSUMRT:
				label2 = "横軸構成比";
				break;
			case amanp_AnaDispItemDefs.AMAN_DI_S_SUMRT:
				label2 = "全体構成比";
				break;
			//case amanp_AnaDispItemDefs.AMAN_DI_S_VACMRT:label2 = "縦軸累計比";	break;
			case amanp_AnaDispItemDefs.AMAN_DI_S_VACMRT:
				label2 = "累計構成比";
				break;
			case amanp_AnaDispItemDefs.AMAN_DI_S_HACMRT:
				label2 = "横軸累計比";
				break;
			case amanp_AnaDispItemDefs.AMAN_DI_S_YVAL:
				label2 = "年累計";
				break;
			//case amanp_AnaDispItemDefs.AMAN_DI_S_VAL:	label2 = "実績";		break;
			case amanp_AnaDispItemDefs.AMAN_DI_S_VAL:
				label2 = "実績";
				break;
			default:
			...
			return {
				dispitem_id: (elem.dispitem_id & ~amanp_AnaDispItemDefs.AMAN_DI_S_MASK) | sbit,
		...
		buildSortkeyList: function(dispItems){
			...
			var items = _.reduce(dispItems, function(array, x){
				...
				if(elem){
					var sortDto = {
						dispitem_id: (elem.dispitem_id & ~amanp_AnaDispItemDefs.AMAN_DI_S_MASK) | x.sbit,
						order: (x.order < 0) ? amanp_AnaSortKey.AMANP_ANA_SORTKEY_ORDER_DESCENDING : amanp_AnaSortKey.AMANP_ANA_SORTKEY_ORDER_ASCENDING,
						idx: (x.idx > 0) ? x.idx : 0
					};
					array.push(sortDto);
				}
		...
		setPrevAnaCond: function(prevCond, opt){
			...
			if(prevCond.memblistFocusDto){
				...
				// オプション
				this.cond.extra.membcond = amanp_AnaHead.AMANP_ANA_REQ_MEMBCOND_MEMB;
			}
		...
		setCatalog: function(catalogRsp, opt) {
			...
			var ff = {
				...
				direct_typeYM: function(val){
					var unit = amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_YM;
				...
				direct_typeYMD: function(val){
					var unit = amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_YMD;
					var findDt = null;		// Date オブジェクト
					var fixedVal = val;
					if(val === 0){
						// カタログ登録時、空欄を指定されたので、ope_date を適用する。
				...
				direct_typeYW: function(val, side){
					var unit = amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_YW;
					var findDt = null;		// Date オブジェクト
				...
				direct_typeY2: function(val, side){
					var unit = amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_Y2;
					var findDt = null;		// Date オブジェクト
					var period = null;		// 期間定義
					var label = null;
					if(val === 0){
				...
				direct_typeY4: function(val, side){
					var unit = amanp_AnaPeriod.AMANP_ANA_PERIOD_TYPE_Y4;
					var findDt = null;		// Date オブジェクト
					var period = null;		// 期間定義
					var label = null;
					if(val === 0){
...
//				return doSearch();
		...
		doDrillDown: function(vLeaf){
			...
			req.f_total = 0;
			req.f_subtotal &= ~amanp_AnaHead.AMANP_ANA_REQ_F_SUBTOTAL_V;
			//req.existsum = amanp_AnaHead.AMANP_ANA_REQ_EXISTSUM_OFF;
			...
			// 5. 処理区分：ドリルダウンをセット
			req.operation = amanp_AnaDefs.AMAN_DEFS_ANA_OPERATION_DRILLDOWN;
		...
		doSaveCustList: function(name, f_open, f_listuse){
			...
			var req = Ana.Util.dclone(this.savedReq);
			req.operation = amanp_AnaDefs.AMAN_DEFS_ANA_OPERATION_MEMBLIST;
		...
		doCreateAnonymousCustList: function(cellSelectReq){
			...
			var req = Ana.Util.dclone(this.savedReq);
			req.operation = amanp_AnaDefs.AMAN_DEFS_ANA_OPERATION_NEXT;
		...
		getMAPDATURI: function(cellSelectReq){
			...
			req.anaHead.f_subtotal &= ~(amanp_AnaHead.AMANP_ANA_REQ_F_SUBTOTAL_V);
		...
		getCSVDLURI: function(){
			return this._getDLURLI(amanp_AnaDefs.AMAN_DEFS_ANA_OPERATION_CSVDL, 'CSV');
		},
		// EXCELダウンロードURIを取得する
		doEXCELDURI: function(){
			return this._getDLURLI(amanp_AnaDefs.AMAN_DEFS_ANA_OPERATION_EXCELDL, 'EXCEL');
		},
		...
		abc_add_ABC: function(list) {
			// ランク実績チェック
			var disp_id = (amanp_AnaDispItemDefs.AMAN_DI_G_SALE|amanp_AnaDispItemDefs.AMAN_DI_S_VAL|amanp_AnaDispItemDefs.AMAN_DI_I_ABC);
		...
		abc_add_RANK: function(list, disp_id, rank_name) {
			//var disp_id = (amanp_AnaDispItemDefs.AMAN_DI_G_SALE|amanp_AnaDispItemDefs.AMAN_DI_S_VAL|rank_id);
			var is_rank = _.find(list, function(o) {
				return o.dispitem_id === disp_id;
			});
			if (is_rank == null) {
				// ランク実績の後に挿入
				var disp_id2 = (amanp_AnaDispItemDefs.AMAN_DI_G_SALE|amanp_AnaDispItemDefs.AMAN_DI_S_VAL|amanp_AnaDispItemDefs.AMAN_DI_I_ABC);
				var rank = {
					dispitem_id: disp_id,
					name: rank_name,
				};
		...
		abc_add_VAL: function(list, disp_id, rank_name) {
			...
			if (is_rank == null) {
				...
				var start = -1;
				_.each(list, function(o, k) {
					var id = o.dispitem_id & amanp_AnaDispItemDefs.AMAN_DI_I_MASK;
					switch (id) {
					case amanp_AnaDispItemDefs.AMAN_DI_I_ITEMATTR_ITEM_NAME:	// 商品名
					case amanp_AnaDispItemDefs.AMAN_DI_I_ITEMATTR_COLOR_NAME:	// カラー
					case amanp_AnaDispItemDefs.AMAN_DI_I_ITEMATTR_MAKER_CODE:	// メーカー品番
					case amanp_AnaDispItemDefs.AMAN_DI_I_ITEMATTR_MAKER_NAME:	// メーカー名
					case amanp_AnaDispItemDefs.AMAN_DI_I_SUPPORT_RT_RANK:		// 支持率（選択期間内）
					case amanp_AnaDispItemDefs.AMAN_DI_I_SUPPORT_RT_CM_RANK:	// 支持率（累計）
					case amanp_AnaDispItemDefs.AMAN_DI_I_AM_RANK:				// 金額ランク
					case amanp_AnaDispItemDefs.AMAN_DI_I_QY_RANK:				// 点数ランク
						if (start < k) {
							start = k;
						}
						break;
		...
		buildAnaReq: function() {
			...
			if(this.f_anakind == amcm_type.AMCM_VAL_ANAKIND_ABC){
				if (c.extra.mdabc_type == amanp_AnaHead.AMANP_ANA_REQ_ABC_TYPE_QY) {
					...
					// 売上数順位実績
					this.abc_add_RANK(displist2, amanp_AnaDispItemDefs.AMAN_DI_G_SALE|amanp_AnaDispItemDefs.AMAN_DI_S_VAL|amanp_AnaDispItemDefs.AMAN_DI_I_QY_RANK, '売上数順位実績');
					// 売上数実績
					this.abc_add_VAL(displist2, amanp_AnaDispItemDefs.AMAN_DI_G_SALE|amanp_AnaDispItemDefs.AMAN_DI_S_VAL|amanp_AnaDispItemDefs.AMAN_DI_I_QY, '売上数実績');
					...
					this.abc_add_ABC(displist2);
					// 売上高(税抜)順位実績
					this.abc_add_RANK(displist2, amanp_AnaDispItemDefs.AMAN_DI_G_SALE|amanp_AnaDispItemDefs.AMAN_DI_S_VAL|amanp_AnaDispItemDefs.AMAN_DI_I_AM_RANK, '売上高(税抜)順位実績');
					// 売上高(税抜)実績
					this.abc_add_VAL(displist2, amanp_AnaDispItemDefs.AMAN_DI_G_SALE|amanp_AnaDispItemDefs.AMAN_DI_S_VAL|amanp_AnaDispItemDefs.AMAN_DI_I_AM, '売上高(税抜)実績');
					...
					this.abc_add_ABC(displist2);
					// 支持率(選択期間内)順位実績
					this.abc_add_RANK(displist2, amanp_AnaDispItemDefs.AMAN_DI_G_SUPPORT|amanp_AnaDispItemDefs.AMAN_DI_S_VAL|amanp_AnaDispItemDefs.AMAN_DI_I_SUPPORT_RT_RANK, '支持率(選択期間内)順位実績');
					// 支持率(選択期間内)実績
					this.abc_add_VAL(displist2, amanp_AnaDispItemDefs.AMAN_DI_G_SUPPORT|amanp_AnaDispItemDefs.AMAN_DI_S_VAL|amanp_AnaDispItemDefs.AMAN_DI_I_SUPPORT_RT, '支持率(選択期間内)実績');
					...
					this.abc_add_ABC(displist2);
					// 支持率(累計)順位実績
					this.abc_add_RANK(displist2, amanp_AnaDispItemDefs.AMAN_DI_G_SUPPORT|amanp_AnaDispItemDefs.AMAN_DI_S_VAL|amanp_AnaDispItemDefs.AMAN_DI_I_SUPPORT_RT_CM_RANK, '支持率(累計)順位実績');
					// 支持率(累計)実績
					this.abc_add_VAL(displist2, amanp_AnaDispItemDefs.AMAN_DI_G_SUPPORT|amanp_AnaDispItemDefs.AMAN_DI_S_VAL|amanp_AnaDispItemDefs.AMAN_DI_I_SUPPORT_RT_CM, '支持率(累計)実績');
				}
			}
			...
			// 絞り込み条件 バスケット分析(LUCASベース) 対応
			// 20211212 パケットにAND、ORを設定するように修正
			if (this.f_anakind == amcm_type.AMCM_VAL_ANAKIND_BASKET_EX2) {
				_tmpfocusList = _tmpfocusList.concat(c.basket_list);
				_tmpfocusList2 = _tmpfocusList2.concat(c.basket_list2);


//				for (var i = 0; i < c.basket_list[0].length; i++) {
//					if (i > 0) {
//						var cond_val = c.basket_list[0][i].cond_val;
//						var cond = {};
//						switch(cond_val) {
//						case 1:
//							cond = {
...
//							break;
//						}
//						_tmpfocusList.push(cond);
//					}
//					for (var j = 0; j < c.basket_list[0][i].length; j++) {
//						_tmpfocusList.push(c.basket_list[0][i][j]);
//					}
//				}
//				for (i = 0; i < c.basket_list[1].length; i++) {
//					if (i > 0) {
//						var cond_val = c.basket_list[0][i].cond_val;
//						var cond = {};
//						switch(cond_val) {
...
//							break;
//						}
//						_tmpfocusList2.push(cond);
//					}
//					for (j = 0; j < c.basket_list[1][i].length; j++) {
//						_tmpfocusList2.push(c.basket_list[1][i][j]);
//					}
//				}
			}
			// 軸情報 バスケット分析（拡張）対応
...
//			} else if (this.f_anakind == amcm_type.AMCM_VAL_ANAKIND_BASKET_EX2) {
//				_tmpVAxisList[0] = c.vAxisListEx;
//				_tmpVAxisList[1] = c.vAxisListEx2;
//				_tmpVAxisList[2] = c.vAxisList[0];
			} else {
			...
			var req = {
				...
				operation: amanp_AnaDefs.AMAN_DEFS_ANA_OPERATION_REG,
				// ページ要求
				pageReq: {
					startRecord: 0,
				...
				anaHead: {
					...
					f_zerosuppress: function(v0, h0){	// ゼロサプレス
						var v0supp = (v0) ? amanp_AnaHead.AMANP_ANA_REQ_F_ZEROSUPPRESS_V : 0;
						var h0supp = (h0) ? amanp_AnaHead.AMANP_ANA_REQ_F_ZEROSUPPRESS_H : 0;
						return (v0supp | h0supp);
					}(c.vfzerosuppress, c.hfzerosuppress),
					f_total: c.dispopt.f_total,			// 合計
					f_subtotal: function(vs, hs){		// 小計有無
						var vstotal = (vs === 1) ? amanp_AnaHead.AMANP_ANA_REQ_F_SUBTOTAL_V : 0;
						var hstotal = (hs === 1) ? amanp_AnaHead.AMANP_ANA_REQ_F_SUBTOTAL_H : 0;
					...
					disp_way: function(cond){			// 表示項目並び
						...
						if(_.isNullOrUndefined(cond.hAxisList) || cond.hAxisList.length === 0){
							...
							disp_way = amanp_AnaHead.AMANP_ANA_REQ_DISP_WAY_H;
						}
						return disp_way;
					}(c),
					...
					exist_iymd: c.dispopt.exist_iymd,	// 既存店基準日 Shimizu追加5/16
					close_iymd: c.dispopt.close_iymd,	// 閉店基準日
				...
				// 表示項目
				anaDispItem: _.reduce(c.dispitemlist, function(array, dto){
					if(_.isNumber(dto.dispitem_id)){
						array.push({
							dispitem_id: dto.dispitem_id,
							dispname: dto.name
						});
						return array;
					}
			...
			if(req.anaHead.anakind == amcm_type.AMCM_VAL_ANAKIND_ABC){
				...
				switch(req.anaHead.abc_type){
				 ...
				 case amanp_AnaHead.AMANP_ANA_REQ_ABC_TYPE_SUPPORT_RT:	// 支持率(選択期間内)
					 req.anaVSortkey = this.buildSortkeyList([
						{
							dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_SUPPORT_RT,
							sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VAL,
							order: amanp_AnaSortKey.AMANP_ANA_SORTKEY_ORDER_DESCENDING,
							idx: 0
						}
				 ...
				 case amanp_AnaHead.AMANP_ANA_REQ_ABC_TYPE_SUPPORT_RT_CM:	// 支持率(累計)
					 req.anaVSortkey = this.buildSortkeyList([
						{
							dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_SUPPORT_RT_CM,
							sbit: amanp_AnaDispItemDefs.AMAN_DI_S_VAL,
							order: amanp_AnaSortKey.AMANP_ANA_SORTKEY_ORDER_DESCENDING,
							idx: 0
						}
				 ...
				 default:
					req.anaVSortkey = this.buildSortkeyList([
						{
							//// #20150518 売上は税抜に統一
							////dispItemId: amanp_AnaDispItemDefs.AMAN_DI_I_AM_TAX,
			...
			if (req.anaHead.anakind == amcm_type.AMCM_VAL_ANAKIND_BASKET_EX
				...
				var dispitem_id1 = amanp_AnaDispItemDefs.AMAN_DI_G_BASKET | amanp_AnaDispItemDefs.AMAN_DI_S_VAL | amanp_AnaDispItemDefs.AMAN_DI_I_BASKET_ITEM1;
				var dispitem_id2 = amanp_AnaDispItemDefs.AMAN_DI_G_BASKET | amanp_AnaDispItemDefs.AMAN_DI_S_VAL | amanp_AnaDispItemDefs.AMAN_DI_I_BASKET_ITEM2;
...
//			'click #ca_btn_savelist'			: 'onSaveListClick',
		...
		initialize: function(opt){
			...
			// 顧客リスト登録
			this.CACMV0300Selector = new CACMV0300SelectorView({
				el				: $('#ca_CACMV0300_dialog'),	// 配置場所
				$parentView		: this.$el,
...
//			// リストに登録
//			var $ca_CACMV0300_dialog = $('#ca_CACMV0300_dialog.cl_dialog');
//			if(!$('#ca_CACMV0300_dialog.cl_dialog')[0]){
//				// 要素を足す。
//				$ca_CACMV0300_dialog = $('.cl_dialog:last').after('<div id="ca_CACMV0300_dialog" class="cl_dialog"></div>').next();
//			}
...
//		},
		...
		onSaveListClick: function(e){
			...
			// リスト名とか公開範囲の入力フォーム画面（CACMV0300）を開いて処理を引き渡す。
			// リスト登録画面を表示
			this.anaProc.cellSelectReq = this.anaResultDataGridView.buildCellSelectReq();
			this.CACMV0300Selector.show(null, this.anaProc);
    		// サブ画面復帰後処理
           	this.CACMV0300Selector.okProc = function(data) {
    			// ボタンにフォーカスする
    			$(e.target).focus();
...
//				this.onSearchCompleted(this.anaProc);
		...
		onNextAnalizeSelected: function(nextAnaDto){
			...
			// 併売分析（品種別分析）に遷移する場合は品種の行を選択必須
			if(nextAnaDto.func_cd == "AMGAV2120" || nextAnaDto.func_cd == 'AMGAV2110') {
				var vAxis = this.anaResultDataGridView.buildCellSelectReq();
				var itgrp_check = function(axis) {
					return axis.v_axis_kind == amanp_AnaDefs.AMAN_DEFS_KIND_RCPT_ITGRP;
				}
				var check = vAxis.some(itgrp_check);
				if(!check) {
					if (nextAnaDto.func_cd == 'AMGAV2120') {
						this.validator.setErrorInfo({_eb_: '併売分析（品種別分析）に遷移するには、品種組合せの表示されている行である必要があります。'});
						return;
					// 併売分析（併売点数分析）が選択された場合は引数を設定
					} else {
						// 画面遷移パラメタ
						...
						deferd.done(_.bind(function(data){
							...
							if(memblist && memblist.elem_num > 0){
								// 会員リストの絞込条件を入れるよ！
								memblistFocusDto = {
									kind: amanp_AnaDefs.AMAN_DEFS_KIND_MEMBLIST,	// 803
									name: this.anaProc.anamenuitem_name + ': 対象人数[' + memblist.elem_num + ']',
									val: memblist.id
								};
//							}else{
...
//								return;
							...
							// 併売分析の画面遷移用に処理を追加（品種の処理は不要）
							if ( this.anaProc.f_anakind == amcm_type.AMCM_VAL_ANAKIND_RCPT_QY) {
								var _this = this;

								var cellSelectData = this.anaResultDataGridView.buildCellSelectReq();

								for (var i = 0; i < cellSelectData.length; i++) {
									var add_focus = {};
									var add_kind = cellSelectData[i].v_axis_kind;
									var add_val  = cellSelectData[i].v_axis_val;
									var add_vid = cellSelectData[i].vid;
									var add_info = _this.anaResultDataGridView.anaProc.savedResult.anaVAxisElem[add_vid - 1];
									var add_code = add_info.code;
									var add_name = add_info.name;
									// 購入点数
									if ( add_kind == amanp_AnaDefs.AMAN_DEFS_KIND_RCPT_QY ) {
										add_focus = {
												anaattr : {
													code : add_code,
													name : add_name,
									...
									// マーケット
									} else if ( add_kind == amanp_AnaDefs.AMAN_DEFS_KIND_TRANATTR_MARKET ) {
										add_focus = {
												code : add_code,
												kind : add_kind,
												name : add_name,
									...
									// 年代
									} else if ( add_kind == amanp_AnaDefs.AMAN_DEFS_KIND_TRANATTR_SEXAGE ) {
										add_focus = {
												code : add_code,
												kind : add_kind,
												name : add_name,
									...
									// 店舗
									} else if ( add_kind == amanp_AnaDefs.AMAN_DEFS_KIND_STORE ) {
										add_focus = {
												code : add_code,
												kind : add_kind,
												name : add_name,
									...
									// 分析条件がセットされている場合は置き換える
									var new_focusList1 = _.filter(nextPageArgs.prevAnaCond.focusList1, function(focusList) {
										return focusList.kind != add_kind;
									});
									new_focusList1.push(add_focus);
									nextPageArgs.prevAnaCond.focusList1 = new_focusList1;
								}
							}
...
//							}
					...
					// 品種が含まれている場合は品種idを取得してから引数を設定する
					var srch_url = "am_pa_variety_srch";
					var mstData_info = this.anaResultDataGridView.anaProc.savedCond.mstsrchdate;
					var srchiymd_type = mstData_info.f_srchiymd_type;
					// マスタ検索日で品種を検索する
					if(srchiymd_type == amanp_AnaHead.AMANP_ANA_REQ_F_SRCHIYMD_TYPE_COM) {
						var srchiymd = mstData_info.srchiymd;
					} else {
						var srchiymd = itgrp_srchiymd;
					}
					// 全品種を取得
					var itgrp_cond = {
							srchFromDate : srchiymd,
							srchToDate : srchiymd,
							ymd : srchiymd
					}
					var itgrp_req = {
					...
					clutil.postJSON(srch_url, itgrp_req).done(_.bind(function(data) {
...
//								return;
							...
							if ( this.anaProc.f_anakind == amcm_type.AMCM_VAL_ANAKIND_RCPT_QY) {
								...
								for (var i = 0; i < cellSelectData.length; i++) {
									...
									// 品種
									} else if ( add_kind == amanp_AnaDefs.AMAN_DEFS_KIND_RCPT_ITGRP ) {
										// 品種の場合はcodeとnameが複数連携されるため分割する
										var code_array = add_code.split(' ');
										var name_array = add_name.split(' ');
										// 最後の2つは空白なので不要
										var code_len = code_array.length - 2;
										var focus_array = [];
										var unit_id = 0;
										var itgrp_id = 0;
										for (var k = 0; k < code_len ; k++) {
											// 取得したコードで品種を絞る
											var ItgrpList = ITGRPLIST.filter(function(list) {
												return list.code == code_array[k];
											});
											// unit_idが設定されていない場合は設定する
											if (unit_id == 0) {
												var ItgrpList = ITGRPLIST.filter(function(list) {
													return list.code == code_array[k];
												});
												if(ItgrpList != null && ItgrpList.length > 0) {
													if(ItgrpList[0].name == name_array[k]) {
														unit_id = ItgrpList[0].unit_id;
														itgrp_id = ItgrpList[0].itgrp_id;
													} else if (ItgrpList[1].name == name_array[k]) {
														unit_id = ItgrpList[1].unit_id;
														itgrp_id = ItgrpList[1].itgrp_id;
													} else {
														// ないはず
													}
												}
											// unit_idが設定されている場合は絞込条件に追加
											} else {
												var ItgrpList = ITGRPLIST.filter(function(list) {
													return list.code == code_array[k] && list.unit_id == unit_id;
												})
												itgrp_id = ItgrpList[0].itgrp_id;
											}
											add_focus1 = {
											...
											// 品種別分析に遷移する場合のみ品種を引数にセットする
											if(nextAnaDto.func_cd == "AMGAV2120") {
												nextPageArgs.prevAnaCond.focusList1.push(add_focus1);
											}
											// 商品分類階層部品は同じ品種はセットしない
											if (k == 0) {
												add_focus = {
														attr : "4",
														func_id : "1",
												...
												focus_array.push(add_focus);
											} else {
												if ( code_array[k] != code_array[k-1]) {
													add_focus = {
															attr : "4",
															func_id : "1",
															kind : "201",
										...
										add_kind = amanp_AnaDefs.AMAN_DEFS_KIND_ITGRP;
									}
									// 分析条件がセットされている場合は置き換える
									var new_focusList1 = _.filter(nextPageArgs.prevAnaCond.focusList1, function(focusList) {
										return focusList.kind != add_kind;
									});
									if ( add_kind != amanp_AnaDefs.AMAN_DEFS_KIND_ITGRP) {
										new_focusList1.push(add_focus);
									} else {
										for (var l = 0; l < focus_array.length; l++) {
											new_focusList1.push(focus_array[l]);
										}
									}
									nextPageArgs.prevAnaCond.focusList1 = new_focusList1;
								}
...
//					for (var i = 0; i < vAxis.length; i++) {
//						if(vAxis[i].v_axis_kind != amanp_AnaDefs.AMAN_DEFS_KIND_RCPT_ITGRP) {
//							continue;
//						} else {
//							var add_vid = vAxis[i].vid;
//							var itgrplist_info = this.anaResultDataGridView.anaProc.savedResult.anaVAxisElem[add_vid - 1];
//							var itgrplist_code = itgrplist_info.code;
//							var itgrplist_name = itgrplist_info.name;
//							var itgrplist_code_array = itgrplist_code.split(' ');
//							var itgrplist_name_array = itgrplist_name.split(' ');
//							var code_len = itgrplist_code_array.length - 1;
//							var ItgrpList = [];
//							var parent_id = 0;
//							var _this = this;
//
...
//								// 品種IDを取得
//								var srch_url = "am_pa_variety_srch";
//								var mstData_info = _this.anaResultDataGridView.anaProc.savedCond.mstsrchdate;
//								var srchiymd_type = mstData_info.f_srchiymd_type;
//								if(srchiymd_type == amanp_AnaHead.AMANP_ANA_REQ_F_SRCHIYMD_TYPE_COM) {
//									var srchiymd = mstData_info.srchiymd;
//								} else {
...
//								if(parent_id == 0) {
//									var itgrp_cond = {
//											codename : itgrplist_code_array[j],
//											srchFromDate : srchiymd,
//											srchToDate : srchiymd,
//											ymd : srchiymd
//									}
...
//										cond : itgrp_cond
//								}
//								var itgrplist = this.getItgrp(itgrp_req);
//
//								if (itgrplist != null && itgrplist.length > 0) {
//									if(itgrplist[0].name == itgrplist_name_array[j]) {
//										parent_id = itgrplist[0].unit_id;
//										ItgrpList.push(itgrplist[0]);
//									} else if (itgrplist[1].name == itgrplist_name_array[j]) {
//										parent_id = itgrplist[1].unit_id;
//										ItgrpList.push(itgrplist[1]);
//									} else {
//										// ないはず
//									}
//								}
//
//
////								clutil.postJSON(srch_url, itgrp_req, _.bind(function(data, dataType) {
////									var itgrplist = data.itgrplist;
////									if (itgrplist != null && itgrplist.length > 0) {
////										if(itgrplist[0].name == itgrplist_name_array[j]) {
////											parent_id = itgrplist[0].unit_id;
...
//					currentPage: 1,
				...
				var isMapServiceAvailable = function(anaProc){
					...
					return (lastVAxis && lastVAxis.kind === amanp_AnaDefs.AMAN_DEFS_KIND_ADDR);
				}(anaProc);
...
//			value = value.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");
			...
			while(!indentNode.parent.isRoot()){
				...
				if(pdata.type !== amanp_AnaAxisElem.AMANP_ANA_AXISELEM_TYPE_DATA){
					break;
				}
				indentNode = pnode;
			}
...
//					}
					...
					if(_.isNullOrUndefined(ctx.hAxisRoot)){
						...
						for(var colIdx = frozenColCount; colIdx < columns.length; colIdx++){
							...
							aRow[field] = cellStr;
							  ...
							  , c_di = dispitem_id & amanp_AnaDispItemDefs.AMAN_DI_C_MASK;
							if(c_di === 0 || c_di === amanp_AnaDispItemDefs.AMAN_DI_C_VALUE1){
							  // 期間対比なし or 実績項目である。
							  rCell = cell;
							}else{
							  // 期間対比項目である。-- 実績データをとる。
							...
							if(rCell){
							  var x_di = dispitem_id & ~amanp_AnaDispItemDefs.AMAN_DI_C_MASK
						...
						// ２軸
						if(ctx.dispopt.disp_way === amanp_AnaHead.AMANP_ANA_REQ_DISP_WAY_V){
							// 表示項目並び＝たて
		...
		render: function(){
			...
			if(!this.anaProc.hasResultData()){
				...
				var vAxAttr = null;
				if((cond.dispopt.disp_way === amanp_AnaHead.AMANP_ANA_REQ_DISP_WAY_H
						|| cond.dispopt.disp_way === amanp_AnaHead.AMANP_ANA_REQ_DISP_WAY_V
				...
				this.ctx = {
					...
					frozenRowCount		: function(axMaxLv, dspMaxLv, disp_way){
						var fzc = axMaxLv;
						if(disp_way === amanp_AnaHead.AMANP_ANA_REQ_DISP_WAY_V){
						}else{
							// 表示項目：横並び
							fzc += dspMaxLv;
						}
					...
					vid2SlickRowId		: function(vid, did){
						if(!_.isNullOrUndefined(this.hAxisRoot)
								|| this.dispopt.disp_way === amanp_AnaHead.AMANP_ANA_REQ_DISP_WAY_V){
							// ２軸で表示項目並び＝たて
							return 'v' + vid + 'd' + did;
						}
						return vid;
				...
				var columns = function(ctx, rhdCellFmt, cellFmt){
					...
					if (_.isNullOrUndefined(ctx.hAxisRoot) || ctx.dispopt.disp_way !== amanp_AnaHead.AMANP_ANA_REQ_DISP_WAY_V) {
						// 軸属性情報(マスタ項目)
						if(ctx.vAxAttr){
							aColumns = new Array();
...
//				}(slickOPT.frozenRow, slickOPT.frozenColumn, columns));
		...
		onSlickGridClick: function(e, args){
			...
			if($target.hasClass('toggle')){
				...
				if(args.row === this.ctx.frozenRowCount-1
							...
							&& (_.isNullOrUndefined(this.ctx.hAxisRoot) || this.ctx.dispopt.disp_way === amanp_AnaHead.AMANP_ANA_REQ_DISP_WAY_H)){
		...
		onContextMenu: function(e, args){
			...
			// 2) 表示項目が縦軸についていること
			if(this.ctx.dispopt.disp_way !== amanp_AnaHead.AMANP_ANA_REQ_DISP_WAY_V){
				// 表示項目が横軸にある
				return;
			}
	...
	Ana.ResultCopyGridView = Backbone.View.extend({
		...
		setData: function(ctx){
			...
			if(ctx.hAxisRoot && ctx.dispopt.disp_way === amanp_AnaHead.AMANP_ANA_REQ_DISP_WAY_V){
				// ２軸：表示並び＝たて
			...
			if(_.isNullOrUndefined(ctx.hAxisRoot)){
				...
				ctx.hAxisMaxLvLeaf = _.find(ctx.hAxisLeafNodeArray, function(lfNode){
					return lfNode.level == ctx.hAxisMaxLevel
						&& (lfNode.data && lfNode.data.type == amanp_AnaAxisElem.AMANP_ANA_AXISELEM_TYPE_NORMAL);
				});
				...
				if (ctx.hAxisRoot
						&& (ctx.dispopt.disp_way === amanp_AnaHead.AMANP_ANA_REQ_DISP_WAY_H
			...
			if(!this.ctx){
				...
				var columns = function(ctx, rhdCellLineno, cellFmt){
					...
					function getVAxAttrColumns(kind) {
						...
						switch (kind) {
						case amanp_AnaDefs.AMAN_DEFS_KIND_ITGRP:	// 商品分類（これで返ってくるみたい）
						case amanp_AnaDefs.AMAN_DEFS_KIND_ITEM:		// 品番（念のためこれも入れておく）
							for(var i=0; i<ctx.vAxAttr.dspList.length; i++){
								var vAttrDsp = ctx.vAxAttr.dspList[i];
							...
							break;
						case amanp_AnaDefs.AMAN_DEFS_KIND_COLORITEM:	// カラー商品
							for(var i=0; i<ctx.vAxAttr.dspList.length; i++){
								var vAttrDsp = ctx.vAxAttr.dspList[i];
								var base = vAttrDsp.dispitem_id & 0x2000FFFF;
								if (base != 0 && !(base & 0x1000) && (base & 0x2000)) {
							...
							break;
						case amanp_AnaDefs.AMAN_DEFS_KIND_COLORSIZEITEM:	// JANコード
							for(var i=0; i<ctx.vAxAttr.dspList.length; i++){
								var vAttrDsp = ctx.vAxAttr.dspList[i];
								var base = vAttrDsp.dispitem_id & 0x2000FFFF;
					...
					for(var lv = 1; lv <= ctx.vAxisMaxLevel; lv++){
						...
						if(node){
							...
							if (node.data) {
								var dispList = getVAxAttrColumns(node.data.kind);
