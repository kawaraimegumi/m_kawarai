...
$(function() {
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMSDV0040 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMSDV0040';
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		doSrch: function(srchReq, selectedIds, $focusElem){
			...
			var defer = clutil.postJSON('AMSDV0040', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMSDV0040GetRsp.packItemList;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
		...
		doDownload: function(req) {
			// リクエストをつくる
			var srchReq = this.buildReq(req.AMSDV0040GetReq);
			if(_.isNull(srchReq)){
				return;
			}
			...
			var defer = clutil.postDLJSON('AMSDV0040', srchReq);
		...
		_doOpeAction: function(rtyp, e){
			var url = clcom.appRoot + '/AMSD/AMSDV0050/AMSDV0050.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
					savedReq: this.savedReq,
					savedCond: this.savedReq.AMSDV0040GetReq,
