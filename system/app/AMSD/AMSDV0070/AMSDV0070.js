		...
		initUIElement: function(){
			...
			this.opeCSVInputCtrl = clutil.View.OpeCSVInputController({
				...
				buildCSVInputReqFunction: _.bind(function(uploadedFile){
					...
					mainView.tmpUploadFile = uploadedFile;
...
//				],
			...
			// グループID -- AMSDV0070 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMSDV0070';
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		doSrch: function(srchReq, selectedIds, $focusElem){
			...
			var defer = clutil.postJSON('AMSDV0070', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMSDV0070GetRsp.itemList;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
		...
		doDownload: function(downloadType){
			...
			if (this.tmpUploadFile && this.tmpUploadFile.id) {
				srchReq.reqHead.fileId = this.tmpUploadFile.id;
			}
			srchReq.AMSDV0070GetReq.downloadType = downloadType;
			srchReq.AMSDV0070GetReq.itemList = this.recListView.getSelectedRecs();

			// 要求を送出
			// clutil.postDLJSON() を使うと共通ヘッダを補完してくれる。
			var defer = clutil.postDLJSON('AMSDV0070', srchReq);
			defer.fail(_.bind(function(data){
				...
				if (data.rspHead && data.rspHead.uri) {
					// CSVダウンロード実行
					clutil.download(data.rspHead.uri);
				}
			}, this));
		...
		_doOpeAction: function(rtyp, e){
			...
			if(this.savedReq){
				...
				myData = {
					...
					savedCond: this.savedReq.AMSDV0070GetReq,
