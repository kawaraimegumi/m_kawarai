...
$(function(){
	...
	var MainView = Backbone.View.extend({
		...
		sampleURL: "/public/sample/EC受注ベース店舗日別計画_サンプル.xlsx",
		...
		initUIelement : function(){
			...
			this.opeCSVInputCtrl = clutil.View.OpeCSVInputController({
				...
				buildCSVInputReqFunction: _.bind(function(uploadedFile){
					// リクエストデータ本体
					var request = {
						AMBPV0090UpdReq: clutil.view2data(this.$("#ca_searchArea"))
					};
			...
			if (clcom.userInfo.unit_id != 0) {
				...
				if (clcom.userInfo.unit_id == aoki1) {
					unit_id = Number(clcom.getSysparam('PAR_AMMS_UNITID_AOKI'));
			...
			this.fieldRelation = clutil.FieldRelation.create('default', {
				...
				dataSource: {
					funcID: Number(clcom.getSysparam('PAR_AMMS_DEFAULT_ORG_FUNCID'))
				}
			});
		...
		doDownload: function(rtyp) {
			...
			var defer = clutil.postDLJSON('AMBPV0090', srchReq);
