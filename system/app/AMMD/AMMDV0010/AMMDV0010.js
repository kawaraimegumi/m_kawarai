...
//							}
	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			// 担当者
			clutil.clusercode2($("#ca_srchOperID"));
			// 検索日
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMMDV0010 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMMDV0010';
			// ページャ
		...
		doSrch: function(srchReq, selectedIds){
			...
			var defer = clutil.postJSON('AMMDV0010', srchReq).done(_.bind(function(data){
				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMMDV0010GetRsp.markDownList;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
		...
		doDownload: function(){
			...
			var defer = clutil.postDLJSON('AMMDV0010', srchReq);
		...
		_onCheckClick:function() {
			var url = clcom.appRoot + '/AMMD/AMMDV0030/AMMDV0030.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
					savedReq: this.savedReq,
					savedCond: this.savedReq.AMMDV0010GetReq,
		...
		_doOpeAction: function(rtyp, e){
			var url = clcom.appRoot + '/AMMD/AMMDV0020/AMMDV0020.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
