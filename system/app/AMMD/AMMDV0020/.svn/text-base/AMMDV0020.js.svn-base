...
var OpeType = {
		AMMDV0020_OPETYPE_SAVE:		101,	// 一時保存
		AMMDV0020_OPETYPE_APPLY:	102,	// 申請
		AMMDV0020_OPETYPE_RETURN:	103,	// 差戻し
		AMMDV0020_OPETYPE_APPROVE1: 104,		// 1次承認済
		AMMDV0020_OPETYPE_APPROVE: 105,		// 承認済

		AMMDV0020_OPETYPE_NUM: 100,			//TypeID調整係数
};

// MD-3581 マークダウン_①マークダウン区分追加_PGM開発
var myMarkdownTypeList = clcom.getTypeList(amcm_type.AMCM_TYPE_MARKDOWN);
...
$(function(){
	...
	var MainView = Backbone.View.extend({
		...
		sampleURL: "/public/sample/マークダウン依頼サンプル.xlsx",
			...
			var mdBaseViewOpt = _.bind(function(o){
				...
				if(o.opeTypeId !== am_proto_defs.AM_PROTO_COMMON_RTYPE_REL){
					...
					_.extend(mdOpt, {
						opeTypeId: [
							{
								opeTypeId: OpeType.AMMDV0020_OPETYPE_SAVE,
								label: '一時保存'
							},
							{
								opeTypeId: OpeType.AMMDV0020_OPETYPE_APPLY,
								label: '申請'
							},
						],
						btn_cancel: true,
		...
		initUIElement: function(){
			...
			this.opeCSVInputCtrl = clutil.View.OpeCSVInputController({
				...
				buildCSVInputReqFunction: _.bind(function(uploadedFile){
					// リクエストデータ本体
					var sendData = {
							markDown : clutil.view2data(this.$("#ca_search"))
					};
					var request = {
							AMMDV0020UpdReq: sendData
					};
			...
			this.opeCSVInputCtrl.on('done', function(data){
				var list = data.AMMDV0020GetRsp.markDownItemList;
		...
		_isConfirmLeaving: function(isSubmitBlocking, pgIndex) {
			...
			if ((flg != true)
					&& (this.opeTypeId == am_proto_defs.AM_PROTO_COMMON_RTYPE_NEW)
					&& (this.f_confirm != true)) {
				flg = true;
			}
		...
		setFooterButtons: function(opt) {
			...
			if (opt.opeTypeId == am_proto_defs.AM_PROTO_COMMON_RTYPE_NEW) {
				...
				if (opt.approveTypeID == amcm_type.AMCM_VAL_APPROVE_APPLY) {
					...
					if (opt.srcId == "AMMDV0010") {
						// マークダウン一覧からの遷移なら、キャンセルのみ
						opeTypeId = -1;
					}
					else{
						// 承認一覧からの遷移
						opeTypeId = [
							{
								opeTypeId: OpeType.AMMDV0020_OPETYPE_RETURN,
								label: '差戻し'
							},
							{
								opeTypeId: OpeType.AMMDV0020_OPETYPE_APPROVE1,
								label: '承認'
							},
						];
						//差し戻し理由欄は編集可能
		...
		removereadOnly: function(){
			...
			//依頼番号のみ編集不可
			clutil.viewReadonly($("#ca_instructCodeArea"));
			this.setFocus();
		...
		_onMDGetCompleted: function(args, e){
			var data = args.data;
			var getRsp = data.AMMDV0020GetRsp;

			var serchData = getRsp.markDown;
			var tblData = getRsp.markDownItemList;
			...
			this.validator.clearErrorMsg($("#ca_alarmDate"));
				...
				if(serchData.stateID == amcm_type.AMCM_VAL_APPROVE_APPLY
					...
					if(today > serchData.releaseDate){
						...
						if(releaseDate >= serchData.limitDate){
							...
							if(limitDate > serchData.alarmDate){
								//対応期限 >= アラーム表示期限となってしまったら
								this.alarmDatePicker.datepicker('setIymd', clutil.addDate(limitDate, 3));
								alarmDate = $('#ca_alarmDate').val();
								alarmDate = clutil.dateFormat(alarmDate, 'yyyymmdd');
								serchData.alarmDate = alarmDate;
								this.validator.setErrorMsg($("#ca_alarmDate"), clmsg.WMD0002, 'alert');
							}
						}
		...
		makeTable: function(list, rtFlag, csvFlag) {
			for(var i=0; i<list.length; i++){
				...
				if(rtFlag == 3){
					...
					var pdo = Number(list[i].retailPriceOrg);
		...
		isValid: function(list){
			...
			if(today >= release){
				//過去日はNG(当日もNG)
				this.validator.setErrorMsg($("#ca_releaseDate"), clmsg.EMD0012);
				retStat = false;
			}

			if(release >= limit){
				this.validator.setErrorMsg($("#ca_limitDate"), clmsg.EMD0005);
				retStat = false;
			}
			if(limit > alarm){
				this.validator.setErrorMsg($("#ca_alarmDate"), clmsg.EMD0006);
				retStat = false;
			}
			...
			var tailIsEmptyFunc = function(dto){
				...
				// MD-3581 マークダウン_①マークダウン区分追加_PGM開発
				if(!(_.isEmpty(dto.markdownTypeID) || dto.markdownTypeID == 0)){
					return false;
				}
			...
			if(emptyCell == list.length){
				...
				clutil.mediator.trigger('onTicker', clmsg.EMD0008);
				return false;
			}
		...
		_buildSubmitReqFunction: function(opeTypeId, pgIndex){
			...
			// 指示条件、テーブル内容取得
			var markDown = clutil.view2data($("#ca_search"));
			var markDownItemList = this.dataGrid.getData({ delflag: false });

			//エラーチェック
			if(this.isValid(markDownItemList) == false){
				return;
			}

			if(markDownItemList.length == 0 || markDownItemList == []){
				// 設定項目がなければヘッダにエラーメッセージを表示
				clutil.mediator.trigger('onTicker', clmsg.EMD0008);
				return;
			}
				...
				//差し戻しが押された場合
				if(opeTypeId == OpeType.AMMDV0020_OPETYPE_RETURN){
					...
					if(reject.length == 0){
						...
						markDown.rejectList = this.rejectList;
					}
				}
				//エラーリセット
				...
				//数値化
				markDown.unitID = Number(markDown.unitID);
				markDown.state = Number(markDown.state);
				markDown.instructID = Number(markDown.instructID);

				//承認状態設定(押下されたボタンの種類にリクエストを更新)
				if(this.opeTypeId == am_proto_defs.AM_PROTO_COMMON_RTYPE_DEL){
					//削除時は一時保存のステータスを渡す
					markDown.stateID = OpeType.AMMDV0020_OPETYPE_SAVE;
				}
				else{
					markDown.stateID = opeTypeId;
				}


				//100ひいてリクエストの数値と合わせる
				//リクエストと違う数にしておかないとフッタ色や動作が削除等の動作にかぶるため
				markDown.stateID = markDown.stateID - OpeType.AMMDV0020_OPETYPE_NUM;


				for(var i=0; i<markDownItemList.length; i++){
					if(markDownItemList[i].itemID != 0
							&& markDownItemList[i].itemID != undefined && markDownItemList[i].itemID != null){
						var tmp = _.clone(markDownItemList[i]);
						//markDownItemList[i].itemID = Number(markDownItemList[i].itemID);
						//markDownItemList[i].discRt = Number(markDownItemList[i].discRt);
						//markDownItemList[i].retailPriceDisc = Number(markDownItemList[i].retailPriceDisc);
						//markDownItemList[i].mfrID = markDownItemList[i].maker.id;

						//更新しない項目は消す
						//delete markDownItemList[i].retailPriceOrg;
						//delete markDownItemList[i].retailPriceDiscOrg;

						tmp.itemID = Number(markDownItemList[i].itemID);
						tmp.discRt = Number(markDownItemList[i].discRt);
						tmp.retailPriceDisc = Number(markDownItemList[i].retailPriceDisc);
						tmp.mfrID = markDownItemList[i].maker.id;
						tmp.markdownType = Number(markDownItemList[i].markdownType);

						delete tmp.retailPriceOrg;
						delete tmp.retailPriceDiscOrg;
		...
		getColumns: function(){
			...
			var _this = this;
			var columns = [
			               { id: 'maker', name: "メーカー", cssClass: 'cl_valid_auto_off', field: "maker", width: 200,
			            	   cellType: {
			            		   type: 'clajaxac',
			            		   editorOptions: {
			            			   funcName: 'vendorcode',
			               ...
			               { id: 'retailPriceOrg', name: "当初指定上代(税抜)", field: "retailPriceOrg", cssClass: 'txtalign-right', width: 140 },
			            	   ...
			            	   cellType: {
			            		   ...
			            		   isEditable: function(item){
			            			   if(_this.gridEditCtrl.discRateEditable){
			            				   ...
			            				   // TODO: 率から価格計算
			            				   if(!(_.isEmpty(item.discRt)) && !(_.isEmpty(item.retailPriceOrg))){
			            					   var rt = Number(item.discRt);
				            				   //var pdo = Number(item.retailPriceDiscOrg.replace(',', ""));
				            				   var pdo = Number(item.retailPriceOrg.replace(',', ""));
...
//			            				   }
			               ...
			               { id: 'markdownTypeID',
			            	 name: "マークダウン区分",
			            	 field: "markdownTypeID",
			            	 width: 140,
			            	 cellType: {
			            		 type: "cltypeselector",
			            		 formatter: function(value, options) {
			     					var label = "";
			     					var result = myMarkdownTypeList.filter(function(val, index, array) {
			     				...
			     				editorOptions: function(item){
			     					var options = { kind: amcm_type.AMCM_TYPE_MARKDOWN };
			    					return options;
			     				},
			     				isEditable: function(item){
			     					return !!(item && item.maker && item.maker.id && item.itemID);
			     				},
			     				validator: 'required',
			            	 },
			               },
		...
		buildReq: function(){
			...
			for(var i=markDownItemList.length-1; i>=0; i--){
				var data = markDownItemList[i];
				var f_send = false;
				if(f_last == false){
					f_send = true;
				}
				...
				//メーカーID、商品ID、商品コード、下代、上代、マークダウン区分のいずれかがあれば送信リスト入り
				...
				if((data.mfrItCode != "" && data.mfrItCode != null && data.mfrItCode != undefined)
						...
						|| (data.markdownType != "" && data.markdownType != null && data.markdownType != undefined)){
					f_send = true;
				}
		...
		_onCSVClick: function(){
			...
			var defer = clutil.postDLJSON('AMMDV0020', srchReq);
	...
	clutil.getIniJSON().done(function(data){
		if(clcom.pageArgs) {
			// ページ遷移引数で、chkDataは存在しないが、srchIDが設定されている場合
			// (通知一覧から遷移してきた場合)
			if(!clcom.pageArgs.chkData) {
				if(clcom.pageArgs.data) {
					var chkData = [];
					chkData[0] = {
						id : Number(clcom.pageArgs.data.srchID)
					};
