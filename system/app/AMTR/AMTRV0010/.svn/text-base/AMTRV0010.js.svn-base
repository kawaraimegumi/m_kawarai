			...
			opeBtnIsEnabled: function(args){
				...
				if(args.selectedCount == 1){
					...
					case am_proto_defs.AM_PROTO_COMMON_RTYPE_DEL:
						var releaseDate = args.selectedRows[0].releaseDate;
...
//						}
					...
					if(args.btnOpeTypeId === am_proto_defs.AM_PROTO_COMMON_RTYPE_UPD){
						...
						for(var i=0; i<args.selectedCount; i++){
							var releaseDate = args.selectedRows[i].releaseDate;
							if(releaseDate <= today){
								return false;
							}
						}
	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			if(unit == null){
				...
				this.deserialize({
					...
					//srchItgrpID: null,					// 品種
					srchCode: null,						// 依頼番号
					srchFromDate: clcom.getOpeDate(),	// 依頼日 yyyymmdd
					srchToDate: null,					// 依頼日 yyyymmdd
					srchUserID: null					// 担当者
				});
			}
			else{
		...
		_onSrchClick: function(e) {
			...
			//担当者入力がおかしかったらエラー
			var userID = dto.srchUserID;
			var userObj = $("#ca_srchUserID").val();

			if((userID == undefined && userObj != "") || (userID == null && userObj != "")){
				this.validator.setErrorMsg($("#ca_srchUserID"), clmsg.EGM0034);
				clutil.mediator.trigger('onTicker', clutil.fmtargs(clmsg.cl_echoback));
				return;
			}
			clutil.mediator.trigger('ca_onSearch', dto);
		},

		_eof: 'AMTRV0010.SrchCondView//'
	});
	...
	var MainView = Backbone.View.extend({
		...
		_onCheckClick:function() {
			...
			var url = clcom.appRoot + '/AMTR/AMTRV0030/AMTRV0030.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
					savedReq: this.savedReq,
					savedCond: this.savedReq.AMTRV0010GetReq,
		...
		initialize: function(){
			...
			// グループID -- AMTRV0010 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMTRV0010';
			// ページャ
		...
		doSrch: function(srchReq, selectedIds){
			this.clearResult();
			var defer = clutil.postJSON('AMTRV0010', srchReq).done(_.bind(function(data){
				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMTRV0010GetRsp.transferList;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
		...
		doDownload: function(){
			...
			var defer = clutil.postDLJSON('AMTRV0010', srchReq);
		...
		_doOpeAction: function(rtyp, e){
			var url = clcom.appRoot + '/AMTR/AMTRV0020/AMTRV0020.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
