$(function(){
			...
			opeBtnIsEnabled: function(args){
				...
				if(args.selectedCount == 1){
					...
					case am_proto_defs.AM_PROTO_COMMON_RTYPE_DEL:
						var editFlag = args.selectedRows[0].editFlag;
						if(editFlag == 0){
							return false;
						}
					}
					...
					if(args.btnOpeTypeId === am_proto_defs.AM_PROTO_COMMON_RTYPE_UPD){
						...
						for(var i=0; i<args.selectedCount; i++){
							var editFlag = args.selectedRows[i].editFlag;
							if(editFlag == 0){
								return false;
							}
						}
	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			clutil.datepicker(this.$('#ca_srchUpdDate'));

			// 担当者
			clutil.clstaffcode2($("#ca_srchUserID"));
			
			// 移動区分
			clutil.cltypeselector(this.$("#ca_srchTransType"), amcm_type.AMCM_TYPE_TRANS_PROCESS_TYPE, 1);
			// 移動状況
			clutil.cltypeselector(this.$("#ca_srchTransState"), amcm_type.AMCM_TYPE_TRANS_STAT, 1);


			// 初期値を設定
			this.deserialize({
				//TODO:初期値->ログインユーザの事業ユニット
				srchOutStoreID: null,					// 出荷店舗
				srchOutFromDate: clcom.getOpeDate(),	// 出荷日 yyyymmdd
				srchOutToDate: null,					// 出荷日 yyyymmdd
				srchInStoreID: null,					// 入荷店舗
				srchInFromDate: null,					// 入荷日 yyyymmdd
				srchInToDate: null,					// 入荷日 yyyymmdd
				srchUpdDate: null,					// 更新日 yyyymmdd
				srchSlipNo: null,					// 伝票番号
				srchUserID: null					// 担当者
			});
		...
		isValid: function(){
			...
			if(OF_Date + OT_Date + IF_Date + IT_Date == 0){
				this.validator.setErrorMsg($(".ca_dateRange"), clmsg.ETR0022);
				retStat = false;
			}
		...
		_onStoreSelClick: function(e) {
			...
			_this.AMPAV0010Selector.okProc = function(data) {
				if (data !== null && data.length == 1) {
					...
					if(store_type == "In"){
						_this.srcInStoreIdField.setValue({
							id: d.val,
							code: d.code,
							name: d.name
						});
						...
						_this.srcOutStoreIdField.setValue({
							id: d.val,
							code: d.code,
							name: d.name
						});
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMTRV0040 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMTRV0040';
			// ページャ
		...
		doSrch: function(srchReq, selectedIds){
			this.clearResult();
			var defer = clutil.postJSON('AMTRV0040', srchReq).done(_.bind(function(data){
				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMTRV0040GetRsp.transferList;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
				...
				for(var i=0; i<recs.length; i++){
					//結果の出荷日が-1(データ未設定)の場合は空白とする(表示がおかしくなるため)
					if(recs[i].outDate == -1){
						recs[i].outDate = "";
					}
					if(recs[i].inDate == clcom.min_date - 1){
						recs[i].inDate = "";
					}
					//移動区分
					if(recs[i].transType == 0){
						recs[i].transType_disp = "";
					}
					else{
						recs[i].transType_disp = clutil.gettypename(amcm_type.AMCM_TYPE_TRANS_PROCESS_TYPE, recs[i].transType);
					}
					//移動状況
					if(recs[i].transState == 0){
						recs[i].transState_disp = "";
					}
					else{
						recs[i].transState_disp = clutil.gettypename(amcm_type.AMCM_TYPE_TRANS_STAT, recs[i].transState);
					}
				}
				...
				if(!_.isEmpty(selectedIds)){
					var selectedRecsWk = [];
					// 検索結果(一覧表示)より、選択していたID(出荷伝票番号)で且つ履歴でないデータを選択状態とする
					// ※出荷伝票修正時は出荷伝票IDが変わってしまうので選択状態とならないが、
					//   入荷伝票修正時は最新データを選択状態とする
					$.each(recs,function(){
						if (this.editFlag == 1){
							for( var idsCnt = 0; idsCnt < selectedIds.length; idsCnt++) {
								if(this.id == selectedIds[idsCnt]) {
									selectedRecsWk.push(this);
								}
							}
						}
					});

					if(!_.isEmpty(selectedRecsWk)){
						this.recListView.setSelectRecs(selectedRecsWk, true, ['uniqID']);
					}
				}
				this.resetFocus();
		...
		doDownload: function(){
			...
			var defer = clutil.postDLJSON('AMTRV0040', srchReq);
		...
		_doOpeAction: function(rtyp, e){
			var url = clcom.appRoot + '/AMTR/AMTRV0050/AMTRV0050.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
					savedReq: this.savedReq,
					savedCond: this.savedReq.AMTRV0040GetReq,
