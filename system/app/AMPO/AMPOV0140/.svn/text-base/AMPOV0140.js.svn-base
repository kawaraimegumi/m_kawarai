...
$(function(){
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMPOV0140 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMPOV0140';
		...
		doSrch: function(srchReq, chkData, $focusElem){
			...
			var defer = clutil.postJSON('AMPOV0140', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMPOV0140GetRsp.stgrpList;
...
//			this.srchAreaCtrl.show_srch();
		...
		_doOpeAction: function(ope_mode, pgIndex/*一覧画面では使用しない*/, e){
			...
			var url = clcom.appRoot + '/AMPO/AMPOV0150/AMPOV0150.html';
//			var myData, destData;
			// 画面遷移引数
...
//							srchDate: this.savedReq.srchDate,
						...
						saved: {
							...
							savedCond: this.savedReq.AMPOV0140GetReq,
							chkData: this.recListView.getSelectedRecs()
						}
					};
				break;
...
//				this.srchAreaCtrl.show_srch();
			...
			var defer = clutil.postDLJSON('AMPOV0140', srchReq);
