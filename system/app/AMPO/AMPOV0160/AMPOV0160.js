...
$(function(){
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMPOV0160 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMPOV0160';
		...
		doSrch: function(srchReq, chkData, $focusElem){
			...
			var defer = clutil.postJSON('AMPOV0160', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMPOV0160GetRsp.makerList;
...
//					// 検索ボタンにフォーカスする
//					this.$('#ca_AMRSV0160_search').focus();
//				} else {
//					// 条件を追加ボタンにフォーカスする
//					this.$('#ca_AMRSV0160_add').focus();
//				}
			}
		},
		...
		_doOpeAction: function(rtyp, pgIndex/*一覧画面では使用しない*/, e){
			...
			var url = clcom.appRoot + '/AMPO/AMPOV0170/AMPOV0170.html';
			...
			if(this.savedReq){
				...
				myData = {
					...
					savedCond: this.savedReq.AMPOV0160GetReq,
		...
		doDownload: function(){
			...
			var defer = clutil.postDLJSON('AMPOV0160', srchReq);
