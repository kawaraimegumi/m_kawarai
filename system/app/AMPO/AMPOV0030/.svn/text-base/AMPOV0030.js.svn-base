	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			clutil.cltypeselector({$select: this.$("#ca_srchPOTypeID"),
				...
				this.$("#ca_srchPOTypeID").selectpicker('val', amcm_type.AMCM_VAL_PO_CLASS_LADYS);
			}));
//			clutil.cltypeselector(this.$("#ca_srchPOTypeID"), amcm_type.AMCM_TYPE_PO_CLASS);

//			//最終更新者
//			clutil.clusercode($("#ca_srchUserID"));
...
//			// 検索日
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMPOV0030 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMPOV0030';
		...
		doSrch: function(srchReq, chkData, $focusElem){
			...
			var defer = clutil.postJSON('AMPOV0030', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMPOV0030GetRsp.brandList;
...
//					// 検索ボタンにフォーカスする
//					this.$('#ca_AMRSV0030_search').focus();
//				} else {
//					// 条件を追加ボタンにフォーカスする
//					this.$('#ca_AMRSV0030_add').focus();
//				}
			}
		},
		...
		_doOpeAction: function(ope_mode, pgIndex/*一覧画面では使用しない*/, e){
			...
			var url = clcom.appRoot + '/AMPO/AMPOV0040/AMPOV0040.html';
			// 画面遷移引数
			...
			case am_proto_defs.AM_PROTO_COMMON_RTYPE_NEW:	// 新規
				pushPageOpt = {
					...
					saved: {
						btnId: e.target.id,
						savedReq: null,
						savedCond: clutil.view2data($('#ca_srchArea')),
			...
			default:										// その他、編集、予約取消、削除
				...
				pushPageOpt = {
					...
					saved: {
						...
						savedCond: this.savedReq.AMPOV0030GetReq,
						chkData: this.recListView.getSelectedRecs()
					}
				};
			}
		...
		doDownload: function(){
			...
			var defer = clutil.postDLJSON('AMPOV0030', srchReq);
