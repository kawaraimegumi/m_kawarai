...
//			// 検索日
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMPOV0200 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMPOV0200';
		...
		doSrch: function(srchReq, chkData, $focusElem){
			...
			var defer = clutil.postJSON('AMPOV0200', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMPOV0200GetRsp.optGrpList;
...
//				// 検索ボタンにフォーカスする
//				this.$('#ca_AMRSV0200_search').focus();
//				} else {
//				// 条件を追加ボタンにフォーカスする
//				this.$('#ca_AMRSV0200_add').focus();
//				}
			}
		},
		...
		_doOpeAction: function(ope_mode, pgIndex/*一覧画面では使用しない*/, e){
			...
			var url = clcom.appRoot + '/AMPO/AMPOV0210/AMPOV0210.html';
			// 画面遷移引数
			...
			pushPageOpt = {
					...
					saved: {
						...
						savedCond: this.savedReq.AMPOV0200GetReq,
						chkData: this.recListView.getSelectedRecs()
					}
			};
			}
		...
		doDownload: function(){
			...
			var defer = clutil.postDLJSON('AMPOV0200', srchReq);
