...
//						updMessageDialog: false
		...
		showLArea:function(base, order, orgOrder){
			...
			if(base.L_jk == 1){
				...
				// スラックスボタン変更隠す
				$('#ca_disp_L_sl_changeButtonTypeDiv').hide();
				$('#ca_disp_L_sl_changeButtonTypeHr').hide();
			}
			if(base.L_sk == 1){
				$('#ca_L_skDiv').show();
			}
		...
		// シャツ領域表示
		showSArea:function(base, order, orgOrder){
			$('.ca_S_Area').show();
			$('#ca_otStoreAdjTypeID_div').hide();
			clutil.data2view($('#ca_S_Field'), order);

			// ネーム領域隠す
			if(orgOrder.S_nameVal == amcm_type.AMCM_VAL_INITIAL_OPTION_TYPE_EXIST){
				$('.ca_S_nameNone').show();
			}
			else if(orgOrder.S_nameVal == amcm_type.AMCM_VAL_INITIAL_OPTION_TYPE_NOT_EXIST){
				$('.ca_S_nameNone').show();
				$('.ca_S_nameLong').hide();
			}
			else{
		...
		makeBaseData : function(baseData, otherData, orderType){
			...
			data.disp_express =this.setExpress(otherData.express);
			...
			if(orderType == amcm_type.AMCM_VAL_PO_CLASS_MENS){
				...
				// 名称
				data.disp_M_wash = this.getYesOrNo(data.M_wash);
				data.M_season = data.seasonObj.name;
				data.M_bland = data.brandObj.name;
				data.M_cloth = data.clothObj.code + ":" + data.clothObj.name;
			}
			else if(orderType == amcm_type.AMCM_VAL_PO_CLASS_LADYS){
				data.disp_orderType = "レディス";
				data.L_bland = data.brandObj.name;
				data.L_cloth = data.clothObj.code + ":" + data.clothObj.name;

				data.L_jkSel = this.makeModelName(data.modelJkObj);
				data.L_skSel = this.makeModelName(data.modelSkObj);
				data.L_slSel = this.makeModelName(data.modelSlObj);
				data.L_veSel = this.makeModelName(data.modelVeObj);
			}
			else{
				data.disp_orderType = "シャツ";
				data.S_bland = data.brandObj.name;
				data.S_cloth = data.clothObj.code + ":" + data.clothObj.name;
				data.S_num = data.S_num;
			}

			return data;
		},
		setExpress:function(flag){
			var disp = "なし";
			if(flag == 1){
				disp = "あり";
			}
		...
		makeOrderData : function(orderData, baseData, orderType){
			...
			if(orderType == amcm_type.AMCM_VAL_PO_CLASS_MENS){
				if(base.M_st == 1){
					...
					order.disp_M_jk_amfType = this.getYesOrNo2(order.M_jk_amfType);
					...
					order.disp_M_jk_layerButtonType = this.getYesOrNo2(order.M_jk_layerButtonType);
				...
				if(base.L_jk == 1){
					...
					// order.disp_L_jk_cuffsType = this.getYesOrNo(order.L_jk_cuffsType);
					// 2015/12/9 STB追加対応
//					order.disp_L_jk_cuffsButtonType = this.getYesOrNo(order.L_jk_cuffsButtonType);
//					order.disp_L_jk_sideVentType = this.getYesOrNo(order.L_jk_sideVentType);
					// 2015/12/9 STB追加対応 ここまで
					//order.disp_L_jk_cuffsPayType = this.getYesOrNo(order.L_jk_cuffsPayType);

					// 2016/1/26 PRS対応
					order.disp_L_jk_ventType = order.jkVentObj.name;
					order.disp_L_jk_cuffsType = order.jkCuffsObj.name;
					order.disp_L_jk_cuffsPayType = order.jkCuffsPayObj.name;
					order.disp_L_jk_buttonHoleType = this.get_jk_buttonHoleSel(order, order.L_jk_buttonHoleType);
					order.disp_L_jk_cuffsFirstType = this.getYesOrNo(order.L_jk_cuffsFirstType);
				...
				order.disp_S_collar = order.collarObj.name;
				order.disp_S_collarType = this.getS_CollarType(order.S_collarType);
				order.disp_S_cuffsType = this.getS_CuffsType(order.S_cuffsType);
				order.disp_S_cuffsSel = order.cuffsObj.name;
				order.disp_S_cleric = order.clericObj.name;
				order.disp_S_frontType = order.frontObj.name;
				order.disp_S_backType = order.backObj.name;
				order.disp_S_amf = order.amfObj.name;
				order.disp_S_pocket = order.pocketObj.name;
				order.disp_S_button = order.buttonObj.name;
				order.disp_S_buttonHole = order.buttonHoleObj.name;
				order.disp_S_buttonThread = order.buttonThreadObj.name;
				order = this.getName(orderType, order, null);		// ネーム関係
				order.disp_S_nameType = order.nameTypeObj.name;
				order.disp_S_namePlace = order.namePlaceObj.name;
				order.disp_S_nameColor = order.nameColorObj.name;
				order.disp_S_sewingType = this.getS_sewingType(order.S_sewingType);
				order.disp_S_pocketChiefType = this.getS_pocketChiefType(order.S_pocketChiefType);
				order.disp_S_body = this.getOverCost(order.bodyObj);
				order.disp_S_neck = this.getOverCost(order.neckObj);
				order.disp_S_armLeft = this.getOverCost(order.armLeftObj);
				order.disp_S_armRigth = this.getOverCost(order.armRigthObj);
				order.disp_S_shoulder = this.getSign(order.S_shoulder);
				order.disp_S_chest =this.getSign( order.S_chest);
				order.disp_S_waist = this.getSign(order.S_waist);
				order.disp_S_bottom = this.getSign(order.S_bottom);
				order.disp_S_length = this.getSign(order.S_length);
				order.disp_S_cuffsLeft = this.getSign(order.S_cuffsLeft);
				order.disp_S_cuffsRight =this.getSign( order.S_cuffsRight);
		...
		getName:function(type, order, makeType){
			...
			if(type == amcm_type.AMCM_VAL_PO_CLASS_MENS){
				...
				// シャツ
				data = this.setNameSpan(type, data.S_nameVal, data);
				data.disp_S_name = this.getNameSel(type, data.S_nameVal);
			}
			return data;
		},
		getNameSel: function(type, sel){
			...
			if(type == amcm_type.AMCM_VAL_PO_CLASS_SHIRT){
				if(sel == amcm_type.AMCM_VAL_INITIAL_OPTION_TYPE_NOT_EXIST){
					disp = "ローマ字(540円)";
				}
				else if(sel == amcm_type.AMCM_VAL_INITIAL_OPTION_TYPE_EXIST){
					disp = "ローマ字フリー(540円)";
				}
				else{
					disp = "なし";
		...
		get_jk_buttonHoleSel:function(order, type){
			var disp = "";
			if(type == 0){
				disp = "なし";
			}
			else{
				disp = order.jkButtonHoleColorLineObj.name;
			}
			return disp;
		},
		...
		getS_CuffsType:function(type){
			var disp = "";
			if(type == amcm_type.AMCM_VAL_ARM_TYPE_LONG){
				disp = "長袖";
			}
			else{
				disp = "半袖";
		...
		getOverCost:function(obj){
			var disp = obj.name;
//			if(obj.cost > 0){
//				disp = disp + "(" + clutil.comma(obj.cost) + "円)";
//			}
			return disp;
		},

		getS_sewingType:function(type){
			var disp = "";
			if(type == 0){
				disp = "なし";
			}
			else{
				disp = "袖付：巻状本縫、脇縫い：巻状二本針(1,080円)";
			}
			return disp;
		},
		getS_pocketChiefType:function(type){
			var disp = "";
			if(type == 0){
				disp = "なし";
			}
...
//		Submit 応答のイベントを受ける
		_onMDSubmitCompleted: function(args, e){
			...
			case 'DONE':		// 確定済
				...
				if(this.options.opeTypeId == am_proto_defs.AM_PROTO_COMMON_RTYPE_NEW){
					//this.$("#ca_no").val(data.AMPOV0270UpdRsp.newNo);

					// T200対応 登録後のメッセージ差分対応 2015/9/5 藤岡
					if(_this.submitType == 0){
						msg = "管理番号 [" + data.AMPOV0270UpdRsp.newNo + "] で発注しました。";
					}
					else{
					...
					var getNo = data.AMPOV0270GetRsp.order.no;
			...
			case 'NG':			// その他エラー。入力値サーバチェックでエラーも含む。
				...
				if(data.AMPOV0270UpdRsp.orderStoppedField != 0){
					this.orderStopWarn(data.AMPOV0270UpdRsp.orderStoppedField);
				}
				// 2016/1/6 ここまで
				// 2016/2/17 店着日が不正
				if(data.AMPOV0270UpdRsp.arrivalDate != 0){
					// 返却項目に、新しい店着日設定
					this.returnObj.f_back = true;
					this.returnObj.arrivalDate = data.AMPOV0270UpdRsp.arrivalDate;
				}
				// 2016/2/17 ここまで
			}
			this.backFooter();
...
//		GET 応答のイベントを受ける
		...
		_buildSubmitReqFunction: function(opeTypeId, pgIndex){
			...
			// シャツ
			var sBody = this.makeSBody(orderType, baseData, orderData);
			var sCollar = this.makeSCollar(orderType, baseData, orderData);
			var sCuffs = this.makeSCuffs(orderType, baseData, orderData);
			var sMain = this.makeSMain(orderType, baseData, orderData);
			var sPktbtn = this.makeSPktbtn(orderType, baseData, orderData);
			var sInitial = this.makeSInitial(orderType, baseData, orderData);
...
//			}
			...
			else if(orderType == amcm_type.AMCM_VAL_PO_CLASS_SHIRT){
				obj.cnt =  baseData.S_num;
				obj.hurryFlag = otherData.express;
			}
		...
		makeLModel:function(orderType, baseData, orderData){
			...
			obj = {
					jacket:{
						modelID:baseData.modelJkObj.id,
					},
					skirt:{
						modelID:baseData.modelSkObj.id,
					},
					pants:{
						modelID:baseData.modelSlObj.id,
					},
					vest:{
						modelID:baseData.modelVeObj.id,
					}
			};
			return obj;
		},
		makeLJacket:function(orderType, baseData, orderData){
			...
			obj = {
					...
					// 2015/12/9 STB追加対応
//					jkSideVent:orderData.L_jk_sideVentType,
//					jkSleeveDesignButton:orderData.L_jk_cuffsButtonType,
					// 2015/12/9 STB追加対応 ここまで

					//2016/1/26 PRS対応
					jkVent:orderData.jkVentObj.id,
					jkSleeveDesign:orderData.jkCuffsPayObj.id,
					jkFreeSleeveDesign:orderData.jkCuffsObj.id,
					jkCuffs:orderData.L_jk_cuffsFirstType,
					//jkButtonHoleColorLine:orderData.jkButtonHoleColorLineObj.id,
					jkButtonHoleColorLine:this.getType2ID(orderType, TYPE.L_JK, amcm_type.AMCM_TYPE_ARM_1ST_BTN_HOLE_COLOR_TYPE
							, orderData.L_jk_buttonHoleType, orderData),

//					jkSleeveDesign:orderData.L_jk_cuffsPayType,
//					jkFreeSleeveDesign:orderData.L_jk_cuffsType,
		 ...
		 * シャツ情報作成
		 */
		makeSBody:function(orderType, baseData, orderData){
			var obj = {};
			if(orderType != amcm_type.AMCM_VAL_PO_CLASS_SHIRT){
				return obj;
			}
			obj={
					bodyType:orderData.S_bodySel,
					neckSize:orderData.S_neckSel,
					leftDegree:orderData.S_armLeftSel,
					rightDegree:orderData.S_armRigthSel,
					sleeveTypeID:orderData.S_cuffsType,
					doAdjTypeID:0,
					shoulder:orderData.S_shoulder,
					chest:orderData.S_chest,
					waist:orderData.S_waist,
					bottom:orderData.S_bottom,
					len:orderData.S_length,
					leftCuff:orderData.S_cuffsLeft,
					rightCuff:orderData.S_cuffsRight
			};
			if(orderData.S_cuffsType != amcm_type.AMCM_VAL_ARM_TYPE_LONG){
				// 半袖ならカフスなし
				obj.leftCuff = 0;
				obj.rightCuff = 0;
				obj.leftDegree = 0;
		...
		makeSCollar:function(orderType, baseData, orderData){
			var obj = {};
			if(orderType != amcm_type.AMCM_VAL_PO_CLASS_SHIRT){
				return obj;
			}
			obj={
					collar1:orderData.S_collarSel,
					collar2:orderData.S_miter,
					cleric:orderData.S_clericSel,
					interLining:orderData.S_collarType
			};
			return obj;
		},
		makeSCuffs:function(orderType, baseData, orderData){
			var obj = {};
			if(orderType != amcm_type.AMCM_VAL_PO_CLASS_SHIRT){
				return obj;
			}
			if(orderData.S_cuffsType == amcm_type.AMCM_VAL_ARM_TYPE_LONG){
				// 長袖
				obj.cuffsType = orderData.S_cuffsSel;
			}
			else{
				// 半袖
				obj.cuffsType = 0;
		...
		makeSMain:function(orderType, baseData, orderData){
			var obj = {};
			if(orderType != amcm_type.AMCM_VAL_PO_CLASS_SHIRT){
				return obj;
			}
			obj={
					front:orderData.S_frontSel,
					back:orderData.S_backSel,
					amfStitch:orderData.S_amfSel
			};
			return obj;
		},
		makeSPktbtn:function(orderType, baseData, orderData){
			var obj = {};
			if(orderType != amcm_type.AMCM_VAL_PO_CLASS_SHIRT){
				return obj;
			}
			obj={
					pocket:orderData.S_pocketSel,
					sewing:orderData.S_sewingType,
					pocketChief:orderData.S_pocketChiefType,
					button:orderData.S_buttonSel,
					buttonHole:orderData.S_buttonHoleSel,
					buttonThread:orderData.S_buttonThreadSel
			};
			return obj;
		},
		makeSInitial:function(orderType, baseData, orderData){
			var obj = {};
			if(orderType != amcm_type.AMCM_VAL_PO_CLASS_SHIRT){
				return obj;
			}
			obj={
					initial:orderData.S_name,
					initialEn1:"",
					initialEn2:this.getCustName(orderType, orderData.S_nameVal, orderData),
					form:this.setS_NameElem(orderData.S_nameVal, orderData.S_nameTypeSel),
					place:this.setS_NameElem(orderData.S_nameVal, orderData.S_namePlaceSel),
					color:this.setS_NameElem(orderData.S_nameVal, orderData.S_nameColorSel)
			};
			return obj;
		},
		setS_NameElem:function(nameType, sel){
			if(nameType == 0){
				sel = 0;
			}
			return sel;
		...
		getType2ID:function(type, chk, kind, val, order){
			...
			if(val != 0){
				if(type == amcm_type.AMCM_VAL_PO_CLASS_MENS){
					...
					else if(kind == amcm_type.AMCM_TYPE_ARM_1ST_BTN_HOLE_COLOR_TYPE){
						// ボタンホールカラー糸変更
						if(chk == TYPE.L_JK){
							id = order.L_jk_buttonHoleSel;
						}
					}
				}
			}
