$(function(){
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMCMV0080 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMCMV0080';
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		doSrch: function(srchReq, chkData, $focusElem){
			...
			var defer = clutil.postJSON('AMCMV0080', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMCMV0080GetRsp.menuList;
				if(_.isEmpty(recs)){
		...
		_doOpeAction: function(rtyp, pgIndex/*一覧画面では使用しない*/, e){
			var url = clcom.appRoot + '/AMCM/AMCMV0090/AMCMV0090.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				...
				myData = {
					...
					savedCond: this.savedReq.AMCMV0080GetReq,
