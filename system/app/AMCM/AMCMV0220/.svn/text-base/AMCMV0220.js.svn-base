...
$(function(){
	...
	var ListView = Backbone.View.extend({
		...
		initialize : function(){
			...
			// グループID -- AMCMV0220 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMCMV0220';
		...
		initUIelement : function(){
			...
			var typelist = clcom.getTypeList(amcm_type.AMCM_TYPE_ALARM_KIND, [amcm_type.AMCM_VAL_ALARM_KIND_MARKDOWN, amcm_type.AMCM_VAL_ALARM_KIND_WRITEDOWN]);
			clutil.cltypeselector2(this.$("#ca_alarmKind"), typelist, 1, 0, 'type_id');
		...
		doSrch: function(srchReq, chkData, $focusElem, f_complete) {
			...
			clutil.postJSON(uri, srchReq).done(_.bind(function(data, dataType) {
					...
					this.resultList = data.AMCMV0220GetRsp.alarmList;
...
//							savedCond: this.savedReq.AMCMV0220GetReq,
//							chkData: [data]
//						},
//						newWindow: true
//					};
...
//			case "館内補正業者請求金額入力":
		...
		onBtnClick : function(e){
			...
			if (confirmFlag) {
				clutil.ConfirmDialog("店舗でのマークダウン作業は完了していますか？", function(_this){
					try{
						_this._doClickReq({
							alarm : data,
							actTypeID : actTypeID
							...
							clutil.MessageDialog2('作業完了報告を受け付けました。');
						},_this));
					}finally{
					}
				}, function(_this){
...
//			}
		...
		_doClickReq : function(updreq){
			var req = {
					...
					AMCMV0220UpdReq : updreq
			};
			return clutil.postJSON('AMCMV0220', req).fail(_.bind(function(data){
				console.log(data.rspHead);
				this.srchAreaCtrl.show_srch();
				// エラーメッセージを通知。
				clutil.mediator.trigger('onTicker', data);
...
//				toLimitDate : clcom.getOpeDate(),
		...
		load2: function(model) {
			if (!_.isEmpty(model.data)) {
				var req = {
					reqHead: {
						opeTypeId: model.opeTypeId,
					},
					reqPage: _.first(this.pagerViews).buildReqPage0(),
					AMCMV0220GetReq: model.data
				};
				this.doSrch(req);
			}
		}
	...
	.done(_.bind(function(data,dataType){				// 画面の初期化を行う
		...
		if (clcom.pageData != null) {
			...
			ca_listView.load2(clcom.pageArgs);
		}
