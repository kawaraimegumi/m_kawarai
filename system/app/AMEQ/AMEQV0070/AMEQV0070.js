...
$(function(){
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMEQV0070 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMEQV0070';
		...
		doSrch: function(srchReq, chkData, $focusElem){
			...
			var defer = clutil.postJSON('AMEQV0070', srchReq).done(_.bind(function(data){

				// データ取得
				var recs = data.AMEQV0070GetRsp.equipList;
				...
				if (_.isEmpty(recs)) {
					...
					$.each(recs,function(){
						this.no = no++;
						this.store_disp = this.storeCode + ':' + this.storeName;
						this.closeType_disp = clutil.gettypename(amcm_type.AMCM_TYPE_EQUIP_ORDER_CLOSE_TYPE, this.closeType);
						var dispList = [];
						...
						this.equip_disp = this.equipName;
					});
		...
		doDownload: function(){
			...
			var defer = clutil.postDLJSON('AMEQV0070', srchReq);
		...
		_doOpeAction: function(rtyp, pgIndex, e){
			...
			var chkDataBuff = this.recListView.getSelectedRecs();
			var chkData = [];
			var checkHash = {};

			if(!_.isEmpty(chkDataBuff)){
				$.each(chkDataBuff,function(){
					var key = this.orderDate.toString();

					if (checkHash[key] == null){
						this.srchEquipIDList = new Array();
						checkHash[key] = this;
					}

					checkHash[key].srchEquipIDList.push(this.equipID);
				});

				$.each(checkHash,function(){
					chkData.push(this);
				});
			}
			...
			if(this.savedReq){
				...
				myData = {
					...
					savedCond: this.savedReq.AMEQV0070GetReq,
					selectedIds: this.recListView.getSelectedRecs(),//this.recListView.getSelectedIdList(),
					chkData: chkData,//this.recListView.getSelectedRecs(),
				};
				destData = {
					opeTypeId: rtyp,
					srchDate: this.savedReq.srchDate,
					srchCond: this.savedReq.AMEQV0070GetReq,
					chkData: chkData,//this.recListView.getSelectedRecs(),
				};
			}else{
				// 検索結果が無い場合
			...
			case am_proto_defs.AM_PROTO_COMMON_RTYPE_REL:	// 照会
				...
				var storeBuff = {
					...
					val		:destData.chkData[0].store_disp,
				};
				destData.srchCond.store = storeBuff;
