...
$(function(){
	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			this.deserialize({
				...
				srchEquipVendCode	: null,						// 備品取引先コード
				srchEquipVendName	: null,						// 備品取引先名称
			...
			if(clcom.userInfo.unit_id >= clutil.getclsysparam('PAR_AMMS_UNITID_AOKI')){
				this.$tgtFocus = $("#ca_srchEquipVendCode");
			}
			else{
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMEQV0010 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMEQV0010';
		...
		doSrch: function(srchReq, chkData, $focusElem){
			...
			var defer = clutil.postJSON('AMEQV0010', srchReq).done(_.bind(function(data){

				// データ取得
				var recs = data.AMEQV0010GetRsp.equipVendList;
				...
				if (_.isEmpty(recs)) {
					...
					// 適用日チェック
					var max_date = clcom.max_date;
					$.each(recs, function() {
						var chkToDate = this.toDate;

						if (chkToDate != max_date) {
							this.disable = true;
						} else {
							this.disable = false;
						}
					...
					if(!_.isEmpty(chkData)){
						this.recListView.setSelectRecs(chkData, true, ['equipVendID']);
					}
		...
		doDownload: function(){
			...
			var defer = clutil.postDLJSON('AMEQV0010', srchReq);
		...
		_doOpeAction: function(rtyp, pgIndex, e){
			var url = clcom.appRoot + '/AMEQ/AMEQV0020/AMEQV0020.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
