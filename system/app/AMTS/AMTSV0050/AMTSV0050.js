...
$(function(){
	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			// 事業ユニット
			this.busUnitSelectorView = clutil.clbusunitselector(this.$('#ca_srchUnitID'),1);

			if(clcom.userInfo.unit_id !== 4) {

				// 事業ユニットにログインユーザの所属ユニットを表示する
				this.busUnitSelectorView.setValue(clcom.userInfo.unit_id);

				// 事業ユニットを操作不可にする
				clutil.viewReadonly(this.$('#ca_srchUnitID_wrapper'));
			}
			...
			// セットアップオートコンプリート
			/*clutil.clsetupcode($("#ca_srchSetupID"), {
				dependAttrs: {
					unit_id: function() {
		...
		_eof: 'AMTSV0050.SrchCondView//'
	});

	//////////////////////////////////////////////
	// View

	// グループID -- AMTSV0050 なデータに関連することを表すためのマーキング文字列
	var groupid = 'AMTSV0050';
	...
	var MainView = Backbone.View.extend({
		...
		render: function() {
			...
			if(clcom.userInfo.unit_id === 4) {
				...
				clutil.setFocus(this.$("#ca_srchSetupID"));
			}
			return this;
		},
		...
		doSrch: function(srchReq, chkData, $focusElem){
			...
			var defer = clutil.postJSON('AMTSV0050', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var rspDto = data.AMTSV0050GetRsp.monthPlans;
				...
				// 内容物がある場合 --> 結果表示する。
				var recs = data.AMTSV0050GetRsp.monthPlans;
...
//				}
		...
		_doOpeAction: function(rtyp, pgIndex/*一覧画面では使用しない*/, e){
			...
			var url = clcom.appRoot + '/AMTS/AMTSV0060/AMTSV0060.html';
			...
			if(this.savedReq){
				...
				myData = {
					...
					savedCond: this.savedReq.AMTSV0050GetReq,
	...
	AMTSV0050GetRsp = {
		...
		boo = function(){
			...
			mainView.recListView.setRecs(AMTSV0050GetRsp.monthPlans);
