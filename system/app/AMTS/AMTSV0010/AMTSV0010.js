...
$(function() {
	...
	var MainView = Backbone.View.extend({
		...
		events: {
			...
			'blur #ca_srchItgrpID'			: '_onItgrpIDChange',	// 品種変更
			'blur #ca_srchVendorID'			: '_onVendorIDChange',	// メーカー変更
			'blur #ca_srchMakerCode'		: '_onMakerCodeChange',	// 品番変更

			'click #ca_btn_store_select'	: '_onStoreSelClick',	// 入荷店舗 選択
			'click #ca_btn_org_select'	: '_onOrgSelClick',	// 出荷店舗絞込組織 選択

			'change #ca_srchDecisionStore'	: '_onChangeDecisionStore',	// 出荷店舗または入荷店舗が決まっているチェックボックス
			'change input[name="srchTransInOut"]:first'	: '_onChangeTransInOut'
		},
		...
		initUIElement: function(){
			...
			// カラーセレクター
			this.colorInfo = clutil.clcolorselector({
				el: $('#ca_srchColorID'),
				dependAttrs: {
					itemID: this.itemID
				}
			...
			// XXX:カラーセレクターを操作不可にする
			clutil.inputReadonly(this.$('#ca_srchColorID'), true);
		...
		_onUnitIDSelect: function(e){
			...
			// 品番を操作不可にする
			clutil.inputReadonly((this.$("#ca_srchMakerCode")), true);
			this.$("#ca_srchMakerCode").val('');

			// カラーを操作不可にする
			clutil.inputReadonly(this.$('#ca_srchColorID'), true);
			if(this.colorInfo) {
				this.colorInfo.setValue('');
			}
		 ...
		 * XXX: 品種変更
		 */
		_onItgrpIDChange: function(){

			// 品種・メーカー情報を取得
			var srchDto = clutil.view2data($("#ca_searchArea"));

			if((srchDto.srchItgrpID) && (srchDto.srchVendorID)) {
				// 品種・メーカーともに指定されている場合は、品番を入力可能にする
				this.$("#ca_srchMakerCode").attr("readonly", false);
		 ...
		 * XXX: 品番変更
		 */
		_onMakerCodeChange: function(){

			// 品種・メーカー・品番情報を取得
			var srchDto = clutil.view2data($("#ca_searchArea"));

			if((srchDto.srchItgrpID) && (srchDto.srchVendorID) && (srchDto.srchMakerCode)) {

				// 品種・メーカー・品番から商品名等を検索用オブジェクト
				var obj = {
					itgrp_id : srchDto.srchItgrpID,		//品種ID
					maker_id : srchDto.srchVendorID,	//メーカーID
					maker_code : srchDto.srchMakerCode	//メーカー品番
				};
					...
					.then(function(data){
						...
						if(!data || !data.head || data.head.status !== 0){
							...
							if(!data.rec.itemID) {
								...
								_this.validator.setErrorMsg(_this.$("#ca_srchMakerCode"),
										clutil.fmt(clutil.getclmsg('EGM0023'),'条件に該当する品番'));
						...
						// カラーを入力可能にする
						clutil.inputReadonly(_this.$('#ca_srchColorID'), false);
						// カラーにフォーカスをセットする
						clutil.setFocus(_this.$("#ca_srchColorID"));

						// カラーセレクター
						_this.colorInfo = clutil.clcolorselector({
							el: $('#ca_srchColorID'),
							dependAttrs: {
						...
						console.log('AMTSV0010GetReq', data);
					})
					.fail(function(data){
		...
		doCSVDownload: function(rtyp){
			...
			// 商品IDをセット
			srchDto.srchItemID = this.itemID;
			...
			var reqDto = {
				...
				AMTSV0010GetReq: srchDto
			};

			// 要求を送出
			// clutil.postDLJSON() を使うと共通ヘッダを補完してくれる。
			var defer = clutil.postDLJSON('AMTSV0010', reqDto);
