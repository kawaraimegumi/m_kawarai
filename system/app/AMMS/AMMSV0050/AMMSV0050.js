$(function() {
	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			// 初期値を設定
			this.deserialize({
				srchFuncID: 0,						// 商品属性定義ID（未使用）
				srchCode: null,						// 商品属性項目コード
				srchName: null,						// 商品属性項目名称
				srchNameKana: null,					// 商品属性項目名称かな（未使用）
				srchShortName: null,				// 商品属性項目略称（未使用）
				srchShortNameKana: null,			// 商品属性項目略称カナ（未使用）
				srchDate: 0,						// 検索日（未使用）
				allHistFlag: 0,						// 全履歴出力フラグ（未使用）
			});
		},
		...
		/**
		 * 商品分類体系が変更されたイベント
		 *  ⇒ 商品分類階層オートコンプリートと上位商品分類オートコンプリートの内部設定値をクリアする。
		 */
		_onSrchFuncChanged: function(e){
			//console.log(e);
		...
		/**
		 * 商品分類階層が変更されたイベント
		 *  ⇒ 上位商品分類オートコンプリートの内部設定値をクリアする。
		 */
		_onSrchLevelChanged: function(e){
			//console.log(e);
			if(this.deserializing){
				// データセット中
				return;
			}
			this.$('#ca_srchUpperID').autocomplete('removeClAutocompleteItem');
		},

		_eof: 'AMSSV0050.SrchCondView//'
	});
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMMSV0050 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMMSV0050';
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		render: function() {
			...
			this.srchCondView._onSrchClick();
			return this;
		},
		...
		doSrch: function(srchReq, selectedIds, $focusElem){
			...
			var defer = clutil.postJSON('AMMSV0050', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMMSV0050GetRsp.list;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
		...
		_doOpeAction: function(rtyp, e){
			var url = clcom.appRoot + '/AMMS/AMMSV0060/AMMSV0060.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
					savedReq: this.savedReq,
					savedCond: this.savedReq.AMMSV0050GetReq,
	...
	clutil.getIniJSON(null).done(function(data){
		...
		if(clcom.pageData){
			...
			mainView.resetFocus($("#ca_srchCode"));
		}
	}).fail(function(data){
