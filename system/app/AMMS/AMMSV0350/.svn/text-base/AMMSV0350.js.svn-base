...
$(function(){
	...
	var ListView = Backbone.View.extend({
		...
		initialize : function(){
			...
			// グループID -- AMMSV0350 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMMSV0350';
			...
			// OPE系イベント
			clutil.mediator.on('onOperation', this.showAMMSV0360page);
		...
		// ページャークリック
		doSrch: function(srchReq, chkData, $fromElem) {
			...
			clutil.postJSON(uri, srchReq).done(_.bind(function(data, dataType) {
					...
					this.resultList = data.AMMSV0350GetRsp.ageTypeList;
		 ...
		 * @param ope_mode
		 */
		showAMMSV0360page: function(ope_mode, pgIndex/*一覧画面では利用しない*/, e){
			var url = clcom.appRoot + '/AMMS/AMMSV0360/AMMSV0360.html';
			...
			default:										// その他、編集、予約取消、削除
				...
				pushPageOpt = {
					...
					saved: {
						...
						savedCond: this.savedReq.AMMSV0350GetReq,
						chkData: this.recListView.getSelectedRecs()
					}
				};
			}
