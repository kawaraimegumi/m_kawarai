...
$(function() {
	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			this.deserialize({
				...
				srchCode: null,						// 関連商品コード
				srchName: null,						// 関連商品名称
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMMSV0210 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMMSV0210';
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		doSrch: function(srchReq, selectedIds, $focusElem){
			...
			var defer = clutil.postJSON('AMMSV0210', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMMSV0210GetRsp.relitemList;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
		...
		_doOpeAction: function(rtyp, e){
			var url = clcom.appRoot + '/AMMS/AMMSV0220/AMMSV0220.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
					savedReq: this.savedReq,
					savedCond: this.savedReq.AMMSV0210GetReq,
