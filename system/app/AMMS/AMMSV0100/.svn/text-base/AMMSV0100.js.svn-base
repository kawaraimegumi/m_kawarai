...
var OpeType = {
	AMMSV0100_OPETYPE_TMP_SAVE:		101,	// 保存
	AMMSV0100_OPETYPE_TAG_APPLY:	102,	// タグ発行申請
	AMMSV0100_OPETYPE_TAG_RETURN:	103,	// タグ発行差戻し
	AMMSV0100_OPETYPE_TAG_APPROVE:	104,	// タグ発行承認
	AMMSV0100_OPETYPE_LAST_APPLY:	105,	// 最終承認申請
	AMMSV0100_OPETYPE_LAST_RETURN:	106,	// 最終承認差戻し
	AMMSV0100_OPETYPE_LAST_APPROVE:	107,	// 最終承認
	AMMSV0100_OPETYPE_OUTPUT:		201,
};
...
$(function() {
	...
	//////////////////////////////////////////////
	// 上代履歴ダイアログView
	var AMMSV0102DialogView = Backbone.View.extend({
		...
		events: {
			"click #ca_AMMSV0102_commit": "_onCommitClick",
		},
		...
		show: function(histList, isSubDialog, options) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_AMMSV0102_main'), {
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_AMMSV0102_commit'));
		},
...
//			};
		...
		setFooterButtons: function(opt) {
			var opeTypeId = [
				{
					opeTypeId: OpeType.AMMSV0100_OPETYPE_TMP_SAVE,
					label: '一時保存'
				},
				{
					opeTypeId: OpeType.AMMSV0100_OPETYPE_TAG_APPLY,
					label: 'タグ発行申請'
				},
				{
					opeTypeId: OpeType.AMMSV0100_OPETYPE_LAST_APPLY,
					label: '最終承認申請'
				},
			];
			...
			if (opt.opeTypeId == am_proto_defs.AM_PROTO_COMMON_RTYPE_NEW
				...
				if (opt.approveTypeID == amcm_type.AMCM_VAL_APPROVE_APPLY
					...
					if (opt.srcId == "AMMSV0090") {
						...
						opeTypeId = [
							{
								opeTypeId: OpeType.AMMSV0100_OPETYPE_LAST_RETURN,
								label: '最終承認差戻し'
							},
							{
								opeTypeId: OpeType.AMMSV0100_OPETYPE_LAST_APPROVE,
								label: '最終承認'
							},
						];
					}
					...
					if (opt.srcId == "AMMSV0090") {
						...
						opeTypeId = [
							{
								opeTypeId: OpeType.AMMSV0100_OPETYPE_TAG_RETURN,
								label: 'タグ発行差戻し'
							},
							{
								opeTypeId: OpeType.AMMSV0100_OPETYPE_TAG_APPROVE,
								label: 'タグ発行承認'
							},
						];
					}
...
//			});
			...
			// 上代履歴ダイアログ
			this.AMMSV0102Dialog = new AMMSV0102DialogView({
				el: this.$("#ca_AMMSV0102_dialog"),		// 配置場所
				$parentView	:	this.$("#mainColumn"),
			});
			this.AMMSV0102Dialog.render();
		...
		setPtnNo: function(opt) {
			...
			case OpeType.AMMSV0100_OPETYPE_TMP_SAVE:
				// 一時保存
				...
				break;
			case OpeType.AMMSV0100_OPETYPE_TAG_APPLY:
				// タグ発行申請
				ptnNo = amcm_type.AMCM_VAL_ITEM_OPEPTN_TAG_APPLY;
				break;
			case OpeType.AMMSV0100_OPETYPE_TAG_RETURN:
				// タグ発行差戻し
				ptnNo = amcm_type.AMCM_VAL_ITEM_OPEPTN_TAG_RETURN;
				break;
			case OpeType.AMMSV0100_OPETYPE_TAG_APPROVE:
				// タグ発行承認
				ptnNo = amcm_type.AMCM_VAL_ITEM_OPEPTN_TAG_APPROVE;
				break;
			case OpeType.AMMSV0100_OPETYPE_LAST_APPLY:
				// 最終承認申請
				switch (opt.opeTypeId) {
				case am_proto_defs.AM_PROTO_COMMON_RTYPE_NEW:
				case am_proto_defs.AM_PROTO_COMMON_RTYPE_COPY:
				...
				break;
			case OpeType.AMMSV0100_OPETYPE_LAST_RETURN:
				// 最終承認差戻し
				switch (opt.editTypeID) {
				case amcm_type.AMCM_VAL_ITEM_EDIT_UPDBASIC:
					// 基本属性編集承認差戻し
				...
				break;
			case OpeType.AMMSV0100_OPETYPE_LAST_APPROVE:
				// 最終承認
				switch (opt.editTypeID) {
				case amcm_type.AMCM_VAL_ITEM_EDIT_UPDBASIC:
					// 基本属性編集承認差戻し
		...
		isValidComment: function(rtyp) {
			...
			if (rtyp == OpeType.AMMSV0100_OPETYPE_TAG_RETURN ||
					rtyp == OpeType.AMMSV0100_OPETYPE_LAST_RETURN) {
		...
		isValidUpdBasic: function(opeTypeID) {
			...
			if (opeTypeID == OpeType.AMMSV0100_OPETYPE_LAST_APPROVE ||
					opeTypeID == OpeType.AMMSV0100_OPETYPE_LAST_RETURN) {
				return true;
			}
			...
			// 素材
			var materialList = this._buildUpdReqAMMSV0100MaterialList({});
			...
			// カラー
			var colorObj = this._buildUpdReqAMMSV0100TgtColorList({});
			...
			// サイズ
			var sizeObj = this._buildUpdReqAMMSV0100TgtSizeList({}, colorObj.tgtColorList);
		...
		isReturnMode: function(opeTypeId) {
			var ret = false;
			if (opeTypeId == OpeType.AMMSV0100_OPETYPE_TAG_RETURN || opeTypeId == OpeType.AMMSV0100_OPETYPE_LAST_RETURN) {
				ret = true;
			}
			return ret;
		},
...
//				}
			...
			var item = this._buildUpdReqAMMSV0100Rec();
			var colorObj = this._buildUpdReqAMMSV0100TgtColorList(item);
			var sizeObj = this._buildUpdReqAMMSV0100TgtSizeList(item, colorObj.tgtColorList);
			var attrObj = this._buildUpdReqAMMSV0100AttrRec(item);
			var attrAnyList = this._buildUpdReqAMMSV0100AttrAnyList(item);
			var tgtStoreList = this._buildUpdReqAMMSV0100TgtStoreList(item, attrObj.saleInfo);
			var price = this._buildUpdReqAMMSV0100PriceRec(item, attrObj.saleInfo);
			var priceOrgList = this._buildUpdReqAMMSV0100PriceOrgList(item);
			var materialList = this._buildUpdReqAMMSV0100MaterialList(item);
			var modelNoList = this._buildUpdReqAMMSV0100ModelNoList(item);
			var specList = this._buildUpdReqAMMSV0100SpecList(item);
			var photoList = this._buildUpdReqAMMSV0100PhotoList(item);
			var costDtlList = this._buildUpdReqAMMSV0100CostDtlList(item);
			var orderObj = this._buildUpdReqAMMSV0100OrderHeadList(item, colorObj, sizeObj);
			var commentList = this._buildUpdReqAMMSV0100CommentList(item, attrObj.commentInfo);
			var info = this._buildUpdReqAMMSV0100InfoRec(item, attrObj.saleInfo);
...
//			if ((opeTypeId != OpeType.AMMSV0100_OPETYPE_TMP_SAVE) &&
...
//				itgrpCode == '67')) { //			ＣＹ・スラックス
			...
			if (((opeTypeId == OpeType.AMMSV0100_OPETYPE_TAG_APPLY) ||
				(opeTypeId == OpeType.AMMSV0100_OPETYPE_LAST_APPLY)) &&
...
//			}
		 ...
		 * @returns {___ca_item1}
		 */
		_buildUpdReqAMMSV0100Rec: function() {
		 ...
		 * @returns {Array}
		 */
		_buildUpdReqAMMSV0100TgtColorList: function(item) {
			var tgtColorList = [];
			var citemList = [];
			var additem = {
		 ...
		 */
		_buildUpdReqAMMSV0100TgtSizeList: function(item, tgtColorList) {
			var _this = this;
			var tgtSizeList = [];
			var csitemList = [];
			var additem = {
		 ...
		 */
		_buildUpdReqAMMSV0100AttrRec: function(item) {
		 ...
		 * @returns {Array}
		 */
		_buildUpdReqAMMSV0100AttrAnyList: function(item) {
		 ...
		 * @returns {Array}
		 */
		_buildUpdReqAMMSV0100TgtStoreList: function(item, saleInfo) {
			var tgtStoreList = [];
			var additem = {
				fromDate: item.fromDate,
				toDate: item.toDate,
		 ...
		 * @returns
		 */
		_buildUpdReqAMMSV0100PriceRec: function(item, saleInfo) {
		 ...
		 * @returns {Array}
		 */
		_buildUpdReqAMMSV0100PriceOrgList: function(item) {
			var additem = {
				fromDate: item.fromDate,
				toDate: item.toDate,
				id: item.id,
		 ...
		 * @returns {Array}
		 */
		_buildUpdReqAMMSV0100MaterialList: function(item) {
			var additem = {
				fromDate: item.fromDate,
				toDate: item.toDate,
				id: item.id,
		 ...
		 * @returns {Array}
		 */
		_buildUpdReqAMMSV0100ModelNoList: function(item) {
			var additem = {
				fromDate: item.fromDate,
				toDate: item.toDate,
				id: item.id,
		 ...
		 * @returns {Array}
		 */
		_buildUpdReqAMMSV0100SpecList: function(item) {
			var additem = {
				fromDate: item.fromDate,
				toDate: item.toDate,
				id: item.id,
		 ...
		 * @returns
		 */
		_buildUpdReqAMMSV0100PhotoList: function(item) {
			var additem = {
				fromDate: item.fromDate,
				toDate: item.toDate,
				id: item.id,
		 ...
		 * @returns {Array}
		 */
		_buildUpdReqAMMSV0100CostDtlList: function(item) {
			var additem = {
				fromDate: item.fromDate,
				toDate: item.toDate,
				id: item.id,
		 ...
		 * @returns {Array}
		 */
		_buildUpdReqAMMSV0100OrderHeadList: function(item, colorObj, sizeObj) {
...
//console.log("DEBUG2: orderNo[" + orderNo + "]");
		 ...
		 * @returns {Array}
		 */
		_buildUpdReqAMMSV0100CommentList: function(item) {
		 ...
		 */
		_buildUpdReqAMMSV0100InfoRec: function(item) {
...
//			}
		...
		doDownload: function(rtyp){
			...
			var defer = clutil.postDLJSON('AMMSV0100', pdfGetReq);
