$(function() {
	...
	AMMSV0101PhotoAddView = Backbone.View.extend({
...
//			clutil.initUIelement(this.$el);
	...
	AMMSV0101PhotoView = Backbone.View.extend({
		events: {
	...
	AMMSV0101SelectorView = Backbone.View.extend({
		...
		events: {
			"click #ca_AMMSV0101_commit": "_onCommitClick",
		},
		...
		show: function(photoList, isSubDialog, options) {
			...
			$('.cl_echoback').hide();
			// validatorエラー時の表示領域
			this.validator = clutil.validator(this.$('#ca_AMMSV0101_main'), {
				echoback		: $('.cl_echoback')
			});
			...
			if (photoList != null) {
				$.each(photoList, function() {
					...
					var newPhotoView = new AMMSV0101PhotoView({el: el});
					newPhotoView.initUIelement();
					newPhotoView.render(this, _this.options.readonly);
					newPhotoView.photoSeq = this.photoSeq;

					_this.AMMSV0101PhotoViewList.push(newPhotoView);
					newPhotoView.setImageURL(this.photoURL);
				});
			}

			this.AMMSV0101PhotoAdd = new AMMSV0101PhotoAddView({el: $("#div_photo_add")});
			this.AMMSV0101PhotoAdd.initUIelement();
			this.AMMSV0101PhotoAdd.render(this.options.readonly);
		},

		AMMSV0101PhotoViewList: [],
		...
		_onAddPhoto: function(args) {
			 ...
			 */
			_.each(this.AMMSV0101PhotoViewList, _.bind(function(v) {
				if (args.mainFlag) {
					// メイン画像フラグを落とす
					v.setMainFlag(0);
				}
			...
			this.AMMSV0101PhotoViewList.push(newPhotoView);
			newPhotoView.setImageURL(args.photoURL);

			this.AMMSV0101PhotoAdd.clear();
		},

		_onDeletePhotoView: function(args, e) {
			var photoSeq = args.photoSeq;
			for (var i = 0; i < this.AMMSV0101PhotoViewList.length; i++) {
				if (this.AMMSV0101PhotoViewList[i].photoSeq == photoSeq) {
					this.AMMSV0101PhotoViewList.splice(i, 1);
					break;
				}
			}
		},
		...
		setFocus: function() {
			// フォーカスの設定
			clutil.setFocus(this.$('#ca_AMMSV0101_commit'));
		},
		...
		_onCommitClick: function() {
			// データを収集
			var photoRecList = [];
			$.each(this.AMMSV0101PhotoViewList, function() {
			...
			// VIEWの刈り取り
			this.AMMSV0101PhotoAdd.remove();
			$.each(this.AMMSV0101PhotoViewList, function() {
				this.remove();
			});
			this.AMMSV0101PhotoViewList.length = 0;
		},
