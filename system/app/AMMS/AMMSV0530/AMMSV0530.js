...
$(function() {
	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			this.deserialize({
				...
				srchCode: null,						// セットアップコード
				srchName: null,						// セットアップ名称
				srchDate: clcom.getOpeDate(),		// 検索日
				allHistFlag: 0,						// 全履歴出力フラグ
			});
		},
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMMSV0530 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMMSV0530';
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		doSrch: function(srchReq, selectedIds, $focusElem){
			...
			var defer = clutil.postJSON('AMMSV0530', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMMSV0530GetRsp.setupList;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
		...
		_doOpeAction: function(rtyp, e){
			var url = clcom.appRoot + '/AMMS/AMMSV0540/AMMSV0540.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
					savedReq: this.savedReq,
					savedCond: this.savedReq.AMMSV0530GetReq,
