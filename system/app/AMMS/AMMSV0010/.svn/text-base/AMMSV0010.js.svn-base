$(function() {
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function() {
			...
			// グループID -- AMMSV0010 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMMSV0010';
		...
		doSrch: function(srchReq, selectedIds, $focusElem){
			...
			var defer = clutil.postJSON('AMMSV0010', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMMSV0010GetRsp.itgrpfuncList;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
				...
				if(!_.isEmpty(selectedIds)){
					this.recListView.setSelectRecs(selectedIds, true);
				}
		...
		_doOpeAction: function(rtyp, e){
			var url = clcom.appRoot + '/AMMS/AMMSV0020/AMMSV0020.html';
			var myData, destData;
			if(this.savedReq){
				...
				myData = {
					...
					savedCond: this.savedReq.AMMSV0010GetReq,
					chkData: chkData,
				};
				destData = {
					opeTypeId: rtyp,
