...
//			return this;
		...
		_onMDSubmitCompleted: function(args, e){
			...
			case 'DONE':		// 確定済
				...
				// 更新した日付をchkdataに反映
					this.options.chkData[args.index].fromDate = args.data.AMMSV0360GetRsp.ageType.fromDate;
				}
...
//				this._tableDisable();
		...
		_onMDGetCompleted: function(args, e){
			...
			switch(args.status){
			case 'OK':
				var getRsp = data.AMMSV0360GetRsp;
				this.viewSeed = getRsp;
				console.log(JSON.stringify(getRsp));
				clutil.data2view(this.$el, getRsp.ageType);
				...
				if(this.options.opeTypeId === am_proto_defs.AM_PROTO_COMMON_RTYPE_REL
					...
					clutil.viewReadonly(this.$(".ca_toDate_div"));
					if (getRsp.ageType.toDate == clcom.max_date){
						clutil.viewRemoveReadonly(this.$(".ca_fromDate_div"));
						if (getRsp.ageType.fromDate <= ope_date) {
...
//			var resId = arg.resId;		// リソースId - "XXXXV0010"などの文字列
			...
			var rec = data.AMMSV0360GetRsp;
			delete rec.ageType.fromDate;
			delete rec.ageType.toDate;
			return data;
		}
	});
