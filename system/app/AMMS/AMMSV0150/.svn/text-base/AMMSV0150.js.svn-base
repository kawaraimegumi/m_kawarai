...
$(function() {
	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			var to = this.getNextMonthLastDay(from);
			// 初期値を設定
			this.deserialize({
				srchUnitID: 0,			// 事業ユニットID
				srchFromDate: from,		// 期間開始日
				srchToDate: to,			// 期間開始日
				srchName: "",			// 組合せ販売名称
			});
		},
		 ...
		 * @param ymd 年月日
		 */
		getNextMonthLastDay: function(ymd) {
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMMSV0150 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMMSV0150';
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		doSrch: function(srchReq, selectedIds, $focusElem){
			...
			var defer = clutil.postJSON('AMMSV0150', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMMSV0150GetRsp.bundleList;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
				...
				$.each(recs, function() {
					if (this.saleEndDate <= ope_date) {
						disableList.push(this);
					}
				});
				if (disableList.length > 0) {
					this.recListView.setEnableRecs(disableList, false);
				}
		...
		_doOpeAction: function(rtyp, e){
			var url = clcom.appRoot + '/AMMS/AMMSV0160/AMMSV0160.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
					savedReq: this.savedReq,
					savedCond: this.savedReq.AMMSV0150GetReq,
