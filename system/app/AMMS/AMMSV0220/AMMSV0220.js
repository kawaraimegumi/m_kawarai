...
//				$inputDiv.autocomplete('clAutocompleteItem', dataDiv);
		...
		_onMDGetCompleted: function(args, e){
			...
			var data = args.data;
			var getRsp = data.AMMSV0220GetRsp;
			var relitem = getRsp.relitem;
			...
			case 'OK':
				...
				case am_proto_defs.AM_PROTO_COMMON_RTYPE_DEL:
					// fromDate調整
					if (relitem.fromDate <= clcom.getOpeDate()) {
						// 開始日が運用日以下の場合は、運用日＋１にする
						relitem.fromDate = clcom.getOpeDate();
					}
					break;
				default:
					break;
				...
				clutil.data2view(this.$("#ca_form"), relitem);
				this.renderTable(this.itemList);

				this.state.recno = relitem.recno;
				this.state.state = relitem.state;
		...
		render: function(){
			...
			if (this.opeTypeId == am_proto_defs.AM_PROTO_COMMON_RTYPE_NEW) {
				...
				$("#ca_tp_del").addClass("txtInFieldUnit flright help").attr("data-original-title", "削除日以降、当関連商品は無効扱いとなります").tooltip({html: true});
			}
...
//						this.validator2.setErrorMsg($inputDiv, msg);
			...
			var relitem = clutil.view2data($("#ca_form"));
			...
			$.each(itemList, function() {
				...
				this.id = relitem.id;
				this.fromDate = relitem.fromDate;
				this.toDate = relitem.toDate;
			});
