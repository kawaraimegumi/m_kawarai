...
$(function() {
	...
	var SrchCondView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			// 核商品区分
			clutil.cltypeselector($("#ca_srchTypeID"), amcm_type.AMCM_TYPE_COREITEM, 1);

			// 対象期間
			clutil.datepicker($("#ca_srchFrom"));
			clutil.datepicker($("#ca_srchTo"));
			...
			this.deserialize({
				...
				srchName: null,						// 核商品名称
				srchTypeID: 0,						// 核商品区分
				srchFrom: opeDate,					// 対象期間開始日
				srchTo: opeDate,					// 対象期間終了日
			});
		},
	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(){
			...
			// グループID -- AMMSV0470 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMMSV0470';
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		doSrch: function(srchReq, selectedIds, $focusElem){
			...
			var defer = clutil.postJSON('AMMSV0470', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMMSV0470GetRsp.coreItemList;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
		...
		_doOpeAction: function(rtyp, e){
			var url = clcom.appRoot + '/AMMS/AMMSV0480/AMMSV0480.html';
			var myData, destData;
			if(this.savedReq){
				...
				myData = {
					...
					savedCond: this.savedReq.AMMSV0470GetReq,
					chkData: chkData
				};
				destData = {
					opeTypeId: rtyp,
