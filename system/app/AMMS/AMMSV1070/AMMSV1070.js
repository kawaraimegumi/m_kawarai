...
//
//			// 品種オートコンプリート
//			clutil.clvarietycode($("#ca_srchItgrpID"), {
...
//			});
			...
			clutil.clyearselector($("#ca_srchYear"), 1, 10, 1);

			// シーズン(未選択あり)
			clutil.cltypeselector($("#ca_srchSeasonID"), amcm_type.AMCM_TYPE_SEASON, 1);

			// 商品登録(未選択あり)
			clutil.cltypeselector($("#ca_srchStatus"), amcm_type.AMCM_TYPE_ITEM_REGIST, 1);
	...
	var MainView = Backbone.View.extend({
		...
		events: {
			...
			//'click #cl_add'			: '_onAddClick',			// 商品マスタへボタン押下
		},
		...
		initialize: function(){
			...
			// グループID -- AMMSV1070 なデータに関連することを表すためのマーキング文字列
			var groupid = 'AMMSV1070';
...
//			'click #ca_rsvcancel'			: '_onReserveCancelClick'	// 予約取消ボタン押下
		...
		_setOpeButtonUI: function(groupid, arg, from) {
			...
			ope1SelectedRecs = _.filter(arg.selectedRecs, function(dto) {
				return dto.status == amcm_type.AMCM_VAL_ITEM_REGIST_DONE;
			});
			// デフォルトの活性/非活性判定関数
		...
		_onAddClick: function() {
			var url = clcom.appRoot + '/AMMS/AMMSV1100/AMMSV1100.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
					savedReq: this.savedReq,
					savedCond: this.savedReq.AMMSV0070GetReq,
		...
		doSrch: function(srchReq, selectedIds, $focusElem){
			...
			var defer = clutil.postJSON('AMMSV1070', srchReq).done(_.bind(function(data){
				//console.log(arguments);

				// 内容物が空の場合 --> ヘッダにエラーメッセージを出す。「検索結果は0件です。」
				var recs = data.AMMSV0070GetRsp.itemList;
				if(_.isEmpty(recs)){
					// 検索ペインを表示？
				...
				_.each(recs, _.bind(function(rec) {
					var makerView;
					if (rec.makerCode == "" && rec.makerName == "") {
						makerView = "";
					} else {
						makerView = rec.makerCode + ":" + rec.makerName;
					}
					rec.makerView = makerView;
				}, this));
		...
		_doOpeAction: function(rtyp, e){
			var url = clcom.appRoot + '/AMMS/AMMSV1080/AMMSV1080.html';
			var myData, destData;
			if(this.savedReq){
				// 検索結果がある場合
				myData = {
			...
			case am_proto_defs.AM_PROTO_COMMON_RTYPE_NEW:	// 新規
				...
				clcom.pushPage(url, destData, myData);
				break;
			case am_proto_defs.AM_PROTO_COMMON_RTYPE_COPY:	// 商品マスタに登録
				url = clcom.appRoot + '/AMMS/AMMSV1100/AMMSV1100.html';
