...
$(function() {
	...
	var MainView = Backbone.View.extend({
		...
		initUIElement: function(){
			...
			// 出力区分取得
			this.cltypeselectorView = clutil.cltypeselector(this.$("#ca_outType"), amcm_type.AMCM_TYPE_DETAIL_OUTPUT_TYPE,1);
			this.cltypeselectorView.setValue(amcm_type.AMCM_VAL_DETAIL_OUTPUT_TYPE_STORE_RETURN_DETAIL);
		...
		isValid: function(){
			...
			var inputYear = String(srchDto.srchYm).substr(0,4);
			var inputMonth = String(srchDto.srchYm).substr(4,2);

			inputYear = Number(inputYear);
			inputMonth = Number(inputMonth);
			opeYear = Number(opeYear);
			opeMonth = Number(opeMonth);

			if((inputYear > opeYear) || (inputYear == opeYear && inputMonth > opeMonth)) {

				// 入力された対象月が運用日より未来の場合はエラー
				var msg = clutil.getclmsg('EAS0001');
				this.validator.setErrorMsg($("#ca_srchYm"), msg);
		...
		doCSVDownload: function(rtyp){
			...
			var defer = clutil.postDLJSON('AMASV0070', srchReq);
