...
//			clutil.clmonthselector(this.$("#ca_srchYm"), 0, 1, 0, null, null, 1); //初期表示調整(長すぎ)
		...
		doCSVDownload: function(rtyp) {
			...
			// 要求を送出
			var defer = clutil.postDLJSON('AMFUV0040', srchReq);
	...
	.done(_.bind(function(data,dataType){
		...
		ca_csvOutView.initUIelement().done(_.bind(function(){
			...
			ca_csvOutView._onUnitChange();
			var unitID = Number(ca_csvOutView.$('#ca_srchUnitID').val());
			if (unitID == 0) {
				clutil.setFocus(ca_csvOutView.$("#ca_srchUnitID"));
			} else {
				clutil.setFocus(ca_csvOutView.$("#ca_srchYm"));
			}
//			clutil.setFocus(ca_csvOutView.$("#ca_srchUnitID"));
		},this));
